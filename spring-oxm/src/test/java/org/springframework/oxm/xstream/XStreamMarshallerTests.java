...
class XStreamMarshallerTests {
    ...
    private static final String EXPECTED_STRING =
            "<flight><flightNumber>42</flightNumber></flight>";
    ...
    void marshalSaxResult() throws Exception {
        ...
        ordered.verify(contentHandler)
                .startElement(eq(""), eq("flight"), eq("flight"), isA(Attributes.class));
        ordered.verify(contentHandler)
                .startElement(
                        eq(""), eq("flightNumber"), eq("flightNumber"), isA(Attributes.class));
                ...
                .forEach(
                        n -> {
                            try {
                                ...
                                assertThat(XmlContent.from(writer))
                                        .isSimilarTo("<byte-array>AQI=</byte-array>");
                                Reader reader = new StringReader(writer.toString());
                                byte[] bufResult =
                                        (byte[]) marshaller.unmarshal(new StreamSource(reader));
