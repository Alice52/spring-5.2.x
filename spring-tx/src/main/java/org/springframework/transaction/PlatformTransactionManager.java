...
/**
 * This is the central interface in Spring's imperative transaction infrastructure. Applications can
 * use this directly, but it is not primarily meant as an API: Typically, applications will work
 * with either TransactionTemplate or declarative transaction demarcation through AOP.
 *
...
public interface PlatformTransactionManager extends TransactionManager {
     ...
     *
     * <p>Furthermore, not all transaction definition settings will be supported by every
     * transaction manager: A proper transaction manager implementation should throw an exception
     * when unsupported settings are encountered.
     *
     * <p>An exception to the above rule is the read-only flag, which should be ignored if no
     * explicit read-only mode is supported. Essentially, the read-only flag is just a hint for
     ...
     *     executed (for example, if a currently active transaction is in conflict with the
     ...
     * the transaction must be fully completed and cleaned up. No rollback call should be expected
     * in such a case.
     *
     ...
     * DataAccessException causing the transaction to fail. The original exception will be
     ...
     * create a new one, resume the previous transaction after rolling back the new one.
     *
