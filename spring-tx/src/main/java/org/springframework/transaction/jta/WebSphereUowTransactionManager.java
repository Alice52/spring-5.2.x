...
public class WebSphereUowTransactionManager extends JtaTransactionManager
     ...
     *
     * <p>Typically just used for test setups; in a Java EE environment, the UOWManager will always
     * be fetched from JNDI.
     *
    ...
    /**
     * Set the JNDI name of the WebSphere UOWManager. The default "java:comp/websphere/UOWManager"
     * is used if not set.
     *
    ...
    protected UOWManager lookupUowManager(String uowManagerName) throws TransactionSystemException {
        try {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Retrieving WebSphere UOWManager from JNDI location ["
                                + uowManagerName
                                + "]");
            }
            ...
            throw new TransactionSystemException(
                    "WebSphere UOWManager is not available at JNDI location ["
                            + uowManagerName
                            + "]",
                    ex);
        }
    ...
    @Nullable
    public <T> T execute(
            @Nullable TransactionDefinition definition, TransactionCallback<T> callback)
        ...
        try {
            ...
            action =
                    new UOWActionAdapter<>(
                            def, callback, actualTransaction, !joinTx, newSynch, debug);
