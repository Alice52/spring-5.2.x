...
public abstract class ReactiveResourceSynchronization<O, K> implements TransactionSynchronization {
     ...
     * ({@code true}) or rather after transaction completion ({@code false}).
     *
     ...
     * true}).
     *
     * <p>The default implementation returns {@code !shouldReleaseBeforeCompletion()}, releasing
     * after completion if no attempt was made before completion.
     *
     * @see #releaseResource
     */
    ...
    /**
     * After-commit callback for the given resource holder. Only called when the resource hasn't
     * been released yet ({@link #shouldReleaseBeforeCompletion()}).
     *
