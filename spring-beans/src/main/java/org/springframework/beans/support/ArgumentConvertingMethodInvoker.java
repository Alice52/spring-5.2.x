...
public class ArgumentConvertingMethodInvoker extends MethodInvoker {
     ...
     *
     * <p>Can be cast to {@link org.springframework.beans.PropertyEditorRegistry} if direct access
     * to the underlying PropertyEditors is desired (provided that the present TypeConverter
     * actually implements the PropertyEditorRegistry interface).
     */
    @Nullable
     ...
     *
     * <p>Called if no explicit TypeConverter has been specified. The default implementation builds
     * a {@link org.springframework.beans.SimpleTypeConverter}. Can be overridden in subclasses.
     */
    ...
    public void registerCustomEditor(Class<?> requiredType, PropertyEditor propertyEditor) {
        ...
        if (!(converter instanceof PropertyEditorRegistry)) {
            throw new IllegalStateException(
                    "TypeConverter does not implement PropertyEditorRegistry interface: "
                            + converter);
        }
                ...
                if (candidate.getName().equals(targetMethod)) {
                    ...
                    if (parameterCount == argCount) {
                        ...
                        for (int j = 0; j < argCount && match; j++) {
                            // Verify that the supplied argument is assignable to the method
                            // parameter.
                            try {
                                convertedArguments[j] =
                                        converter.convertIfNecessary(arguments[j], paramTypes[j]);
                        ...
                        if (match) {
                            int typeDiffWeight =
                                    getTypeDifferenceWeight(paramTypes, convertedArguments);
