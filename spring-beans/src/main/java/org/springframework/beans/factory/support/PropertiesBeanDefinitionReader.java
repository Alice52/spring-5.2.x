...
public class PropertiesBeanDefinitionReader extends AbstractBeanDefinitionReader {
     ...
     * owner.dog(ref)=fido}. Whether this is a reference to a singleton or a prototype will depend
     * on the definition of the target bean.
     */
     ...
     *
     * <p>Can be used e.g. for view definition files, to define a parent with a default view class
     * and common attributes for all views. View definitions that define their own parent or carry
     * their own class can still override this.
     *
     * <p>Strictly speaking, the rule that a default parent setting does not apply to a bean
     * definition that carries a class is there for backwards compatibility reasons. It still
     * matches the typical use case.
     */
     ...
     *
     * @param resource the resource descriptor for the properties file
     * @param prefix a filter within the keys in the map: e.g. 'beans.' (can be empty or {@code
     *     null})
     * @return the number of bean definitions found
     * @throws BeanDefinitionStoreException in case of loading or parsing errors
     */
     ...
     * Load bean definitions from the specified properties file.
     *
     * @param encodedResource the resource descriptor for the properties file, allowing to specify
        ...
        try {
            ...
            int count =
                    registerBeanDefinitions(
                            props, prefix, encodedResource.getResource().getDescription());
     ...
     * @throws BeansException in case of loading or parsing errors
     */
    public int registerBeanDefinitions(Map<?, ?> map, @Nullable String prefix)
    ...
    public int registerBeanDefinitions(
            Map<?, ?> map, @Nullable String prefix, String resourceDescription)
            throws BeansException {
        ...
        for (Object key : map.keySet()) {
            if (!(key instanceof String)) {
                throw new IllegalArgumentException(
                        "Illegal key [" + key + "]: only Strings allowed");
            }
                ...
                if (sepIdx != -1) {
                    ...
                    if (!getRegistry().containsBeanDefinition(beanName)) {
                        ...
                        registerBeanDefinition(
                                beanName, map, prefix + beanName, resourceDescription);
                        ++beanCount;
                    }
                } else {
    ...
    protected void registerBeanDefinition(
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            if (key.startsWith(prefixWithSep)) {
                ...
                if (CLASS_KEY.equals(property)) {
                    ...
                    String val = StringUtils.trimWhitespace((String) entry.getValue());
                    scope =
                            (!StringUtils.hasLength(val) || TRUE_VALUE.equals(val)
                    ...
                    if (property.endsWith(REF_SUFFIX)) {
                        int index =
                                Integer.parseInt(
                                        property.substring(
                                                1, property.length() - REF_SUFFIX.length()));
        ...
        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Registering bean definition for bean name '" + beanName + "' with " + pvs);
        }
    ...
    /**
     * Reads the value of the entry. Correctly interprets bean references for values that are
