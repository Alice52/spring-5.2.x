...
public class PropertyOrFieldReference extends SpelNodeImpl {
    ...
    private TypedValue readProperty(
        ...
        List<PropertyAccessor> accessorsToTry =
                getPropertyAccessorsToTry(
                        contextObject.getValue(), evalContext.getPropertyAccessors());
        // Go through the accessors that may be able to resolve it. If they are a cacheable accessor
        // then
        // get the accessor and use it. If they are not cacheable but report they can read the
        // property
        ...
        if (contextObject.getValue() == null) {
            ...
            throw new SpelEvaluationException(
                    ...
                    FormatHelper.formatClassNameForMessage(
                            getObjectClass(contextObject.getValue())));
        }
    }
     ...
     * the returned list any that are exact matches for the input target type (as opposed to
     * 'general' resolvers that could work for any type) are placed at the start of the list. In
     * addition, there are specific resolvers that exactly name the class in question and resolvers
     * that name a specific class but it is a supertype of the class we have. These are put at the
     * end of the specific resolvers set and will be tried after exactly matching accessors but
     * before generic accessors.
     *
    ...
    public void generateCode(MethodVisitor mv, CodeFlow cf) {
        ...
        if (!(accessorToUse instanceof CompilablePropertyAccessor)) {
            throw new IllegalStateException(
                    "Property accessor is not compilable: " + accessorToUse);
        }
            ...
            if (accessorToUse instanceof CompilablePropertyAccessor) {
                this.ref.setExitTypeDescriptor(
                        CodeFlow.toDescriptor(
                                ((CompilablePropertyAccessor) accessorToUse).getPropertyType()));
            }
            return value;
        }
