...
public class CodeFlow implements Opcodes {
    ...
    /**
     * As SpEL ast nodes are called to generate code for the main evaluation method they can
     * register to add a field to this class. Any registered FieldAdders will be called after the
     * main evaluation function has finished being generated.
     */
    @Nullable private List<FieldAdder> fieldAdders;

    /**
     * As SpEL ast nodes are called to generate code for the main evaluation method they can
     * register to add code to a static initializer in the class. Any registered ClinitAdders will
     * be called after the main evaluation function has finished being generated.
     */
    @Nullable private List<ClinitAdder> clinitAdders;

    /**
     * When code generation requires holding a value in a class level field, this is used to track
     * the next available field id (used as a name suffix).
     */
    private int nextFieldId = 1;

    /**
     * When code generation requires an intermediate variable within a method, this method records
     * the next available variable (variable 0 is 'this').
     */
     ...
     */
    public static void insertUnboxInsns(
            MethodVisitor mv, char ch, @Nullable String stackDescriptor) {
    ...
    public static void insertUnboxNumberInsns(
        ...
        switch (targetDescriptor) {
            ...
            default:
                throw new IllegalArgumentException(
                        "Unboxing should not be attempted for descriptor '"
                                + targetDescriptor
                                + "'");
        }
    }
     ...
     * method parameters surrounded with parentheses, followed by the descriptor for the return
     * type. Note the descriptors here are JVM descriptors, unlike the other descriptor forms the
    ...
    /**
     * Create the JVM signature descriptor for a constructor. This consists of the descriptors for
     * the constructor parameters surrounded with parentheses, followed by the descriptor for the
     * return type, which is always "V". Note the descriptors here are JVM descriptors, unlike the
     * other descriptor forms the compiler is using which do not include the trailing semicolon.
     *
     ...
     * compilation process, this is the one the JVM wants, so this one includes any necessary
     * trailing semicolon (e.g. Ljava/lang/String; rather than Ljava/lang/String)
     *
     * @param clazz a class
     ...
     * @return the type descriptor for the object (descriptor is "Ljava/lang/Object" for {@code
     *     null} value)
     */
     ...
     * compilation process only (currently) supports certain number types. These are double, float,
     * long and int.
     *
    ...
    public static void insertNumericUnboxOrPrimitiveTypeCoercion(
        ...
        if (!CodeFlow.isPrimitive(stackDescriptor)) {
            ...
            CodeFlow.insertAnyNecessaryTypeConversionBytecodes(
                    mv, targetDescriptor, stackDescriptor);
        }
    }
    ...
    /**
     * Register a ClinitAdder which will add code to the static initializer in the generated class
     * to support the code produced by an ast nodes primary generateCode() method.
     */
