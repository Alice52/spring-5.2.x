...
public class ModelResultMatchers {
    ...
    public ResultMatcher attributeExists(String... names) {
        return result -> {
            ...
            for (String name : names) {
                assertNotNull(
                        "Model attribute '" + name + "' does not exist", mav.getModel().get(name));
            }
        };
    }
    ...
    public ResultMatcher attributeHasFieldErrors(String name, String... fieldNames) {
        return mvcResult -> {
            ...
            for (String fieldName : fieldNames) {
                ...
                assertTrue(
                        "No errors for field '" + fieldName + "' of attribute '" + name + "'",
                        hasFieldErrors);
            }
        };
    }
     ...
     * @since 4.1
     */
    public ResultMatcher attributeHasFieldErrorCode(
        ...
        return result -> {
            ...
            for (Object value : mav.getModel().values()) {
                if (value instanceof Errors) {
                    assertFalse(
                            "Unexpected binding/validation errors: " + value,
