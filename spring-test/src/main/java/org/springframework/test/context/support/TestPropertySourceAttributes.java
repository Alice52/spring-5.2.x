...
class TestPropertySourceAttributes {
     ...
     * type hierarchy (i.e., have the same {@linkplain MergedAnnotation#getAggregateIndex()
     * aggregate index}).
     *
     * @since 5.2
    ...
    /**
     * Merge this {@code TestPropertySourceAttributes} instance with the supplied {@code
     * annotation}, asserting that the two sets of test property source attributes have identical
     * values for the {@link TestPropertySource#inheritLocations} and {@link
     * TestPropertySource#inheritProperties} flags and that the two underlying annotations were
     * declared on the same class.
     *
     * <p>This method should only be invoked if {@link #canMergeWith(MergedAnnotation)} returns
     * {@code true}.
     *
     * @since 5.2
    ...
    void mergeWith(MergedAnnotation<TestPropertySource> annotation) {
        ...
        logger.trace(
                LogMessage.format(
                        "Retrieved %s for declaring class [%s].",
     ...
     *
     * <p>Note: The returned value may represent a <em>detected default</em> or merged locations
     * that do not match the original value declared via a single {@code @TestPropertySource}
     * annotation.
     *
