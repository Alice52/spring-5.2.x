...
class TransactionContext {
     ...
     *
     * <p>In other words, should we roll back or commit the current transaction upon completion of
     * the current test?
     */
    ...
    void setFlaggedForRollback(boolean flaggedForRollback) {
        Assert.state(
                ...
                () ->
                        "Failed to set rollback flag - transaction does not exist: "
                                + this.testContext);
    ...
    void endTransaction() {
        ...
        Assert.state(
                this.transactionStatus != null,
                () ->
                        "Failed to end transaction - transaction does not exist: "
                                + this.testContext);
