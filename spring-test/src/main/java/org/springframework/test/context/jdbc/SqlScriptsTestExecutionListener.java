...
public class SqlScriptsTestExecutionListener extends AbstractTestExecutionListener {
    ...
    private void executeSqlScripts(TestContext testContext, ExecutionPhase executionPhase) {
        ...
        if (mergeSqlAnnotations(testContext)) {
            ...
            if (!methodLevelSqlAnnotations.isEmpty()) {
                ...
                executeSqlScripts(
                        getSqlAnnotationsFor(testClass), testContext, executionPhase, true);
            }
        }
    }
    ...
    private Set<Sql> getSqlAnnotationsFor(AnnotatedElement element) {
        return AnnotatedElementUtils.getMergedRepeatableAnnotations(
                element, Sql.class, SqlGroup.class);
    }
    ...
    private void executeSqlScripts(
            ...
            if (StringUtils.hasText(stmt)) {
                ...
                scriptResources.add(
                        new ByteArrayResource(
                                stmt.getBytes(), "from inlined SQL statement: " + stmt));
            }
        }
        ...
        if (txMgr == null) {
            ...
            new TransactionTemplate(txMgr, txAttr)
                    .executeWithoutResult(s -> populator.execute(finalDataSource));
        }
    }
