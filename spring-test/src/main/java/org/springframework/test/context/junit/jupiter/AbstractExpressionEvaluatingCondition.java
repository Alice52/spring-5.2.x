...
abstract class AbstractExpressionEvaluatingCondition implements ExecutionCondition {
    ...
    private static final Log logger =
            LogFactory.getLog(AbstractExpressionEvaluatingCondition.class);
     ...
     *     annotation
     * @param enabledOnTrue indicates whether the returned {@code ConditionEvaluationResult} should
     *     be {@link ConditionEvaluationResult#enabled enabled} if the expression evaluates to
     *     {@code true}
    ...
    protected <A extends Annotation> ConditionEvaluationResult evaluateAnnotation(
        ...
        if (!annotation.isPresent()) {
            String reason =
                    String.format(
                            "%s is enabled since @%s is not present",
                            element, annotationType.getSimpleName());
        ...
        boolean evaluatedToTrue =
                evaluateExpression(expression, loadContext, annotationType, context);
        ...
        if (loadContext && result.isDisabled() && element instanceof Class) {
            ...
            findMergedAnnotation(testClass, DirtiesContext.class)
                    .ifPresent(
                            dirtiesContext -> {
                                HierarchyMode hierarchyMode = dirtiesContext.hierarchyMode();
                                SpringExtension.getTestContextManager(context)
                                        .getTestContext()
                                        .markApplicationContextDirty(hierarchyMode);
                            });
        }
    ...
    private <A extends Annotation> boolean evaluateExpression(
        ...
        BeanExpressionResolver expressionResolver =
        ...
        Object result =
                expressionResolver.evaluate(
                        configurableBeanFactory.resolveEmbeddedValue(expression),
