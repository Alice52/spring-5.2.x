...
public class SpringJUnit4ClassRunner extends BlockJUnit4ClassRunner {
    ...
    /**
     * Construct a new {@code SpringJUnit4ClassRunner} and initialize a {@link TestContextManager}
     ...
     * {@link #getTestContextManager() TestContextManager} to prepare the test instance before
     * returning it.
     *
    ...
    /**
     * Augment the default JUnit behavior {@linkplain #withPotentialRepeat with potential repeats}
     ...
     * However, JUnit-specific timeouts still differ from Spring-specific timeouts in that the
     * former execute in a separate thread while the latter simply execute in the main thread (like
     * regular tests).
     *
     ...
     * except that the <em>expected exception</em> is retrieved using {@link
     ...
     *
     * <p>Supports both Spring's {@link org.springframework.test.annotation.Timed @Timed} and
     * JUnit's {@link Test#timeout() @Test(timeout=...)} annotations, but not both simultaneously.
     *
        ...
        if (springTimeout > 0 && junitTimeout > 0) {
            ...
            statement =
                    FailOnTimeout.builder()
                            .withTimeout(junitTimeout, TimeUnit.MILLISECONDS)
                            .build(next);
        } else {
            statement = next;
        }
    ...
    /**
     * Wrap the {@link Statement} returned by the parent implementation with a {@code
     * RunBeforeTestMethodCallbacks} statement, thus preserving the default functionality while
     * adding support for the Spring TestContext Framework.
     *
    ...
    /**
     * Wrap the {@link Statement} returned by the parent implementation with a {@code
     * RunAfterTestMethodCallbacks} statement, thus preserving the default functionality while
     * adding support for the Spring TestContext Framework.
     *
