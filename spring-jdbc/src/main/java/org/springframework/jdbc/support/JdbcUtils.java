...
public abstract class JdbcUtils {
    ...
    public static Object getResultSetValue(ResultSet rs, int index, @Nullable Class<?> requiredType)
        ...
        // Perform was-null check if necessary (for results that the JDBC driver returns as
        // primitives).
     ...
     * returned value should be a detached value object, not having any ties to the active
     * ResultSet: in particular, it should not be a Blob or Clob object but rather a byte array or
     ...
     * java.sql.Date} for DATE columns leaving out the time portion: These columns will explicitly
     ...
     *
     * <p>This method will open a connection to the database and retrieve its meta-data. Since this
     * method is called before the exception translation feature is configured for a DataSource,
     * this method can not rely on SQLException translation itself.
     *
     * <p>Any exceptions will be wrapped in a MetaDataAccessException. This is a checked exception
     * and any calling code should catch and handle this exception. You can just log the error and
     * hope for the best, but there is probably a more serious error that will reappear when you try
    ...
    public static <T> T extractDatabaseMetaData(
            DataSource dataSource, DatabaseMetaDataCallback<T> action)
     ...
     * @throws MetaDataAccessException if we couldn't access the DatabaseMetaData or failed to
     *     invoke the specified method
     * @see java.sql.DatabaseMetaData
     * @deprecated as of 5.2.9, in favor of {@link #extractDatabaseMetaData(DataSource,
     *     DatabaseMetaDataCallback)} with a lambda expression or method reference and a generically
     *     typed result
     */
    @Deprecated
     ...
     *
     * <p>Typically invoked right before execution of a given set of statements: to decide whether
     * the set of SQL statements should be executed through the JDBC 2.0 batch mechanism or simply
     * in a traditional one-by-one fashion.
     *
     * <p>Logs a warning if the "supportsBatchUpdates" methods throws an exception and simply
    ...
    /**
     * Extract a common name for the target database in use even if various drivers/platforms
     * provide varying names at runtime.
     *
