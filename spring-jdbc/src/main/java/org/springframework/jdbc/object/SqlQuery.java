...
public abstract class SqlQuery<T> extends SqlOperation {
     ...
     * @param context the contextual information passed to the {@code mapRow} callback method. The
     *     JDBC operation itself doesn't rely on this parameter, but it can be useful for creating
     *     the objects of the result list.
     * @return a List of objects, one per row of the ResultSet. Normally all these will be of the
     *     same class, although it is possible to use different types.
     */
    ...
    public List<T> executeByNamedParam(Map<String, ?> paramMap, @Nullable Map<?, ?> context)
        ...
        Object[] params =
                NamedParameterUtils.buildValueArray(
                        parsedSql, paramSource, getDeclaredParameters());
        RowMapper<T> rowMapper = newRowMapper(params, context);
     ...
     * methods are like EJB entity bean finders, in that it is considered an error if they return
     * more than one result.
     *
     * @return the result object, or {@code null} if not found. Subclasses may choose to treat this
