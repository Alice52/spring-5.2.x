...
public class BatchSqlUpdate extends SqlUpdate {
    ...
    /**
     * Set the number of statements that will trigger an automatic intermediate flush. {@code
     * update} calls or the given statement parameters will be queued until the batch size is met,
     * at which point it will empty the queue and execute the batch.
     *
     * <p>You can also flush already queued statements with an explicit {@code flush} call. Note
     * that you need to this after queueing all parameters to guarantee that all statements have
     * been flushed.
     */
     ...
     * rather than executing them immediately. All other {@code update} methods of the SqlUpdate
     * base class go through this method and will thus behave similarly.
     *
     * <p>You need to call {@code flush} to actually execute the batch. If the specified batch size
     * is reached, an implicit flush will happen; you still need to finally call {@code flush} to
     * flush all statements.
     *
     * @param params array of parameter objects
     * @return the number of rows affected by the update (always -1, meaning "not applicable", as
     *     the statement is not actually executed by this method)
     * @see #flush
     */
    @Override
    public int update(Object... params) throws DataAccessException {
        ...
        if (this.parameterQueue.size() == this.batchSize) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Triggering auto-flush because queue reached batch size of "
                                + this.batchSize);
            }
            flush();
        }
