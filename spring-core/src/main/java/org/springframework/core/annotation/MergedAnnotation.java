...
public interface MergedAnnotation<A extends Annotation> {
    ...
    /**
     * Create a new {@link MergedAnnotation} instance of the specified annotation type. The
     * resulting annotation will not have any attribute values but may still be used to query
     * default values.
     *
    ...
    /**
     * Create a new {@link MergedAnnotation} instance of the specified annotation type with
     * attribute values supplied by a map.
     *
     * @param annotationType the annotation type
     ...
     *
     * <p>A directly present annotation is one that the user has explicitly declared and not one
     * that is {@linkplain #isMetaPresent() meta-present} or {@link Inherited @Inherited}.
     *
     ...
     *
     * <p>A meta-present annotation is an annotation that the user hasn't explicitly declared, but
     ...
     *
     * <p>A directly declared annotation has a distance of {@code 0}, a meta-annotation has a
     * distance of {@code 1}, a meta-annotation on a meta-annotation has a distance of {@code 2},
     * etc. A {@linkplain #missing() missing} annotation will always return a distance of {@code
     * -1}.
     *
     ...
     *
     * <p>Can be used to reorder a stream of annotations, for example, to give a higher priority to
     * annotations declared on a superclass or interface. A {@linkplain #missing() missing}
     * annotation will always return an aggregate index of {@code -1}.
     *
     * @return the aggregate index (starting at {@code 0}) or {@code -1} if the annotation is
     *     missing
     */
     ...
     * not known.
     *
     * <p>If this merged annotation was created {@link MergedAnnotations#from(AnnotatedElement)
     * from} an {@link AnnotatedElement} then this source will be an element of the same type. If
     * the annotation was loaded without using reflection, the source can be of any type, but should
     * have a sensible {@code toString()}. Meta-annotations will always return the same source as
     * the {@link #getRoot() root}.
     *
     ...
     * #isMetaPresent() meta-present}.
     *
     * <p>The meta-source is the annotation that was meta-annotated with this annotation.
     *
     ...
     *
     * @param attributeName the attribute name
     * @param type the attribute type. Must be compatible with the underlying attribute type or
     *     {@code Object.class}.
     * @return an optional value or {@link Optional#empty()} if there is no matching attribute
     */
     ...
     *
     * <p>Methods from this view will return attribute values with only alias mirroring rules
     * applied. Aliases to {@link #getMetaSource() meta-source} attributes will not be applied.
     *
    ...
    /**
     * Create a type-safe synthesized version of this merged annotation that can be used directly in
     * code.
     *
     * <p>The result is synthesized using a JDK {@link Proxy} and as a result may incur a
     * computational cost when first invoked.
     *
     * <p>If this merged annotation was created {@linkplain #from(Annotation) from} an annotation
     * instance, that annotation will be returned unmodified if it is not <em>synthesizable</em>. An
     * annotation is considered synthesizable if one of the following is true.
     *
     * <ul>
     *   <li>The annotation declares attributes annotated with {@link AliasFor @AliasFor}.
     *   <li>The annotation is a composed annotation that relies on convention-based annotation
     *       attribute overrides in meta-annotations.
     *   <li>The annotation declares attributes that are annotations or arrays of annotations that
     *       are themselves synthesizable.
     * </ul>
     *
     * @return a synthesized version of the annotation or the original annotation unmodified
     ...
     * computational cost when first invoked.
     *
     * <p>Consult the documentation for {@link #synthesize()} for an explanation of what is
     * considered synthesizable.
     *
     * @param condition the test to determine if the annotation can be synthesized
     * @return an optional containing the synthesized version of the annotation or an empty optional
    ...
    /**
     * Adaptations that can be applied to attribute values when creating {@linkplain
