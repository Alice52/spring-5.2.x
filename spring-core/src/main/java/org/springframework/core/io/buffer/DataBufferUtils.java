...
public abstract class DataBufferUtils {
    ...
    public static Flux<DataBuffer> readByteChannel(
        ...
        return Flux.using(
                ...
                channel ->
                        Flux.generate(
                                new ReadableByteChannelGenerator(
                                        channel, bufferFactory, bufferSize)),
    ...
    /**
     * Obtain a {@code AsynchronousFileChannel} from the given supplier, and read it into a {@code
     * Flux} of {@code DataBuffer}s, starting at the given position. Closes the channel when the
     * Flux is terminated.
     *
     * @param channelSupplier the supplier for the channel to read from
    ...
    public static Flux<DataBuffer> readAsynchronousFileChannel(
        ...
        Flux<DataBuffer> flux =
                Flux.using(
                        ...
                        channel -> {
                            // Do not close channel from here, rather wait for the current read
                            // callback
     ...
     *
     * <p>If the resource is a file, it is read into an {@code AsynchronousFileChannel} and turned
     * to {@code Flux} via {@link #readAsynchronousFileChannel(Callable, DataBufferFactory, int)} or
     * else fall back to {@link #readByteChannel(Callable, DataBufferFactory, int)}. Closes the
     ...
     * <p>If the resource is a file, it is read into an {@code AsynchronousFileChannel} and turned
     * to {@code Flux} via {@link #readAsynchronousFileChannel(Callable, DataBufferFactory, int)} or
     * else fall back on {@link #readByteChannel(Callable, DataBufferFactory, int)}. Closes the
     * channel when the flux is terminated.
     *
     * @param resource the resource to read from
     * @param position the position to start reading from
    ...
    public static Flux<DataBuffer> read(
        ...
        Flux<DataBuffer> result =
                readByteChannel(resource::readableChannel, bufferFactory, bufferSize);
     ...
     *     process when subscribed to, and that publishes any writing errors and the completion
     *     signal
     */
     ...
     * WritableByteChannel}. Does <strong>not</strong> close the channel when the flux is
    ...
    public static Flux<DataBuffer> write(
            Publisher<DataBuffer> source, WritableByteChannel channel) {
        Assert.notNull(source, "'source' must not be null");
        ...
        return Flux.create(
                sink -> {
                    WriteCompletionHandler handler =
                            new WriteCompletionHandler(sink, channel, position);
     ...
     * optional {@code options} parameter specifies how the file is created or opened (defaults to
                ...
                () -> {
                    ...
                    return Flux.from(publisher)
                            .map(
                                    buffer -> {
                                        long remainder =
                                                countDown.addAndGet(-buffer.readableByteCount());
                                        if (remainder < 0) {
                                            int length =
                                                    buffer.readableByteCount() + (int) remainder;
     ...
     * elements. Depending on the {@link DataBuffer} type, the returned buffer may be a single
     * buffer containing all data of the provided buffers, or it may be a zero-copy, composite with
    ...
    /**
     * Variant of {@link #join(Publisher)} that behaves the same way up until the specified max
     * number of bytes to buffer. Once the limit is exceeded, {@link DataBufferLimitException} is
     * raised.
     *
     ...
     * @return a buffer with the aggregated content, possibly an empty Mono if the max number of
     *     bytes to buffer is exceeded.
     ...
     * delimiters in a stream of data buffers.
     *
    ...
    public static Matcher matcher(byte[]... delimiters) {
        ...
        return (delimiters.length == 1
                ? createMatcher(delimiters[0])
                : new CompositeMatcher(delimiters));
    }
    ...
    private static NestedMatcher createMatcher(byte[] delimiter) {
        ...
        switch (delimiter.length) {
            case 1:
                return (delimiter[0] == 10
                        ? SingleByteMatcher.NEWLINE_MATCHER
    ...
    /**
     * Contract to find delimiter(s) against one or more data buffers that can be passed one at a
     * time to the {@link #match(DataBuffer)} method.
     *
     * @since 5.2
    ...
    public interface Matcher {
        ...
        /**
         * Find the first matching delimiter and return the index of the last byte of the delimiter,
    ...
    /**
     * Matcher that can be nested within {@link CompositeMatcher} where multiple matchers advance
     * together using the same index, one byte at a time.
     */
    private interface NestedMatcher extends Matcher {

        /**
         * Perform a match against the next byte of the stream and return true if the delimiter is
         * fully matched.
         */
    ...
    /**
     * Matcher with a 2 byte delimiter that does not benefit from a Knuth-Morris-Pratt suffix-prefix
     * table.
     */
     ...
     *
     * @see <a
     *     href="https://www.nayuki.io/page/knuth-morris-pratt-string-matching">Knuth-Morris-Pratt
     *     string matching</a>
     */
    ...
    private static class ReadCompletionHandler implements CompletionHandler<Integer, DataBuffer> {
        ...
        private void tryRead() {
            if (this.sink.requestedFromDownstream() > 0
                    && this.state.compareAndSet(State.IDLE, State.READING)) {
                read();
            }
        }
    ...
    private static class WritableByteChannelSubscriber extends BaseSubscriber<DataBuffer> {
        ...
        public WritableByteChannelSubscriber(
                FluxSink<DataBuffer> sink, WritableByteChannel channel) {
            this.sink = sink;
            this.channel = channel;
        }
