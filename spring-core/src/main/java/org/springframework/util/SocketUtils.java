...
public class SocketUtils {
     ...
     * <h4>Rationale</h4>
     *
     * <p>Static methods from this class may be invoked from within XML configuration files using
     * the Spring Expression Language (SpEL) and the following syntax.
     *
     * <pre>
     ...
     * class name to SpEL's {@code T()} function for each usage. Thus, the fact that this
    ...
    /**
     * Find an available TCP port randomly selected from the range [{@value #PORT_RANGE_MIN},
    ...
    /**
     * Find an available TCP port randomly selected from the range [{@code minPort}, {@code
     * maxPort}].
     *
     * @param minPort the minimum port number
    ...
    /**
     * Find an available UDP port randomly selected from the range [{@code minPort}, {@code
     * maxPort}].
     *
     * @param minPort the minimum port number
     * @param maxPort the maximum port number
    ...
    private enum SocketType {
            ...
            protected boolean isPortAvailable(int port) {
                try {
                    DatagramSocket socket =
                            new DatagramSocket(port, InetAddress.getByName("localhost"));
                    socket.close();
                    return true;
                } catch (Exception ex) {
                    return false;
        ...
        /**
         * Determine if the specified port for this {@code SocketType} is currently available on
         * {@code localhost}.
         */
        ...
        /**
         * Find an available port for this {@code SocketType}, randomly selected from the range
         * [{@code minPort}, {@code maxPort}].
         *
         * @param minPort the minimum port number
         * @param maxPort the maximum port number
        ...
        int findAvailablePort(int minPort, int maxPort) {
            ...
            Assert.isTrue(
                    maxPort >= minPort, "'maxPort' must be greater than or equal to 'minPort'");
            Assert.isTrue(
                    maxPort <= PORT_RANGE_MAX,
         ...
         * @throws IllegalStateException if the requested number of available ports could not be
         *     found
         */
