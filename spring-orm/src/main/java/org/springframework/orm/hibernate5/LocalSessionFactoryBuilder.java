 ...
 *
 * <p>Compatible with Hibernate 5.0/5.1 as well as 5.2/5.3/5.4, as of Spring 5.2. Set up with
 * Hibernate 5.2+, this builder is also a convenient way to set up a JPA {@code
 * EntityManagerFactory} since the Hibernate {@code SessionFactory} natively exposes the JPA {@code
 * EntityManagerFactory} interface as well now.
 *
 * <p>This builder supports Hibernate 5.3/5.4 {@code BeanContainer} integration, {@link
 * MetadataSources} from custom {@link BootstrapServiceRegistryBuilder} setup, as well as other
 * advanced Hibernate configuration options beyond the standard JPA bootstrap contract.
 *
...
public class LocalSessionFactoryBuilder extends Configuration {
    ...
    public LocalSessionFactoryBuilder(
        ...
        try {
            ...
            getProperties()
                    .put("hibernate.connection.handling_mode", "DELAYED_ACQUISITION_AND_HOLD");
     ...
     * with Hibernate, if any. Allows for using a Spring-managed transaction manager for Hibernate
     * 5's session and cache synchronization, with the "hibernate.transaction.jta.platform"
        ...
        if (jtaTransactionManager instanceof JtaTransactionManager) {
            boolean webspherePresent =
                    ClassUtils.isPresent(
                            "com.ibm.wsspi.uow.UOWManager", getClass().getClassLoader());
            if (webspherePresent) {
                getProperties()
                        .put(
            ...
            getProperties()
                    .put(
                            AvailableSettings.JTA_PLATFORM,
                            new ConfigurableJtaPlatform(
                                    (TransactionManager) jtaTransactionManager, null, null));
        } else {
            throw new IllegalArgumentException(
                    "Unknown transaction manager type: "
                            + jtaTransactionManager.getClass().getName());
        }

        getProperties().put(AvailableSettings.TRANSACTION_COORDINATOR_STRATEGY, "jta");

        // Hibernate 5.1/5.2: manually enforce connection release mode AFTER_STATEMENT (the JTA
        // default)
        try {
            // Try Hibernate 5.2
            AvailableSettings.class.getField("CONNECTION_HANDLING");
    ...
    /**
     * Set a Hibernate {@link org.hibernate.resource.beans.container.spi.BeanContainer} for the
     * given Spring {@link ConfigurableListableBeanFactory}.
     *
     * <p>Note: Bean container integration requires Hibernate 5.3 or higher. It enables autowiring
     * of Hibernate attribute converters and entity listeners.
     *
     * @since 5.1
     * @see SpringBeanContainer
     * @see AvailableSettings#BEAN_CONTAINER
     */
    public LocalSessionFactoryBuilder setBeanContainer(
     ...
     *
     * <p>{@code SessionFactory} initialization will then switch into background bootstrap mode,
     * with a {@code SessionFactory} proxy immediately returned for injection purposes instead of
     * waiting for Hibernate's bootstrapping to complete. However, note that the first actual call
     * to a {@code SessionFactory} method will then block until Hibernate's bootstrapping completed,
     * if not ready by then. For maximum benefit, make sure to avoid early {@code SessionFactory}
     * calls in init methods of related beans, even for metadata introspection purposes.
     *
     * @since 4.3
    ...
    private class BootstrapSessionFactoryInvocationHandler implements InvocationHandler {
            ...
            this.sessionFactoryFuture =
                    bootstrapExecutor.submit(
                            (Callable<SessionFactory>)
                                    LocalSessionFactoryBuilder.this::buildSessionFactory);
        }
        ...
        private SessionFactory getSessionFactory() {
            try {
                ...
                throw new IllegalStateException(
                        "Failed to asynchronously initialize Hibernate SessionFactory: "
                                + ex.getMessage(),
                        cause);
            }
        }
