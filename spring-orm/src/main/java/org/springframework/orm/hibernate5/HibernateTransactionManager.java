...
public class HibernateTransactionManager extends AbstractPlatformTransactionManager
     ...
     * DataSource. Alternatively, consider switching {@link #setPrepareConnection
     * "prepareConnection"} to {@code false}.</b> In both cases, this transaction manager will not
     * eagerly acquire a JDBC Connection for each Hibernate Session anymore (as of Spring 5.1).
     *
     ...
     * per-transaction isolation levels anymore. It will not call {@code
     * Connection.setReadOnly(true)} for read-only transactions anymore either. If this flag is
     * turned off, no cleanup of a JDBC Connection is required after a transaction, since no
     ...
     * underlying JDBC Connection (if {@link #prepareConnection "prepareConnection"} is on) and
     * skips the disconnect-on-completion step.
     *
     ...
     * that is, whether to obtain the Session through Hibernate's {@link
     ...
     *
     * <p>Switch this flag to "true" in order to enforce use of a Hibernate-managed Session. Note
     * that this requires {@link SessionFactory#getCurrentSession()} to always return a proper
     * Session when called for a Spring-managed transaction; transaction begin will fail if the
     * {@code getCurrentSession()} call fails.
     *
     * <p>This mode will typically be used in combination with a custom Hibernate {@link
     * org.hibernate.context.spi.CurrentSessionContext} implementation that stores Sessions in a
     * place other than Spring's TransactionSynchronizationManager. It may also be used in
     * combination with Spring's Open-Session-in-View support (using Spring's default {@link
     * SpringSessionContext}), in which case it subtly differs from the Spring-managed Session mode:
     * The pre-bound Session will <i>not</i> receive a {@code clear()} call (on rollback) or a
     * {@code disconnect()} call (on transaction completion) in such a scenario; this is rather left
     * up to a custom CurrentSessionContext implementation (if desired).
     */
     ...
     *
     * <p>Requires the bean factory to be known, to be able to resolve the bean name to an
     * interceptor instance on session creation. Typically used for prototype interceptors, i.e. a
     * new interceptor instance per session.
     *
    ...
    protected Object doGetTransaction() {
        ...
        if (sessionHolder != null) {
            ...
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Found Hibernate-managed Session ["
                                    + session
    ...
    protected void doBegin(Object transaction, TransactionDefinition definition) {
        ...
        try {
            ...
            boolean holdabilityNeeded =
                    this.allowResultAccessAfterCompletion && !txObject.isNewSession();
            ...
            if (getDataSource() != null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Exposing Hibernate transaction as JDBC ["
