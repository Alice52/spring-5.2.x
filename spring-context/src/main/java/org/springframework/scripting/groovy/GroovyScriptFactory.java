...
public class GroovyScriptFactory implements ScriptFactory, BeanFactoryAware, BeanClassLoaderAware {
     ...
     * that can create a custom MetaClass to supply missing methods and otherwise change the
        ...
        if (classLoader instanceof GroovyClassLoader
                && (this.compilerConfiguration == null
                        || ((GroovyClassLoader) classLoader)
                                .hasCompatibleConfiguration(this.compilerConfiguration))) {
    ...
    /**
     * Groovy scripts determine their interfaces themselves, hence we don't need to explicitly
     * expose interfaces here.
     *
    ...
    public Object getScriptedObject(
            ScriptSource scriptSource, @Nullable Class<?>... actualInterfaces)
     ...
     * @return the result object (either an instance of the script class or the result of running
     *     the script instance)
    ...
    protected Object executeScript(ScriptSource scriptSource, Class<?> scriptClass)
        ...
        try {
            ...
            throw new ScriptCompilationException(
                    scriptSource,
                    "Unable to instantiate Groovy script class: " + scriptClass.getName(),
                    ex);
        } catch (IllegalAccessException ex) {
            throw new ScriptCompilationException(
                    scriptSource,
                    "Could not access Groovy script constructor: " + scriptClass.getName(),
                    ex);
