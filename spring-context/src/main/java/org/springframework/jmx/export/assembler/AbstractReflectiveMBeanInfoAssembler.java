...
public abstract class AbstractReflectiveMBeanInfoAssembler extends AbstractMBeanInfoAssembler {
     ...
     *
     * <p>Set this property to {@code true} for JMX implementations that require the "class" field
     * to be specified, for example WebLogic's. In that case, Spring will expose the target class
     * name there, in case of a plain bean instance or a CGLIB proxy. When encountering a JDK
     * dynamic proxy, the <b>first</b> interface implemented by the proxy will be specified.
     *
     * <p><b>WARNING:</b> Review your proxy definitions when exposing a JDK dynamic proxy through
     * JMX, in particular with this property turned to {@code true}: the specified interface list
     * should start with your management interface in this case, with all other interfaces
     * following. In general, consider exposing your target bean directly or a CGLIB proxy for it
     * instead.
     *
     ...
     * for parameter names of MBean operation methods).
     *
     ...
     * for inclusion, the appropriate metadata is assembled and passed to the subclass for
     * descriptor population.
     *
     * @param managedBean the bean instance (might be an AOP proxy)
     * @param beanKey the key associated with the MBean in the beans map of the {@code
     *     MBeanExporter}
    ...
    protected ModelMBeanAttributeInfo[] getAttributeInfo(Object managedBean, String beanKey)
        ...
        PropertyDescriptor[] props =
                BeanUtils.getPropertyDescriptors(getClassToExpose(managedBean));
    ...
    /**
     * Iterate through all methods on the MBean class and gives subclasses the chance to vote on
     * their inclusion. If a particular method corresponds to the accessor or mutator of an
     * attribute that is included in the management interface, then the corresponding operation is
     * exposed with the &quot;role&quot; descriptor field set to the appropriate value.
     *
     * @param managedBean the bean instance (might be an AOP proxy)
     * @param beanKey the key associated with the MBean in the beans map of the {@code
     *     MBeanExporter}
            ...
            if (pd != null
                    && ((method.equals(pd.getReadMethod()) && includeReadAttribute(method, beanKey))
                            || (method.equals(pd.getWriteMethod())
                                    && includeWriteAttribute(method, beanKey)))) {
     ...
     * @param name the logical name for the operation (method name or property name); not used by
     *     the default implementation but possibly by subclasses
     * @param beanKey the key associated with the MBean in the beans map of the {@code
     *     MBeanExporter}
     ...
     *
     * <p>The default implementation returns the first implemented interface for a JDK proxy, and
     * the target class else.
     *
     * @param managedBean the bean instance (might be an AOP proxy)
    ...
    protected String getAttributeDescription(
            PropertyDescriptor propertyDescriptor, String beanKey) {
    ...
    protected MBeanParameterInfo[] getOperationParameters(Method method, String beanKey) {
        ...
        String[] paramNames =
                (paramNameDiscoverer != null
                        ? paramNameDiscoverer.getParameterNames(method)
                        : null);
        if (paramNames == null) {
        ...
        for (int i = 0; i < info.length; i++) {
            info[i] =
                    new MBeanParameterInfo(
                            paramNames[i], typeParameters[i].getName(), paramNames[i]);
        }
    ...
    /**
     * Set the {@code currencyTimeLimit} field to the specified "defaultCurrencyTimeLimit", if any
     * (by default none).
     *
     ...
     * turns a value of {@code 0} into {@code Integer.MAX_VALUE} ("always cache") and sets the
