 ...
 * issued for any non-static {@code @Bean} methods having a return type assignable to {@code
...
public @interface Bean {
     ...
     *
     * <p>If left unspecified, the name of the bean is the name of the annotated method. If
     ...
     *
     * <p>Default is {@code true}; set this to {@code false} for internal delegates that are not
     * meant to get in the way of beans of the same type in other places.
     *
     * @since 5.1
     */
    ...
    /**
     * The optional name of a method to call on the bean instance during initialization. Not
     * commonly used, given that the method may be called programmatically directly within the body
     * of a Bean-annotated method.
     *
     ...
     * context, for example a {@code close()} method on a JDBC {@code DataSource} implementation, or
     * a Hibernate {@code SessionFactory} object. The method must have no arguments but may throw
     * any exception.
     *
     ...
     * returning an Apache Commons DBCP {@code BasicDataSource}, the container will notice the
     * {@code close()} method available on that object and automatically register it as the {@code
     * destroyMethod}. This 'destroy method inference' is currently limited to detecting only
     * public, no-arg methods named 'close' or 'shutdown'. The method may be declared at any level
     * of the inheritance hierarchy and will be detected regardless of the return type of the
     * {@code @Bean} method (i.e., detection occurs reflectively against the bean instance itself at
     * creation time).
     *
     * <p>To disable destroy method inference for a particular {@code @Bean}, specify an empty
     * string as the value, e.g. {@code @Bean(destroyMethod="")}. Note that the {@link
     * org.springframework.beans.factory.DisposableBean} callback interface will nevertheless get
     * detected and the corresponding destroy method invoked: In other words, {@code
     * destroyMethod=""} only affects custom close/shutdown methods and {@link
     * java.io.Closeable}/{@link java.lang.AutoCloseable} declared close methods.
     *
     * <p>Note: Only invoked on beans whose lifecycle is under the full control of the factory,
     * which is always the case for singletons but not guaranteed for any other scope.
     *
