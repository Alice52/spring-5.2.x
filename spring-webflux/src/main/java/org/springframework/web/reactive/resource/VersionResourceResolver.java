...
public class VersionResourceResolver extends AbstractResourceResolver {
    ...
    /**
     * Insert a content-based version in resource URLs that match the given path patterns. The
     * version is computed from the content of the file, e.g. {@code
     * "css/main-e36d2e05253c6c7085a91522ce43a0b4.css"}. This is a good default strategy to use
     * except when it cannot be, for example when using JavaScript module loaders, use {@link
     ...
     * example: <code>"{version}/js/main.js"</code>. This is useful (vs. content-based versions)
     * when using JavaScript module loaders.
     *
     ...
     * version} will be also configured. For example, adding a {@code "/js/**"} path pattern will
     * also cofigure automatically a {@code "/v1.0.0/js/**"} with {@code "v1.0.0"} the {@code
    ...
    public VersionResourceResolver addFixedVersionStrategy(String version, String... pathPatterns) {
        ...
        for (String pattern : patternsList) {
            ...
            if (!pattern.startsWith(versionPrefix)
                    && !patternsList.contains(versionPrefix + pattern)) {
    ...
    /**
     * Register a custom VersionStrategy to apply to resource URLs that match the given path
     * patterns.
     *
    ...
    protected Mono<Resource> resolveResourceInternal(
        ...
        return chain.resolveResource(exchange, requestPath, locations)
                .switchIfEmpty(
                        Mono.defer(
                                () ->
                                        resolveVersionedResource(
                                                exchange, requestPath, locations, chain)));
    }
                        ...
                        baseUrl -> {
                            if (StringUtils.hasText(baseUrl)) {
                                ...
                                return chain.resolveResource(null, baseUrl, locations)
                                        .flatMap(
                                                resource ->
                                                        strategy.getResourceVersion(resource)
                                                                .map(
                                                                        version ->
                                                                                strategy.addVersion(
                                                                                        baseUrl,
