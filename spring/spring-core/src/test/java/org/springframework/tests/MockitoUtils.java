...
public abstract class MockitoUtils {
    ...
    /**
     * Verify the same invocations have been applied to two mocks. This is generally not the
     * preferred way test with mockito and should be avoided if possible.
     *
    ...
    public static <T> void verifySameInvocations(
        ...
        List<Invocation> expectedInvocations =
                ((InvocationContainerImpl)
                                MockUtil.getMockHandler(expected).getInvocationContainer())
                        .getInvocations();
    ...
    private static void verifySameInvocations(
        ...
        for (int i = 0; i < expectedInvocations.size(); i++) {
            verifySameInvocation(
                    expectedInvocations.get(i), actualInvocations.get(i), argumentAdapters);
        }
    }
