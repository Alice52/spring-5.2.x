 ...
 *
 * <p>Simple utility class for working with the reflection API and handling reflection exceptions.
 *
...
public abstract class ReflectionUtils {
     ...
     *
     * <p>Pre-built MethodFilter that matches all non-bridge non-synthetic methods which are not
     ...
     *
     * <p>Pre-built FieldFilter that matches all non-static, non-final fields.
     */
    ...
    /**
     * CGLIB重命名方法的命名前缀 Naming prefix for CGLIB-renamed methods.
     *
     ...
     *
     * <p>Cache for {@link Class#getDeclaredMethods()} plus equivalent default methods from Java 8
     * based interfaces, allowing for fast iteration.
     */
     ...
     *
     * <p>Cache for {@link Class#getDeclaredFields()}, allowing for fast iteration.
     */
     ...
     * 处理给定的反射异常
     *
     * <p>Handle the given reflection exception.
     *
     ...
     *
     * <p>Handle the given invocation target exception. Should only be called if no checked
    ...
    /**
     * 重新抛出给定的{@link Throwable exception},这大概是{@link
     * InvocationTargetException}的目标异常。仅当目标方法预期不会抛出任何被检查异常时,才应该调用此方法。
     *
     * <p>Rethrow the given {@link Throwable exception}, which is presumably the <em>target
     ...
     * 获取给定的类和参数的一个可访问构造函数
     *
     * <p>Obtain an accessible constructor for the given class and parameters.
     *
    ...
    /**
     * 使给定的构造函数可访问，并在需要时显示设置它的可访问性。{@code setAccessible(true)}方法仅在实际需要时才会被调用， 以避免与JVM
     * SecurityManager发生不必要的冲突（如果处于活动状态)
     *
     * <p>Make the given constructor accessible, explicitly setting it accessible if necessary. The
     ...
     *
     * <p>Attempt to find a {@link Method} on the supplied class with the supplied name and no
    ...
    /**
     * 尝试去找到一个在提供的类和提供的方法名和给定的参数类型数组的{@link Method}。 搜寻所有的父类直到{@code Object}
     *
     * <p>Attempt to find a {@link Method} on the supplied class with the supplied name and
    ...
    /**
     * 针对提供的目标对象执行指定的没有参数的{@link Method}。 当执行一个静态的{@link Method}时，目标对象可以为null
     *
     * <p>Invoke the specified {@link Method} against the supplied target object with no arguments.
    ...
    /**
     * 针对提供的目标对象执行指定的参数数组的{@link Method}。 当执行一个静态的{@link Method}时，目标对象可以为null
     *
     * <p>Invoke the specified {@link Method} against the supplied target object with the supplied
     ...
     *
     * <p>Determine whether the given method explicitly declares the given exception or one of its
     ...
     *
     * <p>Perform the given callback operation on all matching methods of the given class, as
     * locally declared or equivalent thereof (such as default methods on Java 8 based interfaces
     ...
     * 执行给定回调操作在给定类的所有匹配方法
     *
     * <p>Perform the given callback operation on all matching methods of the given class and
     * superclasses.
     *
    ...
    public static void doWithMethods(Class<?> clazz, MethodCallback mc, @Nullable MethodFilter mf) {
        ...
        // 如果clazz的父类不为nulll且(mf不是与未在{@code
        // java.lang.Object}上声明的所有非桥接非合成方法匹配的预购建方法过滤器或者clazz的父类不为Object
     ...
     *
     * <p>Get all declared methods on the leaf class and all superclasses. Leaf class methods are
     * included first.
     *
    ...
    /**
     * 在子类和所有超类上获取一组唯一的已声明方法。 首先包含子类方法和然后遍历父类层次结构任何方法，找到和签名匹配的已经包括被过滤出来的一个方法。
     *
     * <p>Get the unique set of declared methods on the leaf class and all superclasses. Leaf class
     * methods are included first and while traversing the superclass hierarchy any methods found
    ...
    /**
     * 在子类和所有超类上获取一组唯一的已声明方法，即被重写非协变返回类型的方法 首先包含子类方法和然后遍历父类层次结构任何方法，将过滤出所有与已包含的方法匹配的签名方法。
     *
     * <p>Get the unique set of declared methods on the leaf class and all superclasses. Leaf class
     * methods are included first and while traversing the superclass hierarchy any methods found
     * with signatures matching a method already included are filtered out.
                ...
                method -> {
                    ...
                    for (Method existingMethod : methods) {
                        ...
                        if (method.getName().equals(existingMethod.getName())
                            ...
                            // 如果existingMethod的返回类型不等于method的返回类型 且
                            // existingMethod的返回类型是method返回类型的子类或本身
     ...
     *
     * <p>Variant of {@link Class#getDeclaredMethods()} that uses a local cache in order to avoid
     * the JVM's SecurityManager check and new Method instances. In addition, it also includes Java
     ...
     * 判断给定方法是否是'equals'方法
     *
     * <p>Determine whether the given method is an "equals" method.
     *
     ...
     *
     * <p>Determine whether the given method is a "hashCode" method.
     *
     ...
     *
     * <p>Determine whether the given method is a "toString" method.
     *
     ...
     *
     * <p>Determine whether the given method is originally declared by {@link java.lang.Object}.
     */
     ...
     *
     * <p>Determine whether the given method is a CGLIB 'renamed' method, following the pattern
     * "CGLIB$methodName$0".
     *
    ...
    /**
     * 使给定的方法可访问，在需要时显式设置它的可访问性。{@code setAccessible(true)}方法仅在实际需要时才会被调用， 以避免与JVM
     * SecurityManager发生不必要的冲突（如果处于活动状态)
     *
     * <p>Make the given method accessible, explicitly setting it accessible if necessary. The
     * {@code setAccessible(true)} method is only called when actually necessary, to avoid
     * unnecessary conflicts with a JVM SecurityManager (if active).
     *
     ...
     *
     * <p>Attempt to find a {@link Field field} on the supplied {@link Class} with the supplied
    ...
    /**
     * 将在指定的{@linkplain Object target object}上提供的{@linkplain Field file object}表示的字段设置为指定的{@code
     * value}
     *
     * <p>Set the field represented by the supplied {@linkplain Field field object} on the specified
    ...
    /**
     * 获取在指定的{@linkplain Object target object}上提供的{@linkplain Field file object}的属性值。 根据{@link
     * Field#get(Object)}语义，如果基础字段具有原始类型，则返回值会自动包装。
     *
     * <p>Get the field represented by the supplied {@link Field field object} on the specified
     ...
     *
     * <p>Invoke the given callback on all locally declared fields in the given class.
     *
     ...
     *
     * <p>Invoke the given callback on all fields in the target class, going up the class hierarchy
     ...
     * 使用一个本地缓存为了避免JVM的安全管理检查和新建Method实例。
     *
     * <p>This variant retrieves {@link Class#getDeclaredFields()} from a local cache in order to
     * avoid the JVM's SecurityManager check and defensive array copying.
     *
     * @param clazz the class to introspect
     ...
     *
     * <p>Given the source object and the destination, which must be the same class or a subclass,
     * copy all fields, including inherited fields. Designed to work on objects with public no-arg
     * constructors.
     *
     * @throws IllegalStateException if introspection fails
     */
     ...
     *
     * <p>Determine whether the given field is a "public static final" constant.
     *
    ...
    /**
     * 使给定的属性可访问，并在需要时显示设置它的可访问性。{@code setAccessible(true)}方法仅在实际需要时才会被调用， 以避免与JVM
     * SecurityManager发生不必要的冲突（如果处于活动状态)
     *
     * <p>Make the given field accessible, explicitly setting it accessible if necessary. The {@code
     ...
     * 清空内部方法/属性缓存
     *
     * <p>Clear the internal method/field cache.
     *
     * @since 4.2.4
     */
     ...
     * 对每个方法采取的行为
     *
     * <p>Action to take on each method.
     */
    @FunctionalInterface
    public interface MethodCallback {
         ...
         * 使用给定的方法执行操作
         *
         * <p>Perform an operation using the given method.
         *
     ...
     * 回调可选，用于过滤由方法回调操作的方法
     *
     * <p>Callback optionally used to filter methods to be operated on by a method callback.
     */
    @FunctionalInterface
    public interface MethodFilter {
         ...
         * 确定给定的方法是否匹配
         *
         * <p>Determine whether the given method matches.
         *
     ...
     * 在层次结构中每个属性上调用的回调接口
     *
     * <p>Callback interface invoked on each field in the hierarchy.
     */
    @FunctionalInterface
    public interface FieldCallback {
         ...
         * 使用给定的属性执行操作
         *
         * <p>Perform an operation using the given field.
         *
     ...
     * 调用可选 用于过滤由属性回调操作的属性
     *
     * <p>Callback optionally used to filter fields to be operated on by a field callback.
     */
    @FunctionalInterface
    public interface FieldFilter {
         ...
         * 确定给定的属性是否匹配
         *
         * <p>Determine whether the given field matches.
         *
         * @param field the field to check
         */
