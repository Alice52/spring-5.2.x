...
final class TypeMappedAnnotations implements MergedAnnotations {
                    ...
                    if (type != null && !this.annotationFilter.matches(type)) {
                        ...
                        if (!this.directOnly) {
                            AnnotationTypeMappings mappings =
                                    AnnotationTypeMappings.forAnnotationType(type);
                            for (int i = 0; i < mappings.size(); i++) {
                                AnnotationTypeMapping mapping = mappings.get(i);
                                if (isMappingForType(
                                        mapping, this.annotationFilter, requiredType)) {
                                    return Boolean.TRUE;
                                }
                            }
                        }
    ...
    private class MergedAnnotationFinder<A extends Annotation>
        ...
        private MergedAnnotation<A> process(
            ...
            for (int i = 0; i < mappings.size(); i++) {
                ...
                if (isMappingForType(mapping, annotationFilter, this.requiredType)) {
                    ...
                    if (candidate != null
                            && (this.predicate == null || this.predicate.test(candidate))) {
        ...
        private void updateLastResult(MergedAnnotation<A> candidate) {
            ...
            this.result =
                    (lastResult != null ? this.selector.select(lastResult, candidate) : candidate);
        }
    ...
    private class AggregatesSpliterator<A extends Annotation>
        ...
        private boolean tryAdvance(
                Aggregate aggregate, Consumer<? super MergedAnnotation<A>> action) {
        ...
        @Nullable
        private AnnotationTypeMapping getNextSuitableMapping(
                Aggregate aggregate, int annotationIndex) {
        ...
        public long estimateSize() {
            ...
            for (int aggregateIndex = this.aggregateCursor;
                ...
                for (int annotationIndex = 0;
                        annotationIndex < aggregate.size();
                        annotationIndex++) {
                    ...
                    if (aggregateIndex == this.aggregateCursor && this.mappingCursors != null) {
                        numberOfMappings -=
                                Math.min(this.mappingCursors[annotationIndex], mappings.size());
                    }
