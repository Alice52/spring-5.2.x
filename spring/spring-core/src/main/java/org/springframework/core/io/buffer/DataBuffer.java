...
public interface DataBuffer {
     ...
     * @return the index of the first byte that matches {@code predicate}; or {@code -1} if none
     *     match
     */
     ...
     * @param predicate the predicate to match
     * @param fromIndex the index to start the search from
     * @return the index of the last byte that matches {@code predicate}; or {@code -1} if none
     *     match
     */
     ...
     *
     * <p>If the new capacity is lower than the current capacity, the contents of this buffer will
     * be truncated. If the new capacity is higher than the current capacity, it will be expanded.
     *
    ...
    /**
     * Write one or more {@code DataBuffer}s to this buffer, starting at the current writing
     * position. It is the responsibility of the caller to {@linkplain
     * DataBufferUtils#release(DataBuffer) release} the given data buffers.
     *
    ...
    /**
     * Write one or more {@link ByteBuffer} to this buffer, starting at the current writing
     * position.
     *
     * @param buffers the byte buffers to write into this buffer
     * @return this buffer
        ...
        if (charSequence.length() != 0) {
            ...
            while (true) {
                ...
                if (cr.isOverflow()) {
                    ...
                    int maximumSize =
                            (int) (inBuffer.remaining() * charsetEncoder.maxBytesPerChar());
     ...
     * content. Data between this data buffer and the returned buffer is shared; though changes in
     * the returned buffer's position will not be reflected in the reading nor writing position of
     * this data buffer.
     *
     ...
     * the returned {@code ByteBuffer} is shared; though changes in the returned buffer's
     * {@linkplain ByteBuffer#position() position} will not be reflected in the reading nor writing
     * position of this data buffer.
     *
