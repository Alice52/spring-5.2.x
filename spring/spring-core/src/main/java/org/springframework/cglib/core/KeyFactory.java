 ...
 *
 * <p>Generates classes to handle multi-valued keys, for use in things such as Maps and Sets. Code
 * for <code>equals</code> and <code>hashCode</code> methods follow the the rules laid out in
...
public abstract class KeyFactory {
    ...
    /**
     * @deprecated this customizer might result in unexpected class leak since key object still
     *     holds a strong reference to the Object and class. It is recommended to have
    ...
    public static KeyFactory create(ClassLoader loader, Class keyInterface, Customizer customizer) {
        return create(
                loader, keyInterface, customizer, Collections.<KeyFactoryCustomizer>emptyList());
    }
    ...
    public static class Generator extends AbstractClassGenerator {
        ...
        public void generateClass(ClassVisitor v) {
            ...
            CodeEmitter e =
                    ce.begin_method(
                            Constants.ACC_PUBLIC, TypeUtils.parseConstructor(parameterTypes), null);
            e.load_this();
            // 4.1有参构造中调用父类构造方法,即super.构造方法()
            e.super_invoke_constructor();
            e.load_this();
            // 4.2找到传入的定制器 例如一开始传入的hashCode方法定制器
            List<FieldTypeCustomizer> fieldTypeCustomizers =
                    getCustomizers(FieldTypeCustomizer.class);
            ...
            for (int i = 0; i < parameterTypes.length; i++) {
                ...
                EmitUtils.append_string(
                        e, parameterTypes[i], EmitUtils.DEFAULT_DELIMITERS, customizers);
            }
