...
public class ReflectUtils {
    ...
    public static Method findMethod(String desc, ClassLoader loader) {
        try {
            ...
            return getClass(className, loader)
                    .getDeclaredMethod(methodName, parseTypes(desc, loader));
        } catch (ClassNotFoundException | NoSuchMethodException ex) {
            throw new CodeGenerationException(ex);
        }
    }
        ...
        for (int i = 0; i < packages.length; i++) {
            try {
                return Class.forName(
                        prefix + packages[i] + '.' + className + suffix, false, loader);
            } catch (ClassNotFoundException ignore) {
            }
        }
        if (dimensions == 0) {
    ...
    private static PropertyDescriptor[] getPropertiesHelper(
        ...
        try {
            ...
            return (PropertyDescriptor[])
                    properties.toArray(new PropertyDescriptor[properties.size()]);
                ...
                MethodHandles.Lookup lookup =
                        (MethodHandles.Lookup)
                                privateLookupInMethod.invoke(
                                        null, contextClass, MethodHandles.lookup());
    ...
    public static ClassInfo getClassInfo(final Class clazz) {
        ...
        final Type sc =
                (clazz.getSuperclass() == null) ? null : Type.getType(clazz.getSuperclass());
    ...
    public static Method[] findMethods(String[] namesAndDescriptors, Method[] methods) {
        ...
        for (int i = 0; i < result.length; i++) {
            result[i] =
                    (Method) map.get(namesAndDescriptors[i * 2] + namesAndDescriptors[i * 2 + 1]);
            if (result[i] == null) {
                // TODO: error?
            }
        }
                    ...
                    (Method)
                            AccessController.doPrivileged(
                                    new PrivilegedExceptionAction() {
                                        public Object run() throws Exception {
                                            try {
                                                return MethodHandles.Lookup.class.getMethod(
