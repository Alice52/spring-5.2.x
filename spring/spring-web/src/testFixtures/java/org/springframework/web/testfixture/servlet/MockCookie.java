...
public class MockCookie extends Cookie {
    ...
    @Nullable private ZonedDateTime expires;
    ...
    public static MockCookie parse(String setCookieHeader) {
        ...
        String[] attributes =
                (valueAndAttributes.length > 1
                        ? valueAndAttributes[1].split("\\s*;\\s*")
                        : new String[0]);
        ...
        for (String attribute : attributes) {
            if (StringUtils.startsWithIgnoreCase(attribute, "Domain")) {
                ...
                cookie.setMaxAge(
                        Integer.parseInt(extractAttributeValue(attribute, setCookieHeader)));
            } else if (StringUtils.startsWithIgnoreCase(attribute, "Expires")) {
                try {
                    cookie.setExpires(
                            ZonedDateTime.parse(
                                    extractAttributeValue(attribute, setCookieHeader),
     ...
     * requests if the supplied value is {@code "Strict"} or cross-site requests if the supplied
     * value is {@code "Lax"}.
     *
     * @see <a
