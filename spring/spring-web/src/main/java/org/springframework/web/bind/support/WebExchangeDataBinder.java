 ...
 * query parameters or form data in the request data to Java objects.
 *
...
public class WebExchangeDataBinder extends WebDataBinder {
    ...
    /**
     * Combine query params and form data for multipart form data from the body of the request into
     * a {@code Map<String, Object>} of values to use for data binding purposes.
     *
        ...
        return Mono.zip(Mono.just(queryParams), formData, multipartData)
                .map(
                        tuple -> {
                            ...
                            tuple.getT1()
                                    .forEach((key, values) -> addBindValue(result, key, values));
                            tuple.getT2()
                                    .forEach((key, values) -> addBindValue(result, key, values));
                            tuple.getT3()
                                    .forEach((key, values) -> addBindValue(result, key, values));
        ...
        if (!CollectionUtils.isEmpty(values)) {
            values =
                    values.stream()
                            .map(
                                    value ->
                                            value instanceof FormFieldPart
                                                    ? ((FormFieldPart) value).value()
                                                    : value)
