 ...
 *
 * <p><strong>NOTE:</strong> As of 5.0 this class is in maintenance mode, with only minor requests
 * for changes and bugs to be accepted going forward. Please, consider using the {@code
 * org.springframework.web.reactive.client.WebClient} which has a more modern API and supports sync,
 * async, and streaming scenarios.
 *
...
public class RestTemplate extends InterceptingHttpAccessor implements RestOperations {
    ...
    public Set<HttpMethod> optionsForAllow(String url, Object... uriVariables)
        ...
        HttpHeaders headers =
                execute(url, HttpMethod.OPTIONS, null, headersExtractor, uriVariables);
    ...
    /**
     * Return a {@code RequestCallback} that sets the request {@code Accept} header based on the
     * given response type, cross-checked against the configured message converters.
     */
    ...
    private class AcceptHeaderRequestCallback implements RequestCallback {
        ...
        private boolean canReadResponse(Type responseType, HttpMessageConverter<?> converter) {
            Class<?> responseClass =
                    (responseType instanceof Class ? (Class<?>) responseType : null);
    ...
    private class HttpEntityRequestCallback extends AcceptHeaderRequestCallback {
        ...
        public HttpEntityRequestCallback(
                @Nullable Object requestBody, @Nullable Type responseType) {
            super(responseType);
                ...
                for (HttpMessageConverter<?> messageConverter : getMessageConverters()) {
                    if (messageConverter instanceof GenericHttpMessageConverter) {
                        ...
                        if (genericConverter.canWrite(
                                requestBodyType, requestBodyClass, requestContentType)) {
                            if (!requestHeaders.isEmpty()) {
                                requestHeaders.forEach(
                                        (key, values) ->
