...
final class HierarchicalUriComponents extends UriComponents {
    ...
    HierarchicalUriComponents(
        ...
        this.queryParams =
                query != null
                        ? CollectionUtils.unmodifiableMultiValueMap(query)
                        : EMPTY_QUERY_PARAMS;
    ...
    /**
     * Encode the given source into an encoded String using the rules specified by the given
     * component and with the given options.
     *
        ...
        this.queryParams.forEach(
                (key, values) -> {
                    ...
                    for (String value : values) {
                        encodedValues.add(
                                value != null ? encoder.apply(value, Type.QUERY_PARAM) : null);
                    }
    ...
    private MultiValueMap<String, String> expandQueryParams(UriTemplateVariables variables) {
        ...
        this.queryParams.forEach(
                (key, values) -> {
                    ...
                    for (String value : values) {
                        expandedValues.add(
                                expandUriComponent(value, queryVariables, this.variableEncoder));
                    }
     ...
     *
     * <p>Contains methods to indicate whether a given character is valid in a specific URI
     * component.
     *
    ...
    private enum EncodeState {
        ...
        /**
         * URI template encoded first by quoting illegal characters only, and then URI vars encoded
         * more strictly when expanded, by quoting both illegal chars and chars with reserved
         * meaning.
         */
        TEMPLATE_ENCODED;
    ...
    private static class UriTemplateEncoder implements BiFunction<String, Type, String> {
        ...
        private void encodeAndAppendCurrentLiteral(Type type) {
            this.output.append(
                    encodeUriComponent(this.currentLiteral.toString(), this.charset, type));
            clear(this.currentLiteral);
        }
        ...
        public boolean equals(@Nullable Object other) {
            return (this == other
                    || (other instanceof PathSegmentComponent
                            && getPathSegments()
                                    .equals(((PathSegmentComponent) other).getPathSegments())));
        }
