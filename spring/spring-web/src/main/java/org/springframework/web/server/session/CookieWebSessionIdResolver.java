...
public class CookieWebSessionIdResolver implements WebSessionIdResolver {
    ...
    /**
     * Add a {@link Consumer} for a {@code ResponseCookieBuilder} that will be invoked for each
     * cookie being built, just before the call to {@code build()}.
     *
    ...
    public void addCookieInitializer(Consumer<ResponseCookie.ResponseCookieBuilder> initializer) {
        this.cookieInitializer =
                this.cookieInitializer != null
                        ? this.cookieInitializer.andThen(initializer)
                        : initializer;
    }
