 ...
 * {@code **} and its capturing variant <code>{*spring}</code> cannot be used in the middle of a
 * pattern string, only at the end: {@code /pages/{**}} is valid, but {@code /pages/{**}/details} is
 * not.
 *
 * <h3>Examples</h3>
 *
 ...
 *       /resources/image.png} will match with "path" &rarr; "/image.png", and {@code
 *       /resources/css/spring.css} will match with "path" &rarr; "/css/spring.css"
...
public class PathPattern implements Comparable<PathPattern> {
     ...
     * by assuming all captured variables have a normalized length of 1. Effectively this means
     * changing your variable name lengths isn't going to change the length of the active part of
     * the pattern. Useful when comparing two patterns.
     */
    ...
    public boolean matches(PathContainer pathContainer) {
        if (this.head == null) {
            ...
            if (this.head instanceof WildcardTheRestPathElement
                ...
                pathContainer =
                        EMPTY_PATH; // Will allow CaptureTheRest to bind the variable to empty
            } else {
                return false;
            }
        }
    ...
    /**
     * Match this pattern to the given URI path and return extracted URI template variables as well
     * as path parameters (matrix variables).
     *
     * @param pathContainer the candidate path to attempt to match against
    ...
    public PathContainer extractPathWithinPattern(PathContainer path) {
        ...
        int startIndex = 0;
        // Find first path element that is not a separator or a literal (i.e. the first pattern
        // based element)
        ...
        while (startIndex < pathElementsCount
                && (pathElements.get(startIndex) instanceof Separator)) {
            startIndex++;
        }
    ...
    /**
     * Compare this pattern with a supplied pattern: return -1,0,+1 if this pattern is more
     * specific, the same or less specific than the supplied pattern. The aim is to sort more
    ...
    /**
     * The normalized length is trying to measure the 'active' part of the pattern. It is computed
     * by assuming all capture variables have a normalized length of 1. Effectively this means
     * changing your variable name lengths isn't going to change the length of the active part of
     * the pattern. Useful when comparing two patterns.
     */
    ...
    /**
     * Holder for URI variables and path parameters (matrix variables) extracted based on the
     * pattern for a given matched path.
     */
    public static class PathMatchInfo {
        ...
        PathMatchInfo(
            ...
            this.matrixVariables =
                    (matrixVars != null
                            ? Collections.unmodifiableMap(matrixVars)
    ...
    public static class PathRemainingMatchInfo {
        ...
        /**
         * Return variables that were bound in the part of the path that was successfully matched or
         * an empty map.
         */
        public Map<String, String> getUriVariables() {
    ...
    class MatchingContext {
        ...
        String pathElementValue(int pathIndex) {
            Element element =
                    (pathIndex < this.pathLength) ? this.pathElements.get(pathIndex) : null;
