...
public class JaxWsPortClientInterceptor extends LocalJaxWsServiceFactory
     ...
     * org.springframework.jndi.JndiObjectFactoryBean}. If not set, {@link
     * LocalJaxWsServiceFactory}'s properties have to be specified.
     *
    ...
    /**
     * Specify WebServiceFeature objects (e.g. as inner bean definitions) to apply to JAX-WS port
     * stub creation.
     *
     * @since 4.0
    ...
    public void setServiceInterface(@Nullable Class<?> serviceInterface) {
        if (serviceInterface != null) {
            Assert.isTrue(
                    serviceInterface.isInterface(), "'serviceInterface' must be an interface");
        }
     ...
     *
     * <p>Default is "true". Turn this flag off to allow for late start of the target server. In
     * this case, the JAX-WS service will be lazily fetched on first access.
     */
    ...
    /**
     * Set the bean ClassLoader to use for this interceptor: primarily for building a client proxy
     * in the {@link JaxWsPortProxyFactoryBean} subclass.
     */
    @Override
     ...
     * necessary and possible (i.e. if "wsdlDocumentUrl", "namespaceUri", "serviceName" and
     * "portName" haven't been set but corresponding values are declared at the annotation level of
    ...
    protected Object doInvoke(MethodInvocation invocation, @Nullable Object portStub)
        ...
        try {
            ...
            throw new RemoteProxyFailureException(
                    "Invocation of stub method failed: " + method, ex);
        }
    }
}
