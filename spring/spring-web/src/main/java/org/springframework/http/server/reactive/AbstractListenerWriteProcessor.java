...
public abstract class AbstractListenerWriteProcessor<T> implements Processor<T, Void> {
    ...
    /**
     * Indicates we're waiting for one last isReady-onWritePossible cycle after "onComplete" because
     * some Servlet containers expect this to take place prior to calling AsyncContext.complete().
        ...
        if (prev != null) {
            ...
            onError(
                    new IllegalStateException(
                            "Received new data while current not processed yet."));
        }
     ...
     *
     * <p><strong>Note:</strong> Sub-classes are responsible for releasing any data buffer
     * associated with the item, once fully written, if pooled buffers apply to the underlying
     * container.
     *
    ...
    /**
     * Invoked when an I/O error occurs during a write. Sub-classes may choose to ignore this if
     * they know the underlying API will provide an error notification in a container thread.
     *
     * <p>Defaults to no-op.
     */
