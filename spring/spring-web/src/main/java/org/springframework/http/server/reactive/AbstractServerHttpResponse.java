...
public abstract class AbstractServerHttpResponse implements ServerHttpResponse {
    ...
    public final Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
        ...
        if (body instanceof Mono) {
            ...
            return new ChannelSendOperator<>(
                            body, inner -> doCommit(() -> writeWithInternal(inner)))
                    .doOnError(t -> getHeaders().clearContentHeaders());
        }
    }
    ...
    protected Mono<Void> doCommit(@Nullable Supplier<? extends Mono<Void>> writeAction) {
        ...
        if (!this.commitActions.isEmpty()) {
            allActions =
                    Flux.concat(Flux.fromIterable(this.commitActions).map(Supplier::get))
                            .doOnError(
                                    ex -> {
    ...
    /**
     * Invoked when the response is getting committed allowing sub-classes to make apply header
     * values to the underlying response.
     *
     * <p>Note that most sub-classes use an {@link HttpHeaders} instance that wraps an adapter to
     * the native response headers such that changes are propagated to the underlying response on
     * the go. That means this callback is typically not used other than for specialized updates
     * such as setting the contentType or characterEncoding fields in a Servlet response.
     */
