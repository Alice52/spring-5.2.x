...
public class OkHttp3ClientHttpRequestFactory
    ...
    static Request buildRequest(HttpHeaders headers, byte[] content, URI uri, HttpMethod method)
        ...
        Request.Builder builder =
                new Request.Builder().url(uri.toURL()).method(method.name(), body);
        headers.forEach(
    ...
    private static okhttp3.MediaType getContentType(HttpHeaders headers) {
        ...
        return (StringUtils.hasText(rawContentType)
                ? okhttp3.MediaType.parse(rawContentType)
                : null);
    }
    ...
    /**
     * Set the underlying connect timeout in milliseconds. A value of 0 specifies an infinite
     * timeout.
     */
