...
public class ServerSentEventHttpMessageReader implements HttpMessageReader<Object> {
    ...
    /**
     * Configure a limit on the maximum number of bytes per SSE event which are buffered before the
     * event is parsed.
     *
     * <p>Note that the {@link #getDecoder() data decoder}, if provided, must also be customized
     * accordingly to raise the limit if necessary in order to be able to parse the data portion of
     * the event.
     *
    ...
    private Object decodeData(
            StringBuilder data, ResolvableType dataType, Map<String, Object> hints) {
