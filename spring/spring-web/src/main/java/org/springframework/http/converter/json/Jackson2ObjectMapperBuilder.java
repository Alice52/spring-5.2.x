...
public class Jackson2ObjectMapperBuilder {
    ...
    /**
     * Alternative to {@link #annotationIntrospector(AnnotationIntrospector)} that allows combining
     * with rather than replacing the currently set introspector, e.g. via {@link
     * AnnotationIntrospectorPair#pair(AnnotationIntrospector, AnnotationIntrospector)}.
     *
     * @param pairingFunction a function to apply to the currently set introspector (possibly {@code
     *     null}); the result of the function becomes the new introspector.
     * @since 5.2.4
     */
    public Jackson2ObjectMapperBuilder annotationIntrospector(
    ...
    /**
     * Specify a {@link com.fasterxml.jackson.databind.PropertyNamingStrategy} to configure the
     * {@link ObjectMapper} with.
     */
     ...
     */
    public Jackson2ObjectMapperBuilder serializerByType(
            Class<?> type, JsonSerializer<?> serializer) {
    ...
    public Jackson2ObjectMapperBuilder failOnUnknownProperties(boolean failOnUnknownProperties) {
        this.features.put(
                DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, failOnUnknownProperties);
        return this;
    }
     ...
     *
     * <p>Note: If this is set, no finding of modules is going to happen - not by Jackson, and not
     * by Spring either (see {@link #findModulesViaServiceLoader}). As a consequence, specifying an
     ...
     * Joda-Time, or Jackson's finding of modules (see {@link #findModulesViaServiceLoader}),
    ...
    /**
     * Set whether to let Jackson find available modules via the JDK ServiceLoader, based on
     * META-INF metadata in the classpath.
     *
     ...
     */
    public Jackson2ObjectMapperBuilder handlerInstantiator(
     ...
     * JsonSerializer}, {@link JsonDeserializer}, {@link KeyDeserializer}, {@code
     * TypeResolverBuilder} and {@code TypeIdResolver}).
     *
     * @since 4.1.3
        ...
        if (this.createXmlMapper) {
            mapper =
                    (this.defaultUseWrapper != null
                            ? new XmlObjectMapperInitializer()
                                    .create(this.defaultUseWrapper, this.factory)
    ...
    public void configure(ObjectMapper objectMapper) {
        ...
        this.features.forEach(
                (feature, enabled) -> configureFeature(objectMapper, feature, enabled));

        if (this.handlerInstantiator != null) {
            objectMapper.setHandlerInstantiator(this.handlerInstantiator);
        } else if (this.applicationContext != null) {
            objectMapper.setHandlerInstantiator(
                    new SpringHandlerInstantiator(
    ...
    private void customizeDefaultFeatures(ObjectMapper objectMapper) {
        ...
        if (!this.features.containsKey(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES)) {
            configureFeature(
                    objectMapper, DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        }
    }
        ...
        public ObjectMapper create(boolean defaultUseWrapper, @Nullable JsonFactory factory) {
            ...
            if (factory != null) {
                ...
                return new XmlMapper(
                        new XmlFactory(StaxUtils.createDefensiveInputFactory()), module);
            }
        }
    }
