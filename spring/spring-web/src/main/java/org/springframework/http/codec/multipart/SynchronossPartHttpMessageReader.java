...
public class SynchronossPartHttpMessageReader extends LoggingCodecSupport
    ...
    /**
     * Configure the maximum amount of memory that is allowed to use per part. When the limit is
     * exceeded:
     *
     * <ul>
     ...
     *
     * @param byteCount the in-memory limit in bytes; if set to -1 this limit is not enforced, and
     *     all parts may be written to disk and are limited only by the {@link
     *     #setMaxDiskUsagePerPart(long) maxDiskUsagePerPart} property.
     * @since 5.1.11
     */
    ...
    public Flux<Part> read(
        ...
        return getFileStorageDirectory()
                .flatMapMany(
                        directory ->
                                        ...
                                        () -> {
                                            Path tempDirectory =
                                                    Files.createTempDirectory(
                                                            FILE_STORAGE_DIRECTORY_PREFIX);
                                            if (this.fileStorageDirectory.compareAndSet(
                                                    null, tempDirectory)) {
    ...
    private static class SynchronossPart extends AbstractSynchronossPart {
        ...
        public Flux<DataBuffer> content() {
            return DataBufferUtils.readInputStream(
                    getStorage()::getInputStream, bufferFactory, 4096);
        }
    ...
    private static class SynchronossFilePart extends SynchronossPart implements FilePart {
        ...
        public Mono<Void> transferTo(Path dest) {
            ...
            try {
                ...
                long size =
                        (input instanceof FileChannel
                                ? ((FileChannel) input).size()
                                : Long.MAX_VALUE);
                long totalWritten = 0;
    ...
    /**
     * Subscribe to the input stream and feed the Synchronoss parser. Then listen for parser output,
     * creating parts, and pushing them into the FluxSink.
     */
    private class SynchronossPartGenerator extends BaseSubscriber<DataBuffer>
            implements Consumer<FluxSink<Part>> {

        private final ReactiveHttpInputMessage inputMessage;

        private final LimitedPartBodyStreamStorageFactory storageFactory =
                new LimitedPartBodyStreamStorageFactory();

        private final Path fileStorageDirectory;

        @Nullable private NioMultipartParserListener listener;

        @Nullable private NioMultipartParser parser;

        public SynchronossPartGenerator(
                ReactiveHttpInputMessage inputMessage, Path fileStorageDirectory) {
        ...
        public void accept(FluxSink<Part> sink) {
            ...
            Charset charset =
                    Optional.ofNullable(mediaType.getCharset()).orElse(StandardCharsets.UTF_8);
            MultipartContext context =
                    new MultipartContext(mediaType.toString(), length, charset.name());
    ...
    private class LimitedPartBodyStreamStorageFactory implements PartBodyStreamStorageFactory {
        ...
        @Override
        public StreamStorage newStreamStorageForPartBody(
                Map<String, List<String>> headers, int index) {
            this.index = index;
            ...
            if (maxInMemorySize > 0 && !this.isFilePart && this.partSize >= maxInMemorySize) {
                throw new DataBufferLimitException(
                        "Part["
                                + this.index
                                + "] "
                                + "exceeded the in-memory limit of "
                                + maxInMemorySize
                                + " bytes");
            }
            if (maxDiskUsagePerPart > 0 && this.isFilePart && this.partSize > maxDiskUsagePerPart) {
                throw new DecodingException(
                        "Part["
                                + this.index
                                + "] "
                                + "exceeded the disk usage limit of "
                                + maxDiskUsagePerPart
                                + " bytes");
            }
        }
