...
public class CacheControl {
     ...
     *
     * <p>This directive is well suited for publicly caching resources, knowing that they won't
     * change within the configured amount of time. Additional directives can be also used, in case
     * resources shouldn't be cached ({@link #cachePrivate()}) or transformed ({@link
     * #noTransform()}) by shared caches.
     *
     * <p>In order to prevent caches to reuse the cached response even when it has become stale
     * (i.e. the "max-age" delay is passed), the "must-revalidate" directive should be set ({@link
     * #mustRevalidate()}
     *
     ...
     *
     * <p>This directive is well suited for telling caches that the response can be reused only if
     * the client revalidates it with the server. This directive won't disable cache altogether and
     * may result with clients sending conditional requests (with "ETag", "If-Modified-Since"
     * headers) and the server responding with "304 - Not Modified" status.
     *
     ...
     *
     * <p>This directive has the same meaning as the "must-revalidate" directive, except that it
     * does not apply to private caches (i.e. browsers, HTTP clients).
     *
     * @return {@code this}, to facilitate method chaining
     ...
     *
     * <p>This directive indicates that, in shared caches, the maximum age specified by this
     ...
     * becomes stale, up to the indicated number of seconds. If a cached response is served stale
     * due to the presence of this extension, the cache SHOULD attempt to revalidate it while still
    ...
    private String toHeaderValue() {
        ...
        if (this.staleWhileRevalidate != null) {
            appendDirective(
                    headerValue,
                    "stale-while-revalidate=" + this.staleWhileRevalidate.getSeconds());
        }
