...
public class ReactorResourceFactory implements InitializingBean, DisposableBean {
    ...
    private Supplier<ConnectionProvider> connectionProviderSupplier =
            () -> ConnectionProvider.fixed("webflux", 500);
    ...
    private Duration shutdownQuietPeriod =
            Duration.ofSeconds(LoopResources.DEFAULT_SHUTDOWN_QUIET_PERIOD);
     ...
     *
     * <p>Default is "true" in which case this factory initializes and stops the global Reactor
     * Netty resources within Spring's {@code ApplicationContext} lifecycle. If set to "false" the
     * factory manages its resources independent of the global ones.
     *
    ...
    /**
     * Add a Consumer for configuring the global Reactor Netty resources on startup. When this
     * option is used, {@link #setUseGlobalResources} is also enabled.
     *
    ...
    /**
     * Use this when you don't want to participate in global resources and you want to customize the
     * creation of the managed {@code ConnectionProvider}.
     *
    ...
    /**
     * Use this when you want to provide an externally managed {@link ConnectionProvider} instance.
     *
    ...
    /**
     * Use this when you don't want to participate in global resources and you want to customize the
     * creation of the managed {@code LoopResources}.
     *
    ...
    /**
     * Use this option when you want to provide an externally managed {@link LoopResources}
     * instance.
     *
     ...
     * submitted during the {@code shutdownQuietPeriod}, it is guaranteed to be accepted and the
     * {@code shutdownQuietPeriod} will start over.
     *
     * <p>By default, this is set to {@link LoopResources#DEFAULT_SHUTDOWN_QUIET_PERIOD} which is 2
     * seconds but can also be overridden with the system property {@link
     * reactor.netty.ReactorNetty#SHUTDOWN_QUIET_PERIOD ReactorNetty.SHUTDOWN_QUIET_PERIOD}.
     *
     * @since 5.2.4
    ...
    /**
     * Configure the maximum amount of time to wait until the disposal of the underlying resources
     * regardless if a task was submitted during the {@code shutdownQuietPeriod}.
     *
     * <p>By default, this is set to {@link LoopResources#DEFAULT_SHUTDOWN_TIMEOUT} which is 15
     * seconds but can also be overridden with the system property {@link
     * reactor.netty.ReactorNetty#SHUTDOWN_TIMEOUT ReactorNetty.SHUTDOWN_TIMEOUT}.
     *
     * @since 5.2.4
    ...
    public void destroy() {
        if (this.useGlobalResources) {
            HttpResources.disposeLoopsAndConnectionsLater(
                            this.shutdownQuietPeriod, this.shutdownTimeout)
                    .block();
        } else {
            try {
