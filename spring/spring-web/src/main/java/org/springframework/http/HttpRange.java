...
public abstract class HttpRange {
    ...
    public static List<HttpRange> parseRanges(@Nullable String ranges) {
        ...
        if (!ranges.startsWith(BYTE_RANGE_PREFIX)) {
            throw new IllegalArgumentException(
                    "Range '" + ranges + "' does not start with 'bytes='");
        }
    ...
    /**
     * Convert each {@code HttpRange} into a {@code ResourceRegion}, selecting the appropriate
     * segment of the given {@code Resource} using HTTP Range information.
     *
     ...
     * @since 4.3
     */
    public static List<ResourceRegion> toResourceRegions(
            List<HttpRange> ranges, Resource resource) {
    ...
    /**
     * Turn a {@code Resource} into a {@link ResourceRegion} using the range information contained
     * in the current {@code HttpRange}.
     *
    ...
    public ResourceRegion toResourceRegion(Resource resource) {
        ...
        // afterwards...
        // Note: custom InputStreamResource subclasses could provide a pre-calculated content
        // length!
        Assert.isTrue(
        ...
        Assert.isTrue(
                start < contentLength, "'position' exceeds the resource length " + contentLength);
