...
public final class ResponseCookie extends HttpCookie {
    ...
    /**
     * Factory method to obtain a builder for a server-defined cookie. Unlike {@link #from(String,
     * String)} this option assumes input from a remote server, which can be handled more leniently,
     * e.g. ignoring a empty domain name with double quotes.
     *
     * @param name the cookie name
     * @param value the cookie value
     * @return a builder to create the cookie with
    ...
    private static ResponseCookieBuilder from(
            final String name, final String value, boolean lenient) {
     ...
     *
     * <p>A positive value indicates when the cookie expires relative to the current time. A value
     * of 0 means the cookie should expire immediately. A negative value means no "Max-Age"
     * attribute in which case the cookie is removed when the browser is closed.
     */
    ...
    public interface ResponseCookieBuilder {
         ...
         *
         * <p>A positive value indicates when the cookie should expire relative to the current time.
         * A value of 0 means the cookie should expire immediately. A negative value results in no
    ...
    private static class Rfc6265Utils {
        ...
        public static void validateCookieName(String name) {
            for (int i = 0; i < name.length(); i++) {
                ...
                if (c >= 0x80) {
                    throw new IllegalArgumentException(
                            name
                                    + ": RFC2616 token can only have US-ASCII: 0x"
        ...
        public static void validateCookieValue(@Nullable String value) {
            ...
            for (int i = start; i < end; i++) {
                ...
                if (c >= 0x80) {
                    throw new IllegalArgumentException(
                            "RFC2616 cookie value can only have US-ASCII chars: 0x"
                                    + Integer.toHexString(c));
                }
            }
        }
        ...
        public static void validateDomain(@Nullable String domain) {
            ...
            for (int i = 0, c = -1; i < domain.length(); i++) {
                ...
                if (DOMAIN_CHARS.indexOf(c) == -1
                        ...
                        || (p == '-' && c == '.')) {
                    throw new IllegalArgumentException(
                            domain + ": invalid cookie domain char '" + c + "'");
                }
            }
        }
