...
public abstract class Jackson2CodecSupport {
    ...
    /**
     * The key for the hint to specify a "JSON View" for encoding or decoding with the value
     * expected to be a {@link Class}.
     *
    ...
    /**
     * The key for the hint to access the actual ResolvableType passed into {@link
     * org.springframework.http.codec.HttpMessageReader#read(ResolvableType, ResolvableType,
     * ServerHttpRequest, ServerHttpResponse, Map)} (server-side only). Currently set when the
     * method argument has generics because in case of reactive types, use of {@code
     * ResolvableType.getGeneric()} means no MethodParameter source and no knowledge of the
     * containing class.
     */
