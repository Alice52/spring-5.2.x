...
public class FormHttpMessageConverter implements HttpMessageConverter<MultiValueMap<String, ?>> {
     ...
     * multipart request.
     *
     * <p>As of 5.0, this is also used for part headers including {@code Content-Disposition} (and
     * its filename parameter) unless (the mutually exclusive) {@link #setMultipartCharset
        ...
        for (HttpMessageConverter<?> candidate : this.partConverters) {
            if (candidate instanceof AbstractHttpMessageConverter) {
                AbstractHttpMessageConverter<?> converter =
                        (AbstractHttpMessageConverter<?>) candidate;
                // Only override default charset if the converter operates with a charset to begin
                // with...
     ...
     * based on the {@code encoded-word} syntax defined in RFC 2047 and relies on {@code
     * MimeUtility} from {@code javax.mail}.
     *
    ...
    /**
     * Return the content type used to write forms, given the preferred content type. By default,
     * this method returns the given content type, but adds the {@linkplain #setCharset(Charset)
     * charset} if it does not have one. If {@code contentType} is {@code null}, {@code
     * application/x-www-form-urlencoded; charset=UTF-8} is returned.
     *
                            ...
                            value -> {
                                try {
                                    ...
                                    if (value != null) {
                                        ...
                                        builder.append(
                                                URLEncoder.encode(
                                                        String.valueOf(value), charset.name()));
                                    }
    ...
    private void writePart(String name, HttpEntity<?> partEntity, OutputStream os)
        ...
        for (HttpMessageConverter<?> messageConverter : this.partConverters) {
            if (messageConverter.canWrite(partType, partContentType)) {
                ...
                multipartMessage
                        .getHeaders()
                        .setContentDispositionFormData(name, getFilename(partBody));
     ...
     * @return the part Object itself it is an {@link HttpEntity}, or a newly built {@link
     *     HttpEntity} wrapper for that part
     */
