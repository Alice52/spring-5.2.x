...
abstract class AbstractMockWebServerTests {
    ...
    private MockResponse multipartMixedRequest(RecordedRequest request) {
        MediaType mediaType = MediaType.parseMediaType(request.getHeader(CONTENT_TYPE));
        assertThat(mediaType.isCompatibleWith(MULTIPART_MIXED))
                .as(MULTIPART_MIXED.toString())
                .isTrue();
        assertMultipart(request, mediaType);
        return new MockResponse().setResponseCode(200);
    }
            ...
            try {
                ...
                if (request.getPath().equals("/get")) {
                    ...
                    return postRequest(
                            ...
                            helloWorld,
                            "/post/1",
                            textContentType.toString(),
                            helloWorldBytes);
                } else if (request.getPath().equals("/jsonpost")) {
                    return jsonPostRequest(
                            request, "/jsonpost/1", "application/json; charset=utf-8");
                    ...
                    return patchRequest(
                            request, helloWorld, textContentType.toString(), helloWorldBytes);
