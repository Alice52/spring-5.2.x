            ...
            if (referenceName != null) {
                ...
                assertThat(references.isMappedToReference((char) character))
                        .as(
                                "The unicode character "
                                        + character
                                        + " should be mapped to a reference")
                        .isTrue();
                ...
                assertThat(references.isMappedToReference((char) character))
                        .as(
                                "The unicode character "
                                        + character
                                        + " should not be mapped to a reference")
                        .isFalse();
                assertThat(references.convertToReference((char) character))
                        .as(
                                "No entity reference of unicode character "
                                        + character
                                        + " should exist")
                        .isNull();
            }
    ...
    private static class CharacterEntityResourceIterator {
        ...
        public CharacterEntityResourceIterator() {
            try {
                ...
                throw new IllegalStateException(
                        "Failed to open definition resource [" + DTD_FILE + "]");
            }
        }
        ...
        private boolean readNextEntity() {
            try {
                ...
                throw new IllegalStateException(
                        "Could not parse definition resource: " + ex.getMessage());
            }
        }

        private boolean navigateToNextEntity() throws IOException {
            while (tokenizer.nextToken() != StreamTokenizer.TT_WORD
                    || !"ENTITY".equals(tokenizer.sval)) {
