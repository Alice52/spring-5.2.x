...
public class Jaxb2Marshaller
    ...
    /**
     * Set the packages to search for classes with JAXB2 annotations in the classpath. This is using
     * a Spring-bases search and therefore analogous to Spring's component-scan feature ({@link
    ...
    /**
     * Set the {@code JAXBContext} properties. These implementation-specific properties will be set
     * on the underlying {@code JAXBContext}.
     */
    ...
    /**
     * Set whether to lazily initialize the {@link JAXBContext} for this marshaller. Default is
     * {@code false} to initialize on startup; can be switched to {@code true}.
     *
     ...
     * class.
     *
     * <p>Default is {@code false}, meaning that {@code supports(Class)} always returns {@code
     * false} for {@code JAXBElement} classes (though {@link #supports(Type)} can return {@code
     * true}, since it can obtain the type parameters of {@code JAXBElement}).
     *
     ...
     * processing of external entities will only be enabled/disabled when the {@code Source} passed
     * to {@link #unmarshal(Source)} is a {@link SAXSource} or {@link StreamSource}. It has no
     * effect for {@link DOMSource} or {@link StAXSource} instances.
     *
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (!hasContextPath && !hasClassesToBeBound && !hasPackagesToScan) {
            throw new IllegalArgumentException(
                    "Setting either 'contextPath', 'classesToBeBound', "
    ...
    private JAXBContext createJaxbContextFromPackages(String... packagesToScan)
            throws JAXBException {
        if (logger.isDebugEnabled()) {
            logger.debug(
        ...
        if (logger.isDebugEnabled()) {
            logger.debug(
                    "Found JAXB2 classes: ["
                            + StringUtils.arrayToCommaDelimitedString(jaxb2Classes)
                            + "]");
        }
        ...
        if (genericType instanceof ParameterizedType) {
            ...
            if (JAXBElement.class == parameterizedType.getRawType()
                ...
                if (typeArgument instanceof Class) {
                    ...
                    return ((classArgument.isArray()
                                    && Byte.TYPE == classArgument.getComponentType())
    ...
    private boolean isStandardClass(Class<?> clazz) {
        return (String.class == clazz
                ...
                ||
                // Source and subclasses should be supported according to the JAXB2 spec, but aren't
                // in the RI
     ...
     *
     * <p>The default implementation sets the {@link #setMarshallerProperties defined properties},
     * the {@link #setValidationEventHandler validation event handler}, the {@link #setSchemas
     * schemas}, {@link #setMarshallerListener listener}, and {@link #setAdapters adapters}.
     */
        ...
        try {
            ...
            if (this.mtomEnabled && mimeContainer != null) {
                unmarshaller.setAttachmentUnmarshaller(
                        new Jaxb2AttachmentUnmarshaller(mimeContainer));
            }
    ...
    private static class Jaxb2AttachmentMarshaller extends AttachmentMarshaller {
        ...
        public String addMtomAttachment(
            ...
            ByteArrayDataSource dataSource =
                    new ByteArrayDataSource(mimeType, data, offset, length);
            return addMtomAttachment(
                    new DataHandler(dataSource), elementNamespace, elementLocalName);
        }
