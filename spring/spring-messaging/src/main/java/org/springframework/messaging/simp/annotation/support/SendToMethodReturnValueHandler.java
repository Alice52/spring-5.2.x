...
public class SendToMethodReturnValueHandler implements HandlerMethodReturnValueHandler {
    ...
    /**
     * Configure a default prefix to add to message destinations in cases where a method is
     * annotated with {@link SendToUser @SendToUser} but does not specify any destinations through
     * the annotation's value attribute.
     *
    ...
    public void handleReturnValue(
        ...
        if (sendTo != null || sendToUser == null) {
            String[] destinations =
                    getTargetDestinations(sendTo, message, this.defaultDestinationPrefix);
            for (String destination : destinations) {
                destination = destinationHelper.expandTemplateVars(destination);
    ...
    private DestinationHelper getDestinationHelper(
        ...
        SendToUser m1 =
                AnnotatedElementUtils.findMergedAnnotation(
                        returnType.getExecutable(), SendToUser.class);
        SendTo m2 =
                AnnotatedElementUtils.findMergedAnnotation(
                        returnType.getExecutable(), SendTo.class);
        ...
        SendTo c2 =
                AnnotatedElementUtils.findMergedAnnotation(
                        returnType.getDeclaringClass(), SendTo.class);
    ...
    public String toString() {
        return "SendToMethodReturnValueHandler [annotationRequired="
                + this.annotationRequired
                + "]";
    }
