...
class ChannelSendOperator<T> extends Mono<Void> implements Scannable {
    ...
    private enum State {
        ...
        /**
         * At least one signal of any kind has been received; we're ready to call the write function
         * and proceed with actual writing.
         */
        FIRST_SIGNAL_RECEIVED,

        /**
         * The write subscriber has subscribed and requested; we're going to emit the cached
         * signals.
         */
        EMITTING_CACHED_SIGNALS,

        /**
         * The write subscriber has subscribed, and cached signals have been emitted to it; we're
         * ready to switch to a simple pass-through mode for all remaining signals.
         */
        READY_TO_WRITE
    }
     ...
     * adapter) that pre-fetches and waits for the first signal before deciding whether to hook in
     * to the write subscriber.
     *
     * <p>Acts as:
     *
     * <ul>
     ...
     * </ul>
     *
     * <p>Also uses {@link WriteCompletionBarrier} to communicate completion and detect cancel
     * signals from the completion subscriber.
     */
        ...
        public final void onNext(T item) {
            ...
            synchronized (this) {
                if (this.state == State.READY_TO_WRITE) {
                    ...
                    this.writeCompletionBarrier.onError(
                            new IllegalStateException("Unexpected item."));
                }
            }
        }
     ...
     * subscriber.
     *
     * <p>The completionSubscriber is subscribed initially to the WriteBarrier. Later after the
     * first signal is received, we need one more subscriber instance (per spec can only subscribe
     * once) to subscribe to the write function and switch to delegating completion signals from it.
     */
    private class WriteCompletionBarrier implements CoreSubscriber<Void>, Subscription {
        ...
        /**
         * Connect the underlying completion subscriber to this barrier in order to track cancel
         * signals and pass them on to the write barrier.
         */
        public void connect() {
