...
public class SimpleBrokerMessageHandler extends AbstractBrokerMessageHandler {
    ...
    /**
     * When configured, the given PathMatcher is passed down to the underlying SubscriptionRegistry
     * to use for matching destination to subscriptions.
     *
    ...
    private void initPathMatcherToUse() {
        if (this.pathMatcher != null
            ...
            ((DefaultSubscriptionRegistry) this.subscriptionRegistry)
                    .setPathMatcher(this.pathMatcher);
        }
    }
    ...
    private void initCacheLimitToUse() {
        if (this.cacheLimit != null
                && this.subscriptionRegistry instanceof DefaultSubscriptionRegistry) {
            ((DefaultSubscriptionRegistry) this.subscriptionRegistry)
                    .setCacheLimit(this.cacheLimit);
        }
    }

    /**
     * Configure the name of a header that a subscription message can have for the purpose of
     * filtering messages matched to the subscription. The header value is expected to be a Spring
     * EL boolean expression to be applied to the headers of messages matched to the subscription.
     *
     * <p>For example:
     *
     * <pre>
     ...
     * </pre>
     *
     * <p>By default this is set to "selector". You can set it to a different name, or to {@code
     * null} to turn off support for a selector header.
     *
     ...
     * server will write or send a heartbeat. The second is how often the client should write. 0
     * means no heartbeats.
     *
     * <p>By default this is set to "0, 0" unless the {@link #setTaskScheduler taskScheduler} in
     * which case the default becomes "10000,10000" (in milliseconds).
     *
     * @since 4.2
     */
    ...
    protected void handleMessageInternal(Message<?> message) {
        ...
        if (SimpMessageType.MESSAGE.equals(messageType)) {
            ...
            if (sessionId != null) {
                ...
                this.sessions.put(
                        sessionId,
                        new SessionInfo(sessionId, user, outChannel, heartbeatIn, heartbeatOut));
            ...
            if (clientHeartbeat != null && serverHeartbeat != null) {
                this.readInterval =
                        (clientHeartbeat[0] > 0 && serverHeartbeat[1] > 0
                                ? Math.max(clientHeartbeat[0], serverHeartbeat[1])
    ...
    private class HeartbeatTask implements Runnable {
        ...
        public void run() {
            ...
            for (SessionInfo info : sessions.values()) {
                if (info.getReadInterval() > 0
                        && (now - info.getLastReadTime()) > info.getReadInterval()) {
