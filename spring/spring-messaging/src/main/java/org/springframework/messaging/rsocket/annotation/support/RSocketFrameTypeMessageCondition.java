...
public class RSocketFrameTypeMessageCondition
     ...
     * "REQUEST_CHANNEL".
     *
     * @deprecated as of 5.2.2 because matching to all interaction types is too flexible. Please use
     *     one of the other constants in this class that match to specific frames.
     */
    @Deprecated
    ...
    public static FrameType getFrameType(Message<?> message) {
        return (FrameType)
                message.getHeaders().get(RSocketFrameTypeMessageCondition.FRAME_TYPE_HEADER);
    }

    /**
     * Return a condition for matching the RSocket request interaction type with that is selected
     * based on the delcared request and response cardinality of some handler method.
     *
     ...
     * </table>
     *
     * @param cardinalityIn -- the request cardinality: 1 for a single payload, 2 for many payloads,
     *     and 0 if input is not handled.
     * @param cardinalityOut -- the response cardinality: 0 for no output payloads, 1 for a single
     *     payload, and 2 for many payloads.
     * @return a condition to use for matching the interaction type
     * @since 5.2.2
     */
    public static RSocketFrameTypeMessageCondition getCondition(
            int cardinalityIn, int cardinalityOut) {
