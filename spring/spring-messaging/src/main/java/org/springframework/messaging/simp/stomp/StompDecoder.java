...
public class StompDecoder {
     ...
     *
     * <p>If the buffer contains one ore more STOMP frames, those are returned and the buffer reset
     * to point to the beginning of the unused partial content.
     *
     * <p>The output partialMessageHeaders map is used to store successfully parsed headers in case
     * of partial content. The caller can then check if a "content-length" header was read, which
     * helps to determine how much more content is needed before the next attempt to decode.
     *
     * @param byteBuffer the buffer to decode the STOMP frame from
     * @param partialMessageHeaders an empty output map that will store the last successfully parsed
     *     partialMessageHeaders in case of partial message content in cases where the partial
     *     buffer ended with a partial STOMP frame
     * @return the decoded messages, or an empty list if none
     * @throws StompConversionException raised in case of decoding issues
     */
        ...
        if (command.length() > 0) {
            ...
            decodedMessage =
                    MessageBuilder.createMessage(
                            HEARTBEAT_PAYLOAD, headerAccessor.getMessageHeaders());
            if (logger.isTraceEnabled()) {
    ...
    /**
     * Skip one ore more EOL characters at the start of the given ByteBuffer. STOMP, section 2.1
     * says: "The NULL octet can be optionally followed by multiple EOLs."
     */
