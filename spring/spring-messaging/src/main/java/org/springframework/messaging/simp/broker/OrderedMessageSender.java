...
class OrderedMessageSender implements MessageChannel {
    ...
    /**
     * Install or remove an {@link ExecutorChannelInterceptor} that invokes a completion task once
    ...
    static void configureOutboundChannel(MessageChannel channel, boolean preservePublishOrder) {
        if (preservePublishOrder) {
            ...
            if (execChannel.getInterceptors().stream()
                    .noneMatch(i -> i instanceof CallbackInterceptor)) {
    ...
    private void addCompletionCallback(Message<?> msg) {
        ...
        Assert.isTrue(
                accessor != null && accessor.isMutable(),
                "Expected mutable SimpMessageHeaderAccessor");
    ...
    private static class CallbackInterceptor implements ExecutorChannelInterceptor {
        ...
        public void afterMessageHandled(
            ...
            Runnable task =
                    (Runnable) msg.getHeaders().get(OrderedMessageSender.COMPLETION_TASK_HEADER);
            if (task != null) {
                task.run();
            }
        }
