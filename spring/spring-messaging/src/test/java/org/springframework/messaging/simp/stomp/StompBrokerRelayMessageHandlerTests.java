...
class StompBrokerRelayMessageHandlerTests {
    ...
    void virtualHost() {
        ...
        assertThat(headers1.getSessionId())
                .isEqualTo(StompBrokerRelayMessageHandler.SYSTEM_SESSION_ID);
    ...
    private static class StubTcpOperations implements TcpOperations<byte[]> {
        ...
        public StompHeaderAccessor getSentHeaders(int index) {
            assertThat(getSentMessages().size() > index)
                    .as("Size: " + getSentMessages().size())
                    .isTrue();
