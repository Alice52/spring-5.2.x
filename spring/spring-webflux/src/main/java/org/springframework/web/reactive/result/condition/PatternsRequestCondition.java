...
public final class PatternsRequestCondition
     ...
     *       patterns in "other" using {@link PathPattern#combine(PathPattern)}.
     ...
     * at a time, from top to bottom. If all compared patterns match equally, but one instance has
    ...
    public int compareTo(PatternsRequestCondition other, ServerWebExchange exchange) {
        ...
        while (iterator.hasNext() && iteratorOther.hasNext()) {
            int result =
                    PathPattern.SPECIFICITY_COMPARATOR.compare(
                            iterator.next(), iteratorOther.next());
            if (result != 0) {
                return result;
            }
        }
