...
public abstract class AbstractNamedValueArgumentResolver
    ...
    public AbstractNamedValueArgumentResolver(
        ...
        this.expressionContext =
                (factory != null ? new BeanExpressionContext(factory, null) : null);
    }
        ...
        if (resolvedName == null) {
            return Mono.error(
                    new IllegalArgumentException(
                            "Specified name must not resolve to null: ["
                                    + namedValueInfo.name
                                    + "]"));
        }
                        ...
                        arg -> {
                            ...
                            arg =
                                    applyConversion(
                                            ...
                                            exchange);
                            handleResolvedValue(
                                    arg, namedValueInfo.name, parameter, model, exchange);
     ...
     * @param parameter the method parameter to resolve to an argument value (pre-nested in case of
     *     a {@link java.util.Optional} declaration)
    ...
    private Mono<Object> getDefaultValue(
        ...
        return Mono.fromSupplier(
                () -> {
                    ...
                    value =
                            handleNullValue(
                                    namedValueInfo.name, value, parameter.getNestedParameterType());
                    value =
                            applyConversion(
                                    value, namedValueInfo, parameter, bindingContext, exchange);
    ...
    protected void handleMissingValue(String name, MethodParameter parameter) {
        ...
        throw new ServerWebInputException(
                "Missing argument '" + name + "' for method " + "parameter of type " + typeName,
                parameter);
    }

    /**
     * A {@code null} results in a {@code false} value for {@code boolean}s or an exception for
     * other primitives.
     */
    @Nullable
