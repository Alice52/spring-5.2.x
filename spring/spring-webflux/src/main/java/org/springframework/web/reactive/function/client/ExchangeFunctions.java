...
public abstract class ExchangeFunctions {
    ...
    /**
     * Create an {@code ExchangeFunction} with the given {@code ClientHttpConnector}. This is the
     * same as calling {@link #create(ClientHttpConnector, ExchangeStrategies)} and passing {@link
        ...
        public Mono<ClientResponse> exchange(ClientRequest clientRequest) {
            ...
            return this.connector
                    .connect(
                            httpMethod,
                            url,
                            httpRequest -> clientRequest.writeTo(httpRequest, this.strategies))
                    .doOnRequest(n -> logRequest(clientRequest))
                    .doOnCancel(
                            () -> logger.debug(logPrefix + "Cancel signal (to close connection)"))
                    .map(
                            httpResponse -> {
