...
public abstract class RouterFunctions {
     ...
     *
     * <p>For instance, the following example routes GET requests for "/user" to the {@code
     * listUsers} method in {@code userController}:
     *
     * <pre class="code">
     ...
     * @return a router function that routes to {@code handlerFunction} if {@code predicate}
     ...
     * @param <T> the type of response returned by the handler function
     * @return a router function that routes to {@code routerFunction} if {@code predicate}
     *     evaluates to {@code true}
     * @see RequestPredicates
     */
    ...
    /**
     * Returns the resource lookup function used by {@link #resources(String, Resource)}. The
     * returned function can be {@linkplain Function#andThen(Function) composed} on, for instance to
    ...
    public interface Builder {
         ...
         * that match the given pattern and predicate.
         *
         * <p>For instance, the following example routes PATCH requests for "/user" that contain
         * JSON to the {@code editUser} method in {@code userController}:
         *
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; route =
         *   RouterFunctions.route()
         ...
         * @param handlerFunction the handler function to handle all requests that match the
         *     predicate
         * @return this builder
         * @since 5.2
         * @see RequestPredicates
         ...
         * predicate, so that GET requests for "/user" will list users, and POST request for "/user"
        ...
        /**
         * Route to a built router function if the given request predicate applies. This method can
         * be used to create <strong>nested routes</strong>, where a group of routes share a common
         * path (prefix), header, or other request predicate.
         *
         * <p>For instance, the following example creates a nested route with a "/user" path
         * predicate, so that GET requests for "/user" will list users, and POST request for "/user"
         * will create a new user.
        ...
        /**
         * Route to the supplied router function if the given path prefix pattern applies. This
         * method can be used to create <strong>nested routes</strong>, where a group of routes
         * share a common path prefix. Specifically, this method can be used to merge externally
         * defined router functions under a path prefix.
         *
         * <p>For instance, the following example creates a nested route with a "/user" path
         * predicate that delegates to the router function defined in {@code userController}, and
         * with a "/order" path that delegates to {@code orderController}.
         *
         * <pre class="code">
         * RouterFunction&lt;ServerResponse&gt; nestedRoute =
         *   RouterFunctions.route()
        ...
        /**
         * Route to a built router function if the given path prefix pattern applies. This method
         * can be used to create <strong>nested routes</strong>, where a group of routes share a
         * common path prefix.
         *
         * <p>For instance, the following example creates a nested route with a "/user" path
        ...
        Builder onError(
                ...
                BiFunction<? super Throwable, ServerRequest, Mono<ServerResponse>>
                        responseProvider);
    ...
    abstract static class AbstractRouterFunction<T extends ServerResponse>
    ...
    static final class SameComposedRouterFunction<T extends ServerResponse>
        ...
        public Mono<HandlerFunction<T>> route(ServerRequest request) {
            return Flux.concat(
                            this.first.route(request), Mono.defer(() -> this.second.route(request)))
                    .next();
        }
    ...
    /**
     * A composed routing function that first invokes one function, and then invokes another
     * function (of a different response type) if this route had {@linkplain Mono#empty() no
     * result}.
     */
    static final class DifferentComposedRouterFunction
        ...
        private <T extends ServerResponse> HandlerFunction<T> cast(
                            ...
                            nestedRequest -> {
                                if (logger.isTraceEnabled()) {
                                    ...
                                    logger.trace(
                                            logPrefix
                                                    + String.format(
                                                            "Matched nested %s", this.predicate));
                                }
                    ...
                    () -> {
                        ServerRequest request =
                                new DefaultServerRequest(
                                        exchange, this.strategies.messageReaders());
                                        ...
                                        response ->
                                                wrapException(
                                                        () ->
                                                                response.writeTo(
                                                                        exchange,
                                                                        new HandlerStrategiesResponseContext(
                                                                                this.strategies))));
                    });
        }
