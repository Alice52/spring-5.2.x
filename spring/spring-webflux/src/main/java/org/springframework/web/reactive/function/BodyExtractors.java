        ...
        return (inputMessage, context) ->
                readWithMessageReaders(
                        ...
                        (HttpMessageReader<T> reader) ->
                                readToMono(inputMessage, context, elementType, reader),
    ...
    private static <T> BodyExtractor<Flux<T>, ReactiveHttpInputMessage> toFlux(
        ...
        return (inputMessage, context) ->
                readWithMessageReaders(
                        ...
                        elementType,
                        (HttpMessageReader<T> reader) ->
                                readToFlux(inputMessage, context, elementType, reader),
                        ...
                        () -> {
                            List<MediaType> mediaTypes =
                                    context.messageReaders().stream()
                                            .flatMap(
                                                    reader ->
                                                            reader.getReadableMediaTypes().stream())
                                            .collect(Collectors.toList());
                            return errorFunction.apply(
                                    new UnsupportedMediaTypeException(
                                            contentType, mediaTypes, elementType));
                        });
    }
