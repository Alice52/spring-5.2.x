 ...
 * </ul>
 *
 * You can also use {@code bodyToMono(Void.class)} if no response content is expected. However keep
 * in mind the connection will be closed, instead of being placed back in the pool, if any content
 * does arrive. This is in contrast to {@link #releaseBody()} which does consume the full body and
...
public interface ClientResponse {
    ...
    /**
     * Return this response as a delayed {@code ResponseEntity} containing status and headers, but
     * no body. Calling this method will {@linkplain #releaseBody() release} the body of the
     * response.
     *
    ...
    /**
     * Create a {@link WebClientResponseException} that contains the response status, headers, body,
     ...
     *
     * <p>The prefix is based on {@linkplain ClientRequest#logPrefix()}, which itself is based on
     * the value of the {@link ClientRequest#LOG_ID_ATTRIBUTE LOG_ID_ATTRIBUTE} request attribute,
     * further surrounded with "[" and "]".
     *
     * @return the log message prefix or an empty String if the {@link
     *     ClientRequest#LOG_ID_ATTRIBUTE LOG_ID_ATTRIBUTE} is not set
     * @since 5.2.3
     */
    String logPrefix();
    ...
    interface Builder {
         ...
         * {@linkplain MultiValueMap#remove(Object) remove} values, or use any of the other {@link
