 ...
 *
 * <p>{@link org.springframework.beans.factory.config.BeanPostProcessor} implementation that wraps
 * each eligible bean with an AOP proxy, delegating to specified interceptors before invoking the
 * bean itself.
 *
...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
    ...
    /**
     * Set whether or not the proxy should be frozen, preventing advice from being added to it once
     * it is created.
     *
     * <p>Overridden from the super class to prevent the proxy configuration from being frozen
     ...
     * advisors have been found. If a {@code TargetSourceCreator} returns a {@link TargetSource} for
     * a specific bean, that bean will be proxied in any case.
     *
     * <p>{@code TargetSourceCreators} can only be invoked if this post processor is used in a
     * {@link BeanFactory} and its {@link BeanFactoryAware} callback is triggered.
     *
     * @param targetSourceCreators the list of {@code TargetSourceCreators}. Ordering is
     *     significant: The {@code TargetSource} returned from the first matching {@code
     ...
     *
     * <p>If this property isn't set, there will be zero common interceptors. This is perfectly
     * valid, if "specific" interceptors such as matching Advisors are all we want.
     */
    ...
    /**
     * Return the owning {@link BeanFactory}. May be {@code null}, as this post-processor doesn't
     * need to belong to a bean factory.
     */
    @Nullable
    ...
    /**
     * 此处是真正创建aop代理的地方，在实例化之后，初始化之后就行处理 首先查看是否在earlyProxyReferences里存在，如果有就说明处理过了，不存在就考虑是否要包装，也就是代理
     *
     * <p>Create a proxy with the configured interceptors if the bean is identified as one to proxy
     * by the subclass.
     *
     * @see #getAdvicesAndAdvisorsForBean
     */
     ...
     *
     * <p>Wrap the given bean if necessary, i.e. if it is eligible for being proxied.
     *
     * @param bean the raw bean instance
     * @param beanName the name of the bean
     ...
     *
     * <p>Return whether the given bean class represents an infrastructure class that should never
     * be proxied.
     *
     ...
     * reference or if the existing target instance needs to be preserved. This implementation
     * returns {@code false} unless the bean name indicates an "original instance" according to
     * {@code AutowireCapableBeanFactory} conventions.
     *
     * @param beanClass the class of the bean
     * @param beanName the name of the bean
     ...
     *
     * <p>This implementation uses the "customTargetSourceCreators" property. Subclasses can
     ...
     *
     * <p>Create an AOP proxy for the given bean.
     *
     * @param beanClass the class of the bean
     * @param beanName the name of the bean
     ...
     *
     * <p>Determine whether the given bean should be proxied with its target class rather than its
     * interfaces.
     *
