 ...
 *
 * <p>CGLIB-based {@link AopProxy} implementation for the Spring AOP framework.
 *
...
class CglibAopProxy implements AopProxy, Serializable {
     ...
     * and also verifies that {@code null} is not returned as a primitive.
     */
    @Nullable
     ...
     * 获取cglib的代理对象
     *
     * @param classLoader the class loader to create the proxy with (or {@code null} for the
     *     low-level proxy facility's default)
     * @return
     */
    @Override
    ...
    /**
     * Checks to see whether the supplied {@code Class} has already been validated and validates it
     * if not.
     */
     ...
     * across ClassLoaders, and writes warnings to the log for each one found.
     */
    ...
    private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
        ...
        if (isStatic && isFrozen) {
            ...
            System.arraycopy(
                    fixedCallbacks, 0, callbacks, mainCallbacks.length, fixedCallbacks.length);
    ...
    public boolean equals(@Nullable Object other) {
        return (this == other
                || (other instanceof CglibAopProxy
                        && AopProxyUtils.equalsInProxy(
                                this.advised, ((CglibAopProxy) other).advised)));
    }
    ...
    /**
     * 方法拦截器被用来没有通知链的静态目标 Method interceptor used for static targets with no advice chain. The call
     * is passed directly back to the target. Used when the proxy needs to be exposed and it can't
    ...
    /**
     * 拦截器被用来执行动态目标不需要创建方法执行器或者计算通知链 Interceptor used to invoke a dynamic target without creating a
     * method invocation or evaluating an advice chain. (We know there was no advice for this
     * method.)
     */
     ...
     * 静态目标的分发器
     *
     * <p>Dispatcher for a static target. Dispatcher is much faster than interceptor. This will be
     * used whenever it can be determined that a method definitely does not return "this"
     */
     ...
     * 分发方法
     *
     * <p>Dispatcher for any methods declared on the Advised class.
     */
    ...
    /**
     * General purpose AOP callback. Used when the target is dynamic or when the proxy is not
     * frozen.
     */
    private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
        ...
        public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy)
            ...
            try {
                ...
                // Get as late as possible to minimize the time we "own" the target, in case it
                // comes from a pool...
    ...
    private static class ProxyCallbackFilter implements CallbackFilter {
         ...
         *
         * <p>The callbacks for each proxy are built up of a set of fixed callbacks for general use
         * and then a set of callbacks that are specific to a method for use on static targets with
         * a fixed advice chain.
         *
         ...
         *   <dt>For equals():
         *   <dd>The EqualsInterceptor is used to redirect equals() calls to a special handler to
         *       this proxy.
         ...
         *       advice chain. Otherwise a DynamicAdvisedInterceptor is used.
         *   <dt>For non-advised methods:
         *   <dd>Where it can be determined that the method will not return {@code this} or when
         *       {@code ProxyFactory.getExposeProxy()} returns {@code false}, then a Dispatcher is
        ...
        public int accept(Method method) {
            ...
            if (haveAdvice || !isFrozen) {
                ...
                // See if the return type of the method is outside the class hierarchy of the target
                // type.
                // If so we know it never needs to have return type massage and can use a
                // dispatcher.
                // If the proxy is being exposed, then must use the interceptor the correct one is
                // already
                // configured. If the target is not static, then we cannot use a dispatcher because
                // the
        ...
        public boolean equals(@Nullable Object other) {
            ...
            if (this.advised.getTargetSource().isStatic()
                    != otherAdvised.getTargetSource().isStatic()) {
                return false;
            }
