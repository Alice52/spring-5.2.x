 ...
 *
 * <p>Base class for AOP Alliance {@link org.aopalliance.aop.Advice} classes wrapping an AspectJ
 * aspect or an AspectJ-annotated advice method.
 *
 * @author Rod Johnson
...
public abstract class AbstractAspectJAdvice
    ...
    /**
     * Lazily instantiate joinpoint for the current invocation. Requires MethodInvocation to be
     * bound with ExposeInvocationInterceptor.
     *
    ...
    public final Pointcut buildSafePointcut() {
        ...
        MethodMatcher safeMethodMatcher =
                MethodMatchers.intersection(
                        new AdviceExcludingMethodMatcher(this.aspectJAdviceMethod),
                        pc.getMethodMatcher());
    ...
    /**
     * We need to hold the returning name at this level for argument binding calculations, this
     * method allows the afterReturning advice subclass to set the name.
     */
    ...
    /**
     * We need to hold the throwing name at this level for argument binding calculations, this
     * method allows the afterThrowing advice subclass to set the name.
     */
     ...
     * to be bound to which argument name. There are multiple strategies for determining this
     * binding, which are arranged in a ChainOfResponsibility.
     */
     ...
     * 参数绑定
     *
     * <p>Take the arguments at the method execution join point and output a set of arguments to the
     * advice method.
     *
    ...
    /**
     * 这三个参数 JoinPointMatch 都是相同的 returnValue 当执行后置返回通知的时候 传值 其他为null Throwable 当执行后置异常通知的时候
     * 传值，其他为null
     *
     * <p>Invoke the advice method.
     *
    ...
    protected JoinPointMatch getJoinPointMatch() {
        ...
        // 不知道你是否还记得 我们在获取Advisor的时候
        // 调用过这样的一个方法org.springframework.aop.aspectj.AspectJProxyUtils#makeAdvisorChainAspectJCapableIfNecessary
        // 在这个方法中会有这样的一段代码
        // if (foundAspectJAdvice && //!advisors.contains(ExposeInvocationInterceptor.ADVISOR)) {
        //     如果获取到了 Advisor  则向 Advisor集合中添加第一个元素 即 ExposeInvocationInterceptor.ADVISOR
        // 也就是说 我们的 Advisor列表中的第一个元素为ExposeInvocationInterceptor.ADVISOR
        // 它是一个DefaultPointcutAdvisor的实例
