...
public class AspectJAdviceParameterNameDiscoverer implements ParameterNameDiscoverer {
    ...
    /**
     * Indicate whether {@link IllegalArgumentException} and {@link AmbiguousBindingException} must
     * be thrown as appropriate in the case of failing to deduce advice parameter names.
     *
     ...
     *
     * <p>See the {@link AspectJAdviceParameterNameDiscoverer class level javadoc} for this class
    ...
    public String[] getParameterNames(Method method) {
        ...
        try {
            ...
            while ((this.numberOfRemainingUnboundArguments > 0)
                    && algorithmicStep < STEP_FINISHED) {
                switch (algorithmicStep++) {
                    ...
                    default:
                        throw new IllegalStateException(
                                "Unknown algorithmic step: " + (algorithmicStep - 1));
                }
            }
    ...
    /**
     * If a throwing name was specified and there is exactly one choice remaining (argument that is
     * a subtype of Throwable) then bind it.
     */
     ...
     */
    private void maybeExtractVariableNamesFromArgs(
            @Nullable String argsSpec, List<String> varNames) {
        if (argsSpec == null) {
            return;
        }
    ...
    /**
     * Parse the string pointcut expression looking for this(), target() and args() expressions. If
     * we find one, try and extract a candidate variable name and bind it.
     */
    private void maybeBindThisOrTargetOrArgsFromPointcutExpression() {
        ...
        for (int i = 0; i < tokens.length; i++) {
            if (tokens[i].equals("this")
                ...
                // we may have found some var names that were bound in previous primitive args
                // binding step,
                // filter them out...
    ...
    private PointcutBody getPointcutBody(String[] tokens, int startIndex) {
        ...
        if (currentToken.charAt(currentToken.length() - 1) == ')') {
            ...
            return new PointcutBody(
                    0, currentToken.substring(bodyStart + 1, currentToken.length() - 1));
        } else {
