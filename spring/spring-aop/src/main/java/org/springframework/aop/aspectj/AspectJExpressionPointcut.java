...
/**
 * Pointcut的一个重要实现类。AspectJ语法切点类。同时实现了MethodMatcher，AspectJ语法切点的匹配在这个类中完成。
 *
 * <p>Spring {@link org.springframework.aop.Pointcut} implementation that uses the AspectJ weaver to
...
public class AspectJExpressionPointcut extends AbstractExpressionPointcut
    ...
    /**
     * Check whether this pointcut is ready to match, lazily building the underlying AspectJ
     * pointcut expression.
     */
    ...
    private PointcutExpression buildPointcutExpression(@Nullable ClassLoader classLoader) {
        // 初始化一个PointcutParser的实例PointcutParser,aspectj中提供的类
    ...
    /**
     * 初始化一个Pointcut的解析器。注册BeanPointcutDesignatorHandler，处理Spring自己支持的bean() 的切点表达式的
     *
     * <p>Initialize the underlying AspectJ pointcut parser.
     */
    ...
    /**
     * 将表达式中的 and 替换为 && or 替换为 || not 替换为 !
     *
     * <p>If a pointcut expression has been specified in XML, the user cannot write {@code and} as
     * "&&" (though &amp;&amp; will work). We also allow {@code and} between two pointcut
     * sub-expressions.
     *
    ...
    // 这是ClassFilter匹配类，借助PointcutExpression#couldMatchJoinPointsInType去匹配
    @Override
    public boolean matches(Class<?> targetClass) {
        ...
        try {
            try {
                ...
                logger.debug(
                        "PointcutExpression matching rejected target class - trying fallback expression",
                        ex);
                // Actually this is still a "maybe" - treat the pointcut as dynamic if we don't know
                // enough yet
    ...
    public boolean matches(Method method, Class<?> targetClass, Object... args) {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Could not access current invocation - matching with limited context: "
                                + ex);
            }
        }
        ...
        try {
            JoinPointMatch joinPointMatch =
                    shadowMatch.matchesJoinPoint(thisObject, targetObject, args);
        ...
        if (targetMethod.getDeclaringClass().isInterface()) {
            ...
            if (ifcs.size() > 1) {
                try {
                    ...
                    targetMethod =
                            ClassUtils.getMostSpecificMethod(targetMethod, compositeInterface);
                ...
                if (shadowMatch == null) {
                    ...
                    try {
                        try {
                            ...
                            // Failed to introspect target method, probably because it has been
                            // loaded
                            // in a special ClassLoader. Let's try the declaring ClassLoader
                            // instead...
                            try {
                                fallbackExpression =
                                ...
                                if (fallbackExpression != null) {
                                    shadowMatch =
                                            fallbackExpression.matchesMethodExecution(
                                                    methodToMatch);
                                }
                        ...
                        if (targetMethod != originalMethod
                            ...
                            // Fall back to the plain original method in case of no resolvable match
                            // or a
                            // negative match on a proxy class (which doesn't carry any annotations
                            // on its
                            // redeclared methods).
                            ...
                            try {
                                ...
                                // let's try the original method's declaring class before we give
                                // up...
                                try {
                                    fallbackExpression =
                                            getFallbackPointcutExpression(
    ...
    public boolean equals(@Nullable Object other) {
        ...
        return ObjectUtils.nullSafeEquals(this.getExpression(), otherPc.getExpression())
                ...
                && ObjectUtils.nullSafeEquals(
                        this.pointcutParameterNames, otherPc.pointcutParameterNames)
                && ObjectUtils.nullSafeEquals(
    ...
    private static class DefensiveShadowMatch implements ShadowMatch {
        ...
        @Override
        public JoinPointMatch matchesJoinPoint(
     ...
     *
     * <p>Dynamic match tests for this matcher always return true, since the matching decision is
     * made at the proxy creation time. For static match tests, this matcher abstains to allow the
     * overall pointcut to match even when negation is used with the bean() pointcut.
     */
        ...
        private FuzzyBoolean contextMatch(@Nullable Class<?> targetType) {
            ...
            if (targetType != null) {
                ...
                return FuzzyBoolean.fromBoolean(
                        matchesBean(
                                isFactory
                                        ? BeanFactory.FACTORY_BEAN_PREFIX + advisedBeanName
                                        : advisedBeanName));
            } else {
                return FuzzyBoolean.fromBoolean(
