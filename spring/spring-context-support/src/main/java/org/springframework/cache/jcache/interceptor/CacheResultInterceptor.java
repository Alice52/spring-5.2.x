...
class CacheResultInterceptor
     ...
     * @return a clone exception with a rewritten call stack composed of the current call stack up
     *     to (included) the common ancestor specified by the {@code className} and {@code
     *     methodName} arguments, followed by stack trace elements of the specified {@code
     *     exception} after the common ancestor.
     */
    private static CacheOperationInvoker.ThrowableWrapper rewriteCallStack(
        ...
        if (index == -1 || cachedIndex == -1) {
            return new CacheOperationInvoker.ThrowableWrapper(
                    exception); // Cannot find common ancestor
        }
    ...
    private Cache resolveExceptionCache(
        ...
        if (exceptionCacheResolver != null) {
            return extractFrom(
                    context.getOperation().getExceptionCacheResolver().resolveCaches(context));
        }
        return null;
    }
}
