...
public class CciTemplate implements CciOperations {
     ...
     */
    public CciTemplate(
            ConnectionFactory connectionFactory, @Nullable ConnectionSpec connectionSpec) {
        setConnectionFactory(connectionFactory);
     ...
     *
     * <p>Default is none: When no explicit output Record gets passed into an {@code execute}
     * method, CCI's {@code Interaction.execute} variant that returns an output Record will be
     * called.
     *
     * <p>Specify a RecordCreator here if you always need to call CCI's {@code Interaction.execute}
     * variant with a passed-in output Record. Unless there is an explicitly specified output
     * Record, CciTemplate will then invoke this RecordCreator to create a default output Record
     * instance.
     *
     ...
     *
     * @param connectionSpec the CCI ConnectionSpec that the derived template instance is supposed
     *     to obtain Connections for
    ...
    public <T> T execute(ConnectionCallback<T> action) throws DataAccessException {
        ...
        Connection con =
                ConnectionFactoryUtils.getConnection(connectionFactory, getConnectionSpec());
        try {
            return action.doInConnection(con, connectionFactory);
        } catch (NotSupportedException ex) {
            throw new CciOperationNotSupportedException(
    ...
    public Record execute(InteractionSpec spec, RecordCreator inputCreator)
            throws DataAccessException {
        Record output =
                doExecute(spec, createRecord(inputCreator), null, new SimpleRecordExtractor());
    ...
    @Override
    public <T> T execute(
            InteractionSpec spec, Record inputRecord, RecordExtractor<T> outputExtractor)
            throws DataAccessException {
     ...
     *
     * <p>Default implementation returns the connector's RecordFactory if available, falling back to
     * a NotSupportedRecordFactory placeholder. This allows to invoke a RecordCreator callback with
     * a non-null RecordFactory reference in any case.
     *
