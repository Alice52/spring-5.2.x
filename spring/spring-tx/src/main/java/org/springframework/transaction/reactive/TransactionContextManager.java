...
public abstract class TransactionContextManager {
     ...
     * transactional context holder. Context retrieval fails with NoTransactionException if no
     * context or context holder is registered.
     *
                        ...
                        (ctx, sink) -> {
                            ...
                            if (ctx.hasKey(TransactionContextHolder.class)) {
                                TransactionContextHolder holder =
                                        ctx.get(TransactionContextHolder.class);
     ...
     * Interaction with transactional resources through {@link TransactionSynchronizationManager}
     * requires a TransactionContext to be registered in the subscriber context.
     *
     * @return functional context registration.
     */
    ...
    public static Function<Context, Context> getOrCreateContextHolder() {
        return context -> {
            if (!context.hasKey(TransactionContextHolder.class)) {
                return context.put(
                        TransactionContextHolder.class,
                        new TransactionContextHolder(new ArrayDeque<>()));
            }
            return context;
        };
    }
