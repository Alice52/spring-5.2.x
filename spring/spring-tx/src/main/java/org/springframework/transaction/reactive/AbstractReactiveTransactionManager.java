...
public abstract class AbstractReactiveTransactionManager
                    ...
                    suspendedResourcesHolder -> {
                        ...
                        return doBegin(synchronizationManager, transaction, definition)
                                .doOnSuccess(
                                        ignore ->
                                                prepareSynchronization(
                                                        synchronizationManager, status, definition))
                                .thenReturn(status)
                                .onErrorResume(
                        ...
                        return suspendedResources
                                .map(
                                        it -> {
                                            ...
                                            synchronizationManager.setCurrentTransactionReadOnly(
                                                    false);
                                            ...
                                            synchronizationManager.setActualTransactionActive(
                                                    false);
    ...
    private Mono<Void> resumeAfterBeginException(
        ...
        return resume(synchronizationManager, transaction, suspendedResources)
                .doOnError(
                        ErrorPredicates.RUNTIME_OR_ERROR, ex -> logger.error(exMessage, beginEx));
    }
    ...
    /**
     * Reactivate transaction synchronization for the current transaction context and resume all
    ...
    public final Mono<Void> commit(ReactiveTransaction transaction) throws TransactionException {
        ...
        return TransactionSynchronizationManager.forCurrentTransaction()
                .flatMap(
                        synchronizationManager -> {
                            GenericReactiveTransaction reactiveTx =
    ...
    private Mono<Void> processCommit(
                ...
                prepareForCommit(synchronizationManager, status)
                        ...
                        .then(
                                Mono.empty()
                                        .onErrorResume(
                                                ex -> {
                                                    Mono<Object> propagateException =
                                                            Mono.error(ex);
                                                    // Store result in a local variable in order to
                                                    // appease the
                                                    // Eclipse compiler with regard to inferred
                                                    // generics.
                                                    Mono<Object> result = propagateException;
                                                    if (ErrorPredicates.UNEXPECTED_ROLLBACK.test(
                                                            ex)) {
                                                        result =
                                                    ...
                                                    } else if (ErrorPredicates.TRANSACTION_EXCEPTION
                                                            .test(ex)) {
                                                        result =
                                                                triggerAfterCompletion(
                                                                                synchronizationManager,
        ...
        return commit.onErrorResume(
                        ex ->
                                cleanupAfterCompletion(synchronizationManager, status)
                                        .then(Mono.error(ex)))
        ...
        if (status.isNewSynchronization()) {
            ...
            if (!status.hasTransaction() || status.isNewTransaction()) {
                ...
                return invokeAfterCompletion(
                        synchronizationManager, synchronizations, completionStatus);
        ...
        return Mono.defer(
                () -> {
                    ...
                    if (status.isNewTransaction()) {
                        cleanup =
                                doCleanupAfterCompletion(
                                        synchronizationManager, status.getTransaction());
                    }
                    if (status.getSuspendedResources() != null) {
                        ...
                        Object transaction =
                                (status.hasTransaction() ? status.getTransaction() : null);
                        return cleanup.then(
                                resume(
                                        synchronizationManager,
                                        transaction,
     ...
     * or as part of a DefaultReactiveTransactionStatus instance.
     *
