...
public class JtaTransactionManager extends AbstractPlatformTransactionManager
    ...
    /**
     * Fallback JNDI locations for the JTA TransactionManager. Applied if the JTA UserTransaction
     * does not implement the JTA TransactionManager interface, provided that the
     ...
     *
     * <p>Typically just used for local JTA setups; in a Java EE environment, the UserTransaction
     * will always be fetched from JNDI.
     *
     * @see #setUserTransactionName
     ...
     * example when there are known issues with suspend/resume and any attempt to use REQUIRES_NEW
     * or NOT_SUPPORTED should fail fast.
     *
     ...
     * TransactionManager API. Also, it is an official part of the Java EE 5 platform, in contrast
     * to the JTA TransactionManager itself.
     *
     * <p>Note that the TransactionSynchronizationRegistry will be autodetected in JNDI and also
     * from the UserTransaction/TransactionManager object if implemented there as well.
     *
     ...
     * JDNI location ("java:comp/TransactionSynchronizationRegistry") if the UserTransaction has
     * also been obtained from JNDI, and also whether to fall back to checking whether the JTA
     * UserTransaction/TransactionManager object implements the JTA
     * TransactionSynchronizationRegistry interface too.
     *
     * <p>Default is "true", autodetecting the TransactionSynchronizationRegistry unless it has been
     * specified explicitly. Can be turned off to delegate synchronization registration to the
     * regular JTA TransactionManager API.
     */
     ...
     *
     * <p>Default is "false", throwing an exception if a non-default isolation level is specified
     * for a transaction. Turn this flag on if affected resource adapters check the thread-bound
     ...
     *
     * <p>Called by {@code afterPropertiesSet} if no direct UserTransaction reference was set. Can
     * be overridden in subclasses to provide a different UserTransaction object.
     *
    ...
    /**
     * Allows subclasses to retrieve the JTA UserTransaction in a vendor-specific manner. Only
     * called if no "userTransaction" or "userTransactionName" specified.
     *
     ...
     * <p>The default implementation simply returns {@code null}.
     *
     * @return the JTA TransactionSynchronizationRegistry handle to use, or {@code null} if none
     *     found
     * @throws TransactionSystemException in case of errors
     */
    @Nullable
    ...
    protected UserTransaction findUserTransaction() {
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "No JTA UserTransaction found at default JNDI location [" + jndiName + "]",
                        ex);
            }
            return null;
        }
        ...
        for (String jndiName : FALLBACK_TRANSACTION_MANAGER_NAMES) {
            try {
                TransactionManager tm =
                        getJndiTemplate().lookup(jndiName, TransactionManager.class);
                if (logger.isDebugEnabled()) {
                    logger.debug(
    ...
    /**
     * Find the JTA 1.1 TransactionSynchronizationRegistry through autodetection: checking whether
     * the UserTransaction object or TransactionManager object implements it, and checking Java EE
     * 5's standard JNDI location.
     *
     * <p>The default implementation simply returns {@code null}.
     *
     * @param ut the JTA UserTransaction object
        ...
        if (this.userTransactionObtainedFromJndi) {
            ...
            try {
                TransactionSynchronizationRegistry tsr =
                        getJndiTemplate()
                                .lookup(jndiName, TransactionSynchronizationRegistry.class);
                if (logger.isDebugEnabled()) {
                    logger.debug(
     ...
     *
     * <p>The UserTransaction object will either be looked up freshly for the current transaction,
     * or the cached one looked up at startup will be used. The latter is the default: Most
     * application servers use a shared singleton UserTransaction that can be cached. Turn off the
     ...
     *
     * <p>Subclasses can override this to provide a JtaTransactionObject subclass, for example
     * holding some additional JTA handle needed.
     *
    ...
    /**
     * Apply the given transaction isolation level. The default implementation will throw an
     * exception for any level other than ISOLATION_DEFAULT.
     *
    ...
    protected void doResume(@Nullable Object transaction, Object suspendedResources) {
        ...
        try {
            ...
            throw new IllegalTransactionStateException(
                    "Tried to resume invalid JTA transaction", ex);
        ...
        try {
            ...
            if (jtaStatus == Status.STATUS_ROLLEDBACK) {
                ...
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Rollback failure with transaction already marked as rolled back: "
                                        + ex);
                    }
                }
                throw new UnexpectedRollbackException(
            ...
            if (jtaStatus != Status.STATUS_NO_TRANSACTION) {
                try {
                    ...
                    if (jtaStatus == Status.STATUS_ROLLEDBACK) {
                        ...
                        throw new TransactionSystemException(
                                "Unexpected internal transaction state", ex);
                    }
                }
            }
        } catch (SystemException ex) {
