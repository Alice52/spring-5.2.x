 ...
 *
 * <p>A transaction aspect is serializable if its {@code TransactionManager} and {@code
...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
     ...
     *
     * <p>This can either point to a traditional {@link PlatformTransactionManager} or a {@link
     ...
     *
     * <p>This can either be a traditional {@link PlatformTransactionManager} or a {@link
     * ReactiveTransactionManager} for reactive transaction management.
     */
    @Nullable
     ...
     * for a given transaction or if an explicit name for the default transaction manager bean has
     * not been specified.
     *
     ...
     *
     * <p>Internally, a NameMatchTransactionAttributeSource will be created from the given
     * properties.
     *
     * @see #setTransactionAttributeSource
     ...
     * well as regular {@link PlatformTransactionManager} implementations and {@link
     * ReactiveTransactionManager} implementations for reactive return types.
     *
    ...
    protected Object invokeWithinTransaction(
        ...
        if (this.reactiveAdapterRegistry != null && tm instanceof ReactiveTransactionManager) {
            ReactiveTransactionSupport txSupport =
                    this.transactionSupportCache.computeIfAbsent(
                            method,
                            key -> {
        ...
        if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
            ...
            try {
                result =
                        ((CallbackPreferringPlatformTransactionManager) ptm)
                                .execute(
                                        txAttr,
                                        status -> {
                                            TransactionInfo txInfo =
                                            ...
                                            try {
                                                ...
                                                if (retVal != null
                                                    ...
                                                    retVal =
                                                            VavrDelegate.evaluateTryFailure(
     ...
     *     hasTransaction()} method on TransactionInfo can be used to tell if there was a
     *     transaction created.
     ...
     *
     * <p>Prepare a TransactionInfo for the given attribute and status object.
     *
     * @param txAttr the TransactionAttribute (may be {@code null})
     * @param joinpointIdentification the fully qualified method name (used for monitoring and
     *     logging purposes)
     ...
     * 调用事务管理器的提交方法
     *
     * <p>Execute after successful completion of call, but not after an exception was handled. Do
     ...
     *
     * <p>Handle a throwable, completing the transaction. We may commit or roll back, depending on
     * the configuration.
     *
     * @param txInfo information about the current transaction
     ...
     * 清除事务信息
     *
     * <p>Reset the TransactionInfo ThreadLocal.
     *
    ...
    private class ReactiveTransactionSupport {
                        ...
                        .flatMap(
                                context ->
                                        createTransactionIfNecessary(
                                                        rtm, txAttr, joinpointIdentification)
                                                .flatMap(
                                                        it -> {
                                                            try {
                                                                // Need re-wrapping until we get
                                                                // hold of the exception through
                                                                // usingWhen.
                                                                return Mono
                                                                        .<Object,
                                                                ...
                                                                // target invocation exception
                                                                return completeTransactionAfterThrowing(
                                                                                it, ex)
                                                                        .then(Mono.error(ex));
                                                            }
                                                        }))
        ...
        private Mono<ReactiveTransactionInfo> createTransactionIfNecessary(
            ...
            Mono<ReactiveTransaction> tx =
                    (attrToUse != null ? tm.getReactiveTransaction(attrToUse) : Mono.empty());
        ...
        private Mono<Void> completeTransactionAfterThrowing(
            ...
            if (txInfo != null && txInfo.getReactiveTransaction() != null) {
                ...
                // 这里判断是否回滚，默认的依据是抛出的异常是否是RuntimeException或者error类型
                if (txInfo.transactionAttribute != null
                        && txInfo.transactionAttribute.rollbackOn(ex)) {
                    // 根据TransactionStatue信息进行回滚处理
                    return txInfo.getTransactionManager()
