...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
    ...
    /** 配置文件的地址 Explicit context config location. */
    ...
    /**
     * 标记是否接收到ContextRefreshedEvent Flag used to detect whether onRefresh has already been called.
     */
    private volatile boolean refreshEventReceived = false;

    /**
     * Create a new {@code FrameworkServlet} that will create its own internal web application
     ...
     * ConfigurableWebApplicationContext} and (b) has <strong>not</strong> already been refreshed
     * (the recommended approach), then the following will occur:
     *
     * <ul>
     ...
     * ConfigurableWebApplicationContext}, none of the above will occur under the assumption that
     * the user has performed these actions (or not) per his or her specific needs.
     *
    ...
    /**
     * Specify which {@link ApplicationContextInitializer} instances should be used to initialize
     * the application context used by this {@code FrameworkServlet}.
     *
     * @see #configureAndRefreshWebApplicationContext
     ...
     *
     * <p>This is especially handy during testing, although it is debatable whether it's good
    ...
    /**
     * Set whether to expose the LocaleContext and RequestAttributes as inheritable for child
     * threads (using an {@link java.lang.InheritableThreadLocal}).
     *
     ...
     * reuse of the thread).
     *
     * <p><b>WARNING:</b> Do not use inheritance for child threads if you are accessing a thread
     * pool which is configured to potentially add new threads on demand (e.g. a JDK {@link
     * java.util.concurrent.ThreadPoolExecutor}), since this will expose the inherited context to
     * such a pooled thread.
     */
     ...
     * those requests; make sure that those endpoints are actually able to handle an OPTIONS
     * request.
     *
     ...
     * method.
     *
     * <p>Default is "false", applying {@link javax.servlet.http.HttpServlet}'s default behavior
     * (i.e. reflecting the message received back to the client).
     *
     * <p>Turn this flag on if you prefer TRACE requests to go through the regular dispatching
     * chain, just like other HTTP requests. This usually means that your controllers will receive
     * those requests; make sure that those endpoints are actually able to handle a TRACE request.
     *
     * <p>Note that HttpServlet's default TRACE processing will be applied in any case if your
     * controllers happen to not generate a response of content type 'message/http' (as required for
     * a TRACE response).
     */
    ...
    /**
     * Called by Spring via {@link ApplicationContextAware} to inject the current application
     * context. This method allows FrameworkServlets to be registered as Spring beans inside an
     * existing {@link WebApplicationContext} rather than {@link #findWebApplicationContext()
     * finding} a {@link org.springframework.web.context.ContextLoaderListener bootstrapped}
     * context.
     *
    ...
    /**
     * Overridden method of {@link HttpServletBean}, invoked after any bean properties have been
     * set. Creates this servlet's WebApplicationContext.
     */
    @Override
     ...
     *
     * <p>Initialize and publish the WebApplicationContext for this servlet.
     *
     * <p>Delegates to {@link #createWebApplicationContext} for actual creation of the context. Can
     ...
     * org.springframework.web.context.support.XmlWebApplicationContext} or a {@link
     * #setContextClass custom context class}, if set.
     *
     * <p>This implementation expects custom contexts to implement the {@link
     * org.springframework.web.context.ConfigurableWebApplicationContext} interface. Can be
     ...
     * Instantiate the WebApplicationContext for this servlet, either a default {@link
     * org.springframework.web.context.support.XmlWebApplicationContext} or a {@link
     * #setContextClass custom context class}, if set. Delegates to
    ...
    private ApplicationContextInitializer<ConfigurableApplicationContext> loadInitializer(
        ...
        try {
            ...
            throw new ApplicationContextException(
                    String.format(
                            "Could not load class [%s] specified "
    ...
    public void refresh() {
        ...
        if (!(wac instanceof ConfigurableApplicationContext)) {
            throw new IllegalStateException(
                    "WebApplicationContext does not support refresh: " + wac);
        }
     ...
     *
     * <p>The default implementation calls {@link #onRefresh}, triggering a refresh of this
                ...
                new HttpServletResponseWrapper(response) {
                    ...
                    public void setHeader(String name, String value) {
                        if ("Allow".equals(name)) {
                            value =
                                    (StringUtils.hasLength(value) ? value + ", " : "")
     ...
     * @return the ServletRequestAttributes to bind, or {@code null} to preserve the previously
     *     bound instance (or not binding any, if none bound before)
     ...
     *
     * <p>The default implementation takes the name of the UserPrincipal, if any. Can be overridden
     * in subclasses.
     *
     * @param request current HTTP request
