...
/**
 * 使用url做匹配 A logical disjunction (' || ') request condition that matches a request against a set of
 * URL path patterns.
 *
...
public class PatternsRequestCondition extends AbstractRequestCondition<PatternsRequestCondition> {
    ...
    /**
     * Creates a new instance with the given URL patterns. Each pattern that is not empty and does
     * not start with "/" is prepended with "/".
     *
    ...
    /**
     * Alternative constructor with additional, optional {@link UrlPathHelper}, {@link PathMatcher},
     * and whether to automatically match trailing slashes.
     *
     ...
     * @param useTrailingSlashMatch whether to match irrespective of a trailing slash
     * @deprecated as of 5.2.4. See class-level note in {@link
     *     org.springframework.web.servlet.mvc.method.annotation.RequestMappingHandlerMapping} on
     ...
     *
     * <ul>
     *   <li>If there are patterns in both instances, combine the patterns in "this" with the
     *       patterns in "other" using {@link PathMatcher#combine(String, String)}.
     ...
     * @return the same instance if the condition contains no patterns; or a new condition with
     *     sorted matching patterns; or {@code null} if no patterns match.
     */
    @Override
    @Nullable
    ...
    /**
     * Compare the two conditions based on the URL patterns they contain. Patterns are compared one
     * at a time, from top to bottom via {@link PathMatcher#getPatternComparator(String)}. If all
     * compared patterns match equally, but one instance has more patterns, it is considered a
     * closer match.
     *
     * <p>It is assumed that both instances have been obtained via {@link
     * #getMatchingCondition(HttpServletRequest)} to ensure they contain only patterns that match
     * the request and are sorted with the best matches on top.
     */
    @Override
