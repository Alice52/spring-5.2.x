...
/**
 * 继承自InvocableHandlerMethod类，能够通过已经注册的HandlerMethodReturnValueHandler来处理返回值并且支持设置响应状态以及ResponseStatus注解
 *
 * <p>Extends {@link InvocableHandlerMethod} with the ability to handle return values through a
...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
    ...
    ServletInvocableHandlerMethod wrapConcurrentResult(Object result) {
        return new ConcurrentResultHandlerMethod(
                result, new ConcurrentResultMethodParameter(result));
    }

    /**
     * A nested subclass of {@code ServletInvocableHandlerMethod} that uses a simple {@link
     * Callable} instead of the original controller as the handler in order to return the fixed
     * (concurrent) result value given to it. Effectively "resumes" processing with the
    ...
    private class ConcurrentResultHandlerMethod extends ServletInvocableHandlerMethod {
         ...
         * Foo instead of {@code DeferredResult<Foo>}.
         */
        @Override
        ...
        public ConcurrentResultMethodParameter(Object returnValue) {
            ...
            this.returnType =
                    (returnValue instanceof ReactiveTypeHandler.CollectedValuesList
                            ? ((ReactiveTypeHandler.CollectedValuesList) returnValue)
                                    .getReturnType()
        ...
        public <T extends Annotation> boolean hasMethodAnnotation(Class<T> annotationType) {
            ...
            return (super.hasMethodAnnotation(annotationType)
                    || (annotationType == ResponseBody.class
                            && this.returnValue
                                    instanceof ReactiveTypeHandler.CollectedValuesList));
        }
