...
public abstract class AbstractDataBoundFormElementTag extends AbstractFormTag
    ...
    /**
     * Get the {@link #evaluate resolved} property path for the {@link FormTag#setModelAttribute
     * form object}.
     */
    ...
    /**
     * Set the property path from the {@link FormTag#setModelAttribute form object}. May be a
     * runtime expression.
     */
     ...
     *
     * <p>Concrete sub-classes should call this method when/if they want to render default
     * attributes.
     *
     ...
     * property path as the name. For the most part this is desirable as it links with the
     * server-side expectation for data binding. However, some subclasses may wish to change the
     * value of the '{@code name}' attribute without changing the bind path.
     *
    ...
    /**
     * Get a display String for the given value, converted by a PropertyEditor that the BindStatus
     * may have registered for the value's Class.
     */
    protected String convertToDisplayString(@Nullable Object value) throws JspException {
        PropertyEditor editor =
                (value != null ? getBindStatus().findEditor(value.getClass()) : null);
    ...
    protected final String processFieldValue(@Nullable String name, String value, String type) {
        ...
        if (processor != null && request instanceof HttpServletRequest) {
            value =
                    processor.processFormFieldValue(
                            (HttpServletRequest) request, name, value, type);
        }
        return value;
    }
