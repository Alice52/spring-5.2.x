...
public class WebMvcConfigurationSupportExtensionTests {
    ...
    public void handlerMappings() throws Exception {
        ...
        assertThat(chain.getInterceptors()[0].getClass().getSimpleName())
                .isEqualTo("CorsInterceptor");
        ...
        RequestMappingInfo info =
                map.entrySet().stream()
                        .filter(
                                entry ->
                                        entry.getValue().getBeanType().equals(UserController.class))
                        .findFirst()
    ...
    public void requestMappingHandlerAdapter() throws Exception {
        ...
        assertThat(converters.get(1).getClass())
        ...
        assertThat(objectMapper.getDeserializationConfig().isEnabled(DEFAULT_VIEW_INCLUSION))
                .isFalse();
        assertThat(objectMapper.getSerializationConfig().isEnabled(DEFAULT_VIEW_INCLUSION))
                .isFalse();
    ...
    /**
     * Since WebMvcConfigurationSupport does not implement WebMvcConfigurer, the purpose of this
     * test class is also to ensure the two are in sync with each other. Effectively that ensures
     * that application config classes that use the combo {@code @EnableWebMvc} plus
     * WebMvcConfigurer can switch to extending WebMvcConfigurationSupport directly for more
