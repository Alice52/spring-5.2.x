package top.hubby.bridge;

/**
 * 在此方法中只声明了一个方法，但是从字节码中可以看到三个方法， 第一个是无参的构造方法，编译器自动生成 第二个是实现接口中的方法
 * 第三个是编译器自动生成的桥接方法，可以看到那两个标志，同时方法的参数和返回值都是Object类型
 *
 * <p>在声明SuperClass类型的变量时，不指定泛型类型，那么在方法调用时就可以传任何类型的参数，因为SuperClass中的方法参数实际上是Object类型，而且
 * 编译器也不能够发现错误，但是当运行的时候就会发现参数类型不是Subclass声明的类型，会抛出类型转换异常，因为此时调用的是桥接方法，而在桥接方法中会 进行类型强制转换，所以会抛出此异常
 *
 * <p>如果在声明的时候直接就指定好了泛型的类型，那么编译的时候就会出现异常情况，这样的话就会把错误提前
 *
 * <p>为什么要生成桥接方法呢？ 在1.5版本之前创建一个集合对象之后，可以向其中放置任何类型的元素，无法确定和限制具体的类型，而引入泛型之后可以指定存放什么类型的数据，
