...
public abstract class AbstractEntityManagerFactoryBean
     ...
     * EntityManagerFactory. If not specified, the persistence provider will be taken from the
     * JpaVendorAdapter (if any) or retrieved through scanning (as far as possible).
     *
     ...
     *
     * <p>Default is none, indicating the default EntityManagerFactory configuration. The
     * persistence provider will throw an exception if ambiguous EntityManager configurations are
     * found.
     *
     ...
     * any).
     *
     * <p>Can be populated with a String "value" (parsed via PropertiesEditor) or a "props" element
     ...
     *
     * <p>The default will be taken from the specific JpaVendorAdapter, if any, or set to the
     * standard {@code javax.persistence.EntityManagerFactory} interface else.
     *
     * @see JpaVendorAdapter#getEntityManagerFactoryInterface()
     */
    public void setEntityManagerFactoryInterface(
            Class<? extends EntityManagerFactory> emfInterface) {
     ...
     * initialize appropriate defaults for the given provider, such as persistence provider class
     * and JpaDialect, unless locally overridden in this FactoryBean.
     */
     ...
     *
     * <p>{@code EntityManagerFactory} initialization will then switch into background bootstrap
     * mode, with a {@code EntityManagerFactory} proxy immediately returned for injection purposes
     * instead of waiting for the JPA provider's bootstrapping to complete. However, note that the
     * first actual call to a {@code EntityManagerFactory} method will then block until the JPA
     * provider's bootstrapping completed, if not ready by then. For maximum benefit, make sure to
     * avoid early {@code EntityManagerFactory} calls in init methods of related beans, even for
    ...
    public void afterPropertiesSet() throws PersistenceException {
        ...
        if (jpaVendorAdapter != null) {
            ...
            if (this.entityManagerFactoryInterface == null) {
                ...
                if (!ClassUtils.isVisible(
                        this.entityManagerFactoryInterface, this.beanClassLoader)) {
                ...
                if (cause != null) {
                    ...
                    if (!message.endsWith(causeString)) {
                        throw new PersistenceException(
                                message + "; nested exception is " + causeString, cause);
                    }
                }
            }
            throw ex;
    ...
    protected EntityManagerFactory createEntityManagerFactoryProxy(
        ...
        if (entityManagerFactoryInterface != null) {
            ...
            ifcs.addAll(
                    ClassUtils.getAllInterfacesForClassAsSet(emf.getClass(), this.beanClassLoader));
        } else {
    ...
    /**
     * Delegate an incoming invocation from the proxy, dispatching to EntityManagerFactoryInfo or
     * the native EntityManagerFactory accordingly.
     */
    Object invokeProxyMethod(Method method, @Nullable Object[] args) throws Throwable {
        if (method.getDeclaringClass().isAssignableFrom(EntityManagerFactoryInfo.class)) {
            ...
            // Redirect to plain createEntityManager and add synchronization semantics through
            // Spring proxy
            EntityManager rawEntityManager =
                    (args.length > 1
                            ? getNativeEntityManagerFactory()
                                    .createEntityManager((Map<?, ?>) args[1])
        ...
        // Standard delegation to the native factory, just post-processing EntityManager return
        // values
    ...
    /**
     * Subclasses must implement this method to create the EntityManagerFactory that will be
     * returned by the {@code getObject()} method.
     *
        ...
        if (this.nativeEntityManagerFactory != null) {
            ...
            try {
                ...
                if (cause instanceof PersistenceException) {
                    // Rethrow a provider configuration exception (possibly with a nested cause)
                    // directly
                    throw (PersistenceException) cause;
                }
                throw new IllegalStateException(
                        "Failed to asynchronously initialize native EntityManagerFactory: "
                                + ex.getMessage(),
                        cause);
            }
        }
    ...
    private static class SerializedEntityManagerFactoryBeanReference implements Serializable {
        ...
        public SerializedEntityManagerFactoryBeanReference(
        ...
        private Object readResolve() {
            return this.beanFactory.getBean(
                    this.lookupName, AbstractEntityManagerFactoryBean.class);
        }
    }

    /**
     * Dynamic proxy invocation handler for an {@link EntityManagerFactory}, returning a proxy
     * {@link EntityManager} (if necessary) from {@code createEntityManager} methods.
     */
    @SuppressWarnings("serial")
