...
public class JmsTemplate extends JmsDestinationAccessor implements JmsOperations {
     ...
     *
     * <p>Note: The ConnectionFactory has to be set before using the instance. This constructor can
     * be used to prepare a JmsTemplate via a BeanFactory, typically setting the ConnectionFactory
     * via setConnectionFactory.
     *
     * @see #setConnectionFactory
     */
    public JmsTemplate() {
    ...
    public String getDefaultDestinationName() {
        return (this.defaultDestination instanceof String
                ? (String) this.defaultDestination
                : null);
    }
     ...
     *
     * <p>The default is {@link #RECEIVE_TIMEOUT_INDEFINITE_WAIT}, which indicates a blocking
    ...
    /**
     * If "true", then the values of deliveryMode, priority, and timeToLive will be used when
     * sending a message. Otherwise, the default values, that may be set administratively, will be
     * used.
     *
    ...
    /**
     * Set whether message delivery should be persistent or non-persistent, specified as boolean
     * value ("true" or "false"). This will set the delivery mode accordingly, to either
     * "PERSISTENT" (2) or "NON_PERSISTENT" (1).
     *
     ...
     * just necessary for receiving messages, which is preferably achieved through the {@code
     * receive} methods.
     *
    ...
    public Message receiveSelected(
            final Destination destination, @Nullable final String messageSelector)
            throws JmsException {
    ...
    @Nullable
    public Message receiveSelected(
            final String destinationName, @Nullable final String messageSelector)
            throws JmsException {
        return execute(
                session -> {
                    Destination destination = resolveDestinationName(session, destinationName);
    ...
    protected Message doReceive(Session session, MessageConsumer consumer) throws JMSException {
        try {
            ...
            if (connectionFactory != null) {
                resourceHolder =
                        (JmsResourceHolder)
                                TransactionSynchronizationManager.getResource(connectionFactory);
            }
    ...
    public Message sendAndReceive(
            final Destination destination, final MessageCreator messageCreator)
            throws JmsException {
        return executeLocal(
                session -> doSendAndReceive(session, destination, messageCreator), true);
    }
    ...
    protected Message doSendAndReceive(
            Session session, Destination destination, MessageCreator messageCreator)
            throws JMSException {
     ...
     * non-transactional {@link Session}. The given {@link SessionCallback} does not participate in
