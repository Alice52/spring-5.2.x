        ...
        if (!this.sessions.contains(session)) {
            ...
            if (connection != null) {
                LinkedList<Session> sessions =
                        this.sessionsPerConnection.computeIfAbsent(
                                connection, k -> new LinkedList<>());
                ...
                if (this.connectionFactory != null) {
                    try {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "No working getDataSource method found on ConnectionFactory: "
                                            + ex2);
                        }
                        // No working getDataSource method - cannot perform DataSource transaction
                        // check
                    }
                }
                throw ex;
