...
public abstract class AbstractAdaptableMessageListener
    ...
    /**
     * Set the DestinationResolver that should be used to resolve response destination names for
     * this adapter.
     *
     * <p>The default resolver is a DynamicDestinationResolver. Specify a JndiDestinationResolver
    ...
    /**
     * Return the converter that will convert incoming JMS messages to listener method arguments,
     * and objects returned from listener methods back to JMS messages.
     */
    @Nullable
    ...
    /**
     * Return the {@link QosSettings} to use when sending a response, or {@code null} if the
        ...
        if (session != null) {
            ...
            try {
                ...
                Destination destination =
                        getResponseDestination(request, response, session, result);
                sendResponse(session, destination, response);
            } catch (Exception ex) {
                throw new ReplyFailureException(
                        "Failed to send reply with payload [" + result + "]", ex);
            }
        } else {
    ...
    protected Message buildMessage(Session session, Object result) throws JMSException {
        Object content =
                preProcessResponse(
                        result instanceof JmsResponse
                                ? ((JmsResponse<?>) result).getResponse()
                                : result);
    ...
    private Destination getResponseDestination(
        ...
        if (result instanceof JmsResponse) {
            ...
            Destination destination =
                    jmsResponse.resolveDestination(getDestinationResolver(), session);
            if (destination != null) {
                return destination;
            }
        }
     ...
     *
     * <p>The default implementation first checks the JMS Reply-To {@link Destination} of the
     * supplied request; if that is not {@code null} it is returned; if it is {@code null}, then the
     * configured {@link #resolveDefaultResponseDestination default response destination} is
     * returned; if this too is {@code null}, then an {@link javax.jms.InvalidDestinationException}
     * is thrown.
     *
     * @param request the original incoming JMS message
     * @param response the outgoing JMS message about to be sent
    ...
    protected Destination resolveDefaultResponseDestination(Session session) throws JMSException {
        ...
        if (this.defaultResponseDestination instanceof DestinationNameHolder) {
            DestinationNameHolder nameHolder =
                    (DestinationNameHolder) this.defaultResponseDestination;
    ...
    private class MessagingMessageConverterAdapter extends MessagingMessageConverter {
        ...
        protected Message createMessageForPayload(
                Object payload, Session session, @Nullable Object conversionHint)
                throws JMSException {

            MessageConverter converter = getMessageConverter();
            if (converter == null) {
                throw new IllegalStateException(
                        "No message converter, cannot handle '" + payload + "'");
            }
            if (converter instanceof SmartMessageConverter) {
                return ((SmartMessageConverter) converter)
                        .toMessage(payload, session, conversionHint);
            }
