...
public abstract class AbstractPollingMessageListenerContainer
     ...
     *
     * <p>Default is none, not performing any transactional wrapping. If specified, this will
     * usually be a Spring {@link org.springframework.transaction.jta.JtaTransactionManager} or one
     * of its subclasses, in combination with a JTA-aware ConnectionFactory that this message
     ...
     * #setSessionTransacted "sessionTransacted"} flag to "true" in order to use a locally
     * transacted JMS Session for the entire receive processing, including any Session operations
     * performed by a {@link SessionAwareMessageListener} (e.g. sending a response message). This
     * allows for fully synchronized Spring transactions based on local JMS transactions, similar to
     * what {@link org.springframework.jms.connection.JmsTransactionManager} provides. Check {@link
     * AbstractMessageListenerContainer}'s javadoc for a discussion of transaction choices and
    ...
    /**
     * Set the timeout to use for receive calls, in <b>milliseconds</b>. The default is 1000 ms,
     * that is, 1 second.
     *
     ...
     * discouraged since such a listener container cannot cleanly shut down. A negative value such
     * as -1 indicates a no-wait receive operation.
     *
    ...
    public void initialize() {
        ...
        if (!this.sessionTransactedCalled
                ...
                && !TransactionSynchronizationUtils.sameResourceFactory(
                        (ResourceTransactionManager) this.transactionManager,
    ...
    /**
     * Template method that gets called when <i>no</i> message has been received, before returning
     * to the receive loop again. Allows subclasses to react to the event of no incoming message,
     * for example marking the invoker as idle.
     *
     * @param invoker the invoker object (passed through)
     * @param session the receiving JMS Session
     */
        ...
        public Connection createConnection() throws JMSException {
            if (AbstractPollingMessageListenerContainer.this.sharedConnectionEnabled()) {
                Connection sharedCon =
                        AbstractPollingMessageListenerContainer.this.getSharedConnection();
