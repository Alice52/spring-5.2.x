 ...
 * 代表解析构造函数和工厂方法
 *
 * <p>Delegate for resolving constructors and factory methods.
 *
...
class ConstructorResolver {
     ...
     *
     * <p>Marker for autowired arguments in a cached argument array, to be later replaced by a
            ...
            if (argsToResolve != null) {
                ...
                argsToUse =
                        resolvePreparedArguments(
                                beanName, mbd, bw, constructorToUse, argsToResolve, true);
            }
        }
        ...
        if (constructorToUse == null || argsToUse == null) {
            ...
            if (candidates.length == 1
                ...
                if (uniqueCandidate.getParameterCount() == 0) {
                    // 使用mdb的构造函数字段的通用锁【{@link
                    // RootBeanDefinition#constructorArgumentLock}】进行加锁以保证线程安全
                    synchronized (mbd.constructorArgumentLock) {
                        // 让mbd缓存已解析的构造函数或工厂方法
            ...
            /**
             * 没有传入参与构造函数参数列表的参数时，对构造函数缓存到BeanDefinition中 1、缓存BeanDefinition进行实例化时使用的构造函数
             * 2、缓存BeanDefinition代表的Bean的构造函数已解析完标识 3、缓存参与构造函数参数列表值的参数列表
             */
     ...
     *
     * <p>Resolve the factory method in the specified bean definition, if possible. {@link
    ...
    /**
     * 根据mbd的是否允许访问非公共构造函数和方法标记【{@link
     * RootBeanDefinition#isNonPublicAccessAllowed}】来获取factoryClass的所有候选方法
     *
     * <p>Retrieve all candidate methods for the given class, considering the {@link
     ...
     * specified in the RootBeanDefinition (the method may be overloaded) and trying to match with
     * the parameters. We don't have the types attached to constructor args, so trial and error is
     * the only way to go here. The explicitArgs array may contain argument values passed in
    ...
    public BeanWrapper instantiateUsingFactoryMethod(
        ...
        if (explicitArgs != null) {
            ...
            if (argsToResolve != null) {
                ...
                argsToUse =
                        resolvePreparedArguments(
                                beanName, mbd, bw, factoryMethodToUse, argsToResolve, true);
            }
        }
        ...
        if (factoryMethodToUse == null || argsToUse == null) {
            ...
            if (candidates.size() == 1
                ...
                if (uniqueCandidate.getParameterCount() == 0) {
                    ...
                    // RootBeanDefinition#constructorArgumentLock}】进行加锁以保证线程安全
                    synchronized (mbd.constructorArgumentLock) {
                        // 让mbd缓存已解析的构造函数或工厂方法【{@link
                        // RootBeanDefinition#resolvedConstructorOrFactoryMethod}】
                        mbd.resolvedConstructorOrFactoryMethod = uniqueCandidate;
                        // 让mbd标记构造函数参数已解析【{@link RootBeanDefinition#constructorArgumentsResolved}】
                        mbd.constructorArgumentsResolved = true;
                        // 让mbd缓存完全解析的构造函数参数【{@link
                        // RootBeanDefinition#resolvedConstructorArguments}】
                        mbd.resolvedConstructorArguments = EMPTY_ARGS;
                    }
            ...
            if (factoryMethodToUse == null || argsToUse == null) {
                ...
                if (causes != null) {
                    ...
                    // 遍历causes,元素为cause
                    for (Exception cause : causes) {
                        // 将cause添加到该Bean工厂的抑制异常列表【{@link
                        // DefaultSingletonBeanRegistry#suppressedExceptions】中
                        this.beanFactory.onSuppressedException(cause);
                    }
                    // 重新抛出ex
                    throw ex;
            ...
            if (explicitArgs == null && argsHolderToUse != null) {
                // 让mbd的唯一方法候选【{@link
                // RootBeanDefinition#factoryMethodToIntrospect}】引用factoryMethodToUse
     ...
     *
     * <p>Resolve the constructor arguments for this bean into the resolvedValues object. This may
     ...
     *
     * <p>Create an array of arguments to invoke a constructor or factory method, given the resolved
    ...
    private ArgumentsHolder createArgumentArray(
        ...
        for (int paramIndex = 0; paramIndex < paramTypes.length; paramIndex++) {
            ...
            if (resolvedValues != null) {
                ...
                valueHolder =
                        resolvedValues.getArgumentValue(
                ...
                // 如果valueHolder为null 且 (mbd不支持使用构造函数进行自动注入 或者
                // paramTypes数组长度与resolvedValues的(索引参数值+泛型参数值)数量相等)
                if (valueHolder == null
                        && (!autowiring
                                || paramTypes.length == resolvedValues.getArgumentCount())) {
                    // 在resovledValues中查找任意，不按名称匹配参数值的下一个泛型参数值，而忽略usedValueHolders参数值
                    valueHolder =
                            resolvedValues.getGenericArgumentValue(null, null, usedValueHolders);
                }
            }
            // 如果valueHolder不为null
            if (valueHolder != null) {
                ...
                if (valueHolder.isConverted()) {
                    ...
                    try {
                        ...
                        convertedValue =
                                converter.convertIfNecessary(originalValue, paramType, methodParam);
                    }
                    // 捕捉在转换类型时出现的类型不匹配异常
     ...
     * 解析缓存在mbd中准备好的参数值:
     *
     * <p>Resolve the prepared arguments stored in the given bean definition.
     */
    private Object[] resolvePreparedArguments(
        ...
        for (int argIndex = 0; argIndex < argsToResolve.length; argIndex++) {
            ...
            if (argValue == autowiredArgumentMarker) {
                ...
                argValue =
                        resolveAutowiredArgument(methodParam, beanName, null, converter, fallback);
            }
            ...
            try {
                ...
                resolvedArgs[argIndex] =
                        converter.convertIfNecessary(argValue, paramType, methodParam);
            }
            // 捕捉转换类型时抛出的类型不匹配异常
            catch (TypeMismatchException ex) {
                throw new UnsatisfiedDependencyException(
    ...
    /**
     * 解析应该自动装配的指定参数的Bean对象 Template method for resolving the specified argument which is supposed
     * to be autowired.
     */
    @Nullable
     ...
     * 私有内部类，用于保存参数组合
     *
     * <p>Private inner class for holding argument combinations.
     */
    private static class ArgumentsHolder {
        ...
        /**
         * 获取类型差异权重，宽容模式下使用 1、获取表示paramTypes和arguments之间的类层次结构差异的权重【变量 typeDiffWeight】
        ...
        public void storeCache(RootBeanDefinition mbd, Executable constructorOrFactoryMethod) {
            ...
            synchronized (mbd.constructorArgumentLock) {
                // 让mbd的已解析的构造函数或工厂方法【{@link
                // RootBeanDefinition#resolvedConstructorOrFactoryMethod}】引用constructorOrFactoryMethod
                ...
                if (this.resolveNecessary) {
                    // 让mbd的缓存部分准备好的构造函数参数值属性【{@link
                    // RootBeanDefinition#preparedConstructorArguments}】引用preparedArguments
                    mbd.preparedConstructorArguments = this.preparedArguments;
                } else {
                    // 让mbd的缓存完全解析的构造函数参数属性【{@link
                    // RootBeanDefinition#resolvedConstructorArguments}】引用arguments
     ...
     *
     * <p>Delegate for checking Java 6's {@link ConstructorProperties} annotation.
     */
