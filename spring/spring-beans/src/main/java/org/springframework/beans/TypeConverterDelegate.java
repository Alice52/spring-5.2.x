...
class TypeConverterDelegate {
     ...
     *
     * <p>Create a new TypeConverterDelegate for the given editor registry.
     *
     ...
     *
     * <p>Create a new TypeConverterDelegate for the given editor registry and bean instance.
     *
     * @param propertyEditorRegistry the editor registry to use
     ...
     * 将该值转换为指定属性所需的类型
     *
     * <p>Convert the value to the required type for the specified property.
     *
     ...
     *
     * <p>Convert the value to the required type (if necessary from a String), for the specified
     * property.
     *
     * @param propertyName name of the property
     * @param oldValue the previous value, if available (may be {@code null})
    ...
    public <T> T convertIfNecessary(
        ...
        if (editor == null
            ...
            if (conversionService.canConvert(sourceTypeDesc, typeDescriptor)) {
                try {
                    // 从conversionService 中找到
                    // sourceTypeDesc,typeDesriptor对于的转换器进行对newValue的转换成符合typeDesciptor类型的对象，并返回出去
        ...
        if (requiredType != null) {
            ...
            if (convertedValue != null) {
                ...
                // 如果requiredType是数组
                    ...
                    return (T)
                            convertToTypedArray(
                                    convertedValue, propertyName, requiredType.getComponentType());
                }
                ...
                // 如果convertedValue是String类型&& convertedValue不是requiredType类型
                    ...
                    if (conversionAttemptEx == null
                        ...
                        try {
                            ...
                            if (logger.isTraceEnabled()) {
                                logger.trace(
                                        "No String constructor found on type ["
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "Construction via String failed for type ["
                                                + requiredType.getName()
                                                + "]",
                                        ex);
                            }
                    ...
                    convertedValue =
                            attemptToConvertStringToEnum(
                                    requiredType, trimmedValue, convertedValue);
                    // 更新standardConversion标记
                    standardConversion = true;
                }
     ...
     * 找到给定类型的默认编辑器
     *
     * <p>Find a default editor for the given type.
     *
     ...
     *
     * <p>Convert the value to the required type (if necessary from a String), using the given
     * property editor.
     *
     * @param oldValue the previous value, if available (may be {@code null})
     * @param newValue the proposed new value
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "PropertyEditor ["
                                    ...
                                    + "] does not support setValue call",
                            ex);
                }
                // Swallow and proceed.
            }
     ...
     * 使用给定属性编辑器转换给定的文本值
     *
     * <p>Convert the given text value using the given property editor.
     *
     * @param oldValue the previous value, if available (may be {@code null})
    ...
    private Object convertToTypedArray(
        ...
        // 如果input是数组类型
            ...
            if (componentType.equals(input.getClass().getComponentType())
                    && !this.propertyEditorRegistry.hasCustomEditorForElement(
        ...
        while (it.hasNext()) {
            ...
            Object convertedKey =
                    convertIfNecessary(
                            ...
                            null,
                            key,
                            (keyType != null ? keyType.getType() : null),
                            keyType);
