 ...
 *
 * <p>共享bean实例的通用注册表，实现了SingletonBeanRegistry。允许注册单例实例， 该实例应该为注册中心的所有调用者共享，并通过bean名称获得。还支持一次性bean实例
 * 的注册(它可能对应于已注册的单例，也可能不对应于已注册的单例)，在注册表关闭时销毁。 可以注册bean之间的依赖关系，以强制执行适当的关闭顺序
 *
 * <p>Generic registry for shared bean instances, implementing the {@link
...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry
     ...
     *
     * <p>Cache of singleton objects: bean name to bean instance.
     */
     ...
     * 用来保存当前所有已经注册的bean
     *
     * <p>Set of registered singletons, containing the bean names in registration order.
     */
     ...
     * 当前在创建检查中排除的bean名
     *
     * <p>Names of beans currently excluded from in creation checks.
     */
    private final Set<String> inCreationCheckExclusions =
            Collections.newSetFromMap(new ConcurrentHashMap<>(16));

    /**
     * 一次性Bean实例：bean名称 - DisposableBean实例。 Disposable bean instances: bean name to disposable
     * instance.
     */
     ...
     *
     * <p>Map between containing bean names: bean name to Set of bean names that the bean contains.
     */
    private final Map<String, Set<String>> containedBeanMap = new ConcurrentHashMap<>(16);

    /**
     * 存储 bean名到该bean名所要依赖的bean名 的Map Map between dependent bean names: bean name to Set of
     * dependent bean names.
     */
    private final Map<String, Set<String>> dependentBeanMap = new ConcurrentHashMap<>(64);

    /**
     * 存储 bean名到依赖于该bean名的bean名 的Map Map between depending bean names: bean name to Set of bean
     * names for the bean's dependencies.
     */
     ...
     * 抑制的异常列表，可用于关联相关原因
     *
     * <p>Collection of suppressed Exceptions, available for associating related causes.
     */
     ...
     *
     * <p>Flag that indicates whether we're currently within destroySingletons.
     */
     ...
     * 将beanName和singletonObject的映射关系添加到该工厂的单例缓存中
     *
     * <p>Add the given singleton object to the singleton cache of this factory.
     *
     ...
     *
     * <p>Add the given singleton factory for building the specified singleton if necessary.
     *
     ...
     *
     * <p>Checks already instantiated singletons and also allows for an early reference to a
     ...
     *
     * <p>Return the (raw) singleton object registered under the given name, creating and
     ...
     *
     * <p>Register an exception that happened to get suppressed during the creation of a singleton
     * bean instance, e.g. a temporary circular reference resolution problem.
     *
     * <p>The default implementation preserves any given exception in this registry's collection of
     * suppressed exceptions, up to a limit of 100 exceptions, adding them as related causes to an
     ...
     *
     * <p>Remove the bean with the given name from the singleton cache of this factory, to be able
     * to clean up eager registration of a singleton if creation failed.
     *
     * @param beanName the name of the bean
     ...
     *
     * <p>Return whether the specified singleton bean is currently in creation (within the entire
     * factory).
     *
     * @param beanName the name of the bean
     */
    ...
    /**
     * 创建单例之前的回调: 如果当前在创建检查中的排除bean名列表【inCreationCheckExclusions】中不包含该beanName且将beanName添加到
     * 当前正在创建的bean名称列表【singletonsCurrentlyInCreation】后，出现beanName已经在当前正在创建的bean名称列表中添加过
     *
     * <p>创建单例之前的回调 Callback before singleton creation.
     *
     ...
     *
     * <p>Add the given bean to the list of disposable beans in this registry.
     *
     ...
     *
     * <p>Register a containment relationship between two beans, e.g. between an inner bean and its
     ...
     *
     * <p>Register a dependent bean for the given bean, to be destroyed before the given bean is
     * destroyed.
     *
     * @param beanName the name of the bean
     ...
     *
     * <p>Determine whether the specified dependent bean has been registered as dependent on the
     * given bean or on any of its transitive dependencies.
     *
     ...
     *
     * <p>Determine whether a dependent bean has been registered for the given name.
     *
     * @param beanName the name of the bean to check
     */
     ...
     *
     * <p>Return the names of all beans which depend on the specified bean, if any.
     *
     * @param beanName the name of the bean
     ...
     *
     * <p>Return the names of all beans that the specified bean depends on, if any.
     *
     * @param beanName the name of the bean
     ...
     * 清除此注册表中所有缓存的单例实例
     *
     * <p>Clear all cached singleton instances in this registry.
     *
     * @since 4.3.15
     */
     ...
     *
     * <p>Destroy the given bean. Delegates to {@code destroyBean} if a corresponding disposable
     * bean instance is found.
     *
     * @param beanName the name of the bean
     * @see #destroyBean
     */
     ...
     *
     * <p>Destroy the given bean. Must destroy beans that depend on the given bean before the bean
     ...
     * 将单例互斥暴露给子类和外部协作者
     *
     * <p>Exposes the singleton mutex to subclasses and external collaborators.
     *
