 ...
 *
 * <p>Adapter that implements the {@link DisposableBean} and {@link Runnable} interfaces performing
...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
     ...
     * 检查bean是否有destroy方法
     *
     * <p>Check whether the given bean has any kind of destroy method to call.
     *
     ...
     *
     * <p>Check whether the given bean has destruction-aware post-processors applying to it.
     *
     * @param bean the bean instance
     ...
     * 根据需要推断破环方法
     *
     * <p>If the current value of the given beanDefinition's "destroyMethodName" property is {@link
     * AbstractBeanDefinition#INFER_METHOD}, then attempt to infer a destroy method. Candidate
     * methods are currently limited to public, no-arg methods named "close" or "shutdown" (whether
     * declared locally or inherited). The given BeanDefinition's "destroyMethodName" is updated to
     * be null if no such method is found, otherwise set to the name of the inferred method. This
     * constant serves as the default for the {@code @Bean#destroyMethod} attribute and the value of
     * the constant may also be used in XML within the {@code <bean destroy-method="">} or {@code
     ...
     * 搜索列表中的所有可支持Bean销毁的DestructionAwareBeanPostProcessors
     *
     * <p>Search for all DestructionAwareBeanPostProcessors in the List.
     *
    ...
    private Method findDestroyMethod(String name) {
        // findMethodWithMinimalParameters(Class<?> clazz,String
        // methodName)：找到一个具有给定方法名和最小参数(最好是none)的
        // 方法，在给定的类或它的超类中声明。 首选公共方法，但也将返回受保护的包访问或私有访问
        // BeanUtils.findMethodWithMinimalParameters(Method[] methods, String
        // methodName)：在给定方法列表中找到一个具有给定
        // 方法名和最小参数(最好是无)的方法
        // 根据beanDefinition是否允许访问非公共构造函数和方法的情况来查找最小参数(最好是none)的销毁方法对象
        return (this.nonPublicAccessAllowed
                ? BeanUtils.findMethodWithMinimalParameters(this.bean.getClass(), name)
                : BeanUtils.findMethodWithMinimalParameters(
                        this.bean.getClass().getMethods(), name));
    }
