...
class ExtendedBeanInfo implements BeanInfo {
     ...
     * SimplePropertyDescriptor non-indexed} {@code PropertyDescriptor} variant that bypasses
     * default JDK weak/soft reference management; then search through its method descriptors to
     * find any non-void returning write methods and update or create the corresponding {@link
        ...
        for (PropertyDescriptor pd : delegate.getPropertyDescriptors()) {
            try {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Ignoring invalid bean property '"
                                    + pd.getName()
                                    + "': "
                                    + ex.getMessage());
                }
        ...
        if (methodDescriptors != null) {
            for (Method method : findCandidateWriteMethods(methodDescriptors)) {
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Ignoring candidate write method ["
                                        + method
                                        + "]: "
                                        + ex.getMessage());
                    }
    ...
    private void handleCandidateWriteMethod(Method method) throws IntrospectionException {
        ...
        if (nParams == 1) {
            if (existingPd == null) {
                this.propertyDescriptors.add(
                        new SimplePropertyDescriptor(propertyName, null, method));
            } else {
    ...
    static class SimplePropertyDescriptor extends PropertyDescriptor {
        ...
        public Class<?> getPropertyType() {
            if (this.propertyType == null) {
                try {
                    this.propertyType =
                            PropertyDescriptorUtils.findPropertyType(
    ...
    static class SimpleIndexedPropertyDescriptor extends IndexedPropertyDescriptor {
        ...
        public boolean equals(@Nullable Object other) {
            ...
            return (ObjectUtils.nullSafeEquals(
                            getIndexedReadMethod(), otherPd.getIndexedReadMethod())
                    && ObjectUtils.nullSafeEquals(
                            getIndexedWriteMethod(), otherPd.getIndexedWriteMethod())
                    && ObjectUtils.nullSafeEquals(
                            getIndexedPropertyType(), otherPd.getIndexedPropertyType())
