...
public abstract class AbstractNestablePropertyAccessor extends AbstractPropertyAccessor {
     ...
     */
    protected AbstractNestablePropertyAccessor(
            Object object, String nestedPath, Object rootObject) {
        registerDefaultEditors();
                ...
                for (int i = 0; i < tokens.keys.length; i++) {
                    ...
                    if (value == null) {
                        ...
                        growCollectionIfNecessary(
                                list, index, indexedPropertyName.toString(), ph, i + 1);
                        ...
                        Class<?> mapKeyType =
                                ph.getResolvableType().getNested(i + 1).asMap().resolveGeneric(0);
                        // IMPORTANT: Do not pass full property name in here - property editors
                        // must not kick in for map keys but rather only for map values.
                        TypeDescriptor typeDescriptor = TypeDescriptor.valueOf(mapKeyType);
                        Object convertedMapKey =
                    ...
                    indexedPropertyName
                            .append(PROPERTY_KEY_PREFIX)
                            .append(key)
                            .append(PROPERTY_KEY_SUFFIX);
                }
            }
            return value;
        } catch (IndexOutOfBoundsException ex) {
     ...
     * 根据属性名，获取属性访问器
     *
     * <p>Retrieve a Property accessor for the given nested property. Create a new one if not found
     * in the cache.
     *
    ...
    private AbstractNestablePropertyAccessor getNestedPropertyAccessor(String nestedProperty) {
        ...
        if (value == null || (value instanceof Optional && !((Optional<?>) value).isPresent())) {
            ...
            if (isAutoGrowNestedPaths()) {
                ...
                throw new NullValueInNestedPathException(
                        getRootClass(), this.nestedPath + canonicalName);
            }
        }
        ...
        if (nestedPa == null
                || nestedPa.getWrappedInstance() != ObjectUtils.unwrapOptional(value)) {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace(
                        "Using cached nested property accessor for property '"
                                + canonicalName
                                + "'");
            }
        }
        ...
        try {
            if (type.isArray()) {
                ...
                TypeDescriptor elementDesc =
                        (desc != null ? desc.getElementTypeDescriptor() : null);
                ...
                return CollectionFactory.createMap(
                        type, (keyDesc != null ? keyDesc.getType() : null), 16);
            } else {
        ...
        while (searchIndex != -1) {
            ...
            if (keyStart != -1) {
                ...
                if (keyEnd != -1) {
                    ...
                    String key =
                            propertyName.substring(keyStart + PROPERTY_KEY_PREFIX.length(), keyEnd);
