...
public class ConstructorReference extends SpelNodeImpl {
    ...
    // TODO is this caching safe - passing the expression around will mean this executor is also
    // being passed around
    /** The cached executor that may be reused on subsequent evaluations. */
    @Nullable private volatile ConstructorExecutor cachedExecutor;

    /**
     * Create a constructor reference. The first argument is the type, the rest are the parameters
    ...
    private TypedValue createNewInstance(ExpressionState state) throws EvaluationException {
        ...
        if (executorToUse != null) {
            try {
                ...
                // 2. the method invoked was not passed the arguments it expected and has become
                // 'stale'

                // In the first case we should not retry, in the second case we should see if there
                // is a
                // better suited method.

                // To determine which situation it is, the AccessException will contain a cause.
                // If the cause is an InvocationTargetException, a user exception was thrown inside
                // the constructor.
                ...
                if (ex.getCause() instanceof InvocationTargetException) {
                    ...
                    if (rootCause instanceof RuntimeException) {
                        ...
                        String typeName =
                                (String) this.children[0].getValueInternal(state).getValue();
                ...
                // At this point we know it wasn't a user problem so worth a retry if a better
    ...
    private TypedValue createArray(ExpressionState state) throws EvaluationException {
        ...
        if (!hasInitializer()) {
            // Confirm all dimensions were specified (for example [3][][5] is missing the 2nd
            // dimension)
            ...
            if (arrayTypeCode == TypeCode.OBJECT) {
                populateReferenceTypeArray(
                        state, newArray, typeConverter, initializer, componentType);
    ...
    public boolean isCompilable() {
        ...
        ReflectiveConstructorExecutor executor =
                (ReflectiveConstructorExecutor) this.cachedExecutor;
        if (executor == null) {
            return false;
        }
    ...
    public void generateCode(MethodVisitor mv, CodeFlow cf) {
        ...
        // children[0] is the type of the constructor, don't want to include that in argument
        // processing
        ...
        mv.visitMethodInsn(
                ...
                classDesc,
                "<init>",
                CodeFlow.createSignatureDescriptor(constructor),
                false);
