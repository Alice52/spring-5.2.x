...
public abstract class AbstractWebSocketHandlerRegistration<M>
    ...
    /**
     * Expose the {@code SockJsServiceRegistration} -- if SockJS is enabled or {@code null}
     * otherwise -- so that it can be configured with a TaskScheduler if the application did not
     * provide one. This should be done prior to calling {@link #getMappings()}.
     */
    @Nullable
                    ...
                    (wsHandler, paths) -> {
                        for (String path : paths) {
                            ...
                            addSockJsServiceMapping(
                                    mappings, sockJsService, wsHandler, pathPattern);
                        }
                    });
        } else {
                    ...
                    (wsHandler, paths) -> {
                        for (String path : paths) {
                            addWebSocketHandlerMapping(
                                    mappings, wsHandler, handshakeHandler, interceptors, path);
                        }
                    });
        }
    ...
    private HandshakeHandler getOrCreateHandshakeHandler() {
        return (this.handshakeHandler != null
