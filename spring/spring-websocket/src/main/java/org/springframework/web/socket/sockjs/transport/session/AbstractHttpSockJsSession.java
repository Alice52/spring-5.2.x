...
public abstract class AbstractHttpSockJsSession extends AbstractSockJsSession {
    ...
    /**
     * Handle all requests, except the first one, to receive messages on a SockJS HTTP transport
     * based session.
     *
     ...
     * "eventsource", and "htmlfile") leave the response open longer for further streaming of
     * message frames but will also close it eventually after some amount of data has been sent.
     *
     * @param request the current request
     * @param response the current response
     * @param frameFormat the transport-specific SocksJS frame format to use
    ...
    @Override
    protected final void sendMessageInternal(String message)
    ...
    /**
     * Called when the connection is active and ready to write to the response. Subclasses should
     * only call this method from a method where the "responseLock" is acquired.
     */
