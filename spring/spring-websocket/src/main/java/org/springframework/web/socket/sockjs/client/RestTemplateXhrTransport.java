...
public class RestTemplateXhrTransport extends AbstractXhrTransport {
        ...
        getTaskExecutor()
                .execute(
                        () -> {
                            ...
                            XhrRequestCallback requestCallback =
                                    new XhrRequestCallback(handshakeHeaders);
                            XhrRequestCallback requestCallbackAfterHandshake =
                                    new XhrRequestCallback(httpHeaders);
                            XhrReceiveExtractor responseExtractor =
                                    new XhrReceiveExtractor(session);
                            while (true) {
                                ...
                                try {
                                    ...
                                    if (!connectFuture.isDone()) {
                                        ...
                                        session.afterTransportClosed(
                                                new CloseStatus(1006, ex.getMessage()));
                                    }
                                    break;
                                }
                            }
    ...
    protected ResponseEntity<String> executeInfoRequestInternal(URI infoUrl, HttpHeaders headers) {
        ...
        return nonNull(
                this.restTemplate.execute(
                        infoUrl, HttpMethod.GET, requestCallback, textResponseExtractor));
    }
    ...
    public ResponseEntity<String> executeSendRequestInternal(
        ...
        return nonNull(
                this.restTemplate.execute(
                        url, HttpMethod.POST, requestCallback, textResponseExtractor));
    }
                ...
                if (request instanceof StreamingHttpOutputMessage) {
                    ((StreamingHttpOutputMessage) request)
                            .setBody(
                                    outputStream ->
                                            StreamUtils.copy(
                                                    this.body, SockJsFrame.CHARSET, outputStream));
                } else {
