...
/**
 * 覆盖ClassLoader的特殊变体,用于{@link AbstractApplicationContext}中的临时类型匹配。 为每个{@code
 * loadClass}调用从缓存的字节数组中重新定义类，以便在父类ClassLoader中拾取 最近加载的类型
 *
 * <p>Special variant of an overriding ClassLoader, used for temporary type matching in {@link
...
class ContextTypeMatchClassLoader extends DecoratingClassLoader implements SmartClassLoader {
     ...
     *
     * <p>ClassLoader to be created for each loaded class. Caches class file content but redefines
     * class for each call.
     */
    private class ContextOverridingClassLoader extends OverridingClassLoader {
        ...
        protected boolean isEligibleForOverriding(String className) {
            ...
            while (parent != null) {
                ...
                if (ReflectionUtils.invokeMethod(findLoadedClassMethod, parent, className)
                        != null) {
