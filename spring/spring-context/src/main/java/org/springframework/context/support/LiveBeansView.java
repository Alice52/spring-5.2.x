 ...
 * @since 3.2
 * @see #getSnapshotAsJson() see org.springframework.web.context.support.LiveBeansViewServlet
 */
public class LiveBeansView implements LiveBeansViewMBean, ApplicationContextAware {

    /** 'MBean Domain'属性名 The "MBean Domain" property name. */
    public static final String MBEAN_DOMAIN_PROPERTY_NAME = "spring.liveBeansView.mbeanDomain";

    /** MBean应用Key The MBean application key. */
                ...
                if (applicationContexts.isEmpty()) {
                    try {
                        ...
                        server.registerMBean(
                                new LiveBeansView(),
                                new ObjectName(
                                        mbeanDomain, MBEAN_APPLICATION_KEY, applicationName));
                    }
                    // 捕捉注册时抛出的异常
                    catch (Throwable ex) {
                        throw new ApplicationContextException(
                                "Failed to register LiveBeansView MBean", ex);
                    }
                }
            ...
            if (applicationContexts.remove(applicationContext) && applicationContexts.isEmpty()) {
                try {
                    ...
                    String mbeanDomain =
                            applicationContext
                                    .getEnvironment()
                                    .getProperty(MBEAN_DOMAIN_PROPERTY_NAME);
                    if (mbeanDomain != null) {
                    ...
                    throw new ApplicationContextException(
                            "Failed to unregister LiveBeansView MBean", ex);
                } finally {
     ...
     * context and parent attribute as well as a beans attribute with nested bean description
     * objects. Each bean object contains a bean, scope, type and resource attribute, as well as a
     * dependencies attribute with a nested array of bean names that the present bean depends on.
     *
     ...
     */
    protected boolean isBeanEligible(
            String beanName, BeanDefinition bd, ConfigurableBeanFactory bf) {
