...
class ConfigurationClassBeanDefinitionReader {
    ...
    private static final Log logger =
            LogFactory.getLog(ConfigurationClassBeanDefinitionReader.class);
     ...
     *
     * <p>Read a particular {@link ConfigurationClass}, registering bean definitions for the class
     * itself and all of its {@link Bean} methods.
     */
    ...
    private void registerBeanDefinitionForImportedConfigurationClass(
        ...
        BeanDefinitionHolder definitionHolder =
                new BeanDefinitionHolder(configBeanDef, configBeanName);
        definitionHolder =
     ...
     *
     * <p>Read the given {@link BeanMethod}, registering bean definitions with the
        ...
        if (metadata.isStatic()) {
            ...
            if (configClass.getMetadata() instanceof StandardAnnotationMetadata) {
                beanDef.setBeanClass(
                        ((StandardAnnotationMetadata) configClass.getMetadata())
    ...
    /**
     * 首先判断已存在的bean定义是不是和bean注册方法同一个配置类的重载方法，如果是的话就不覆盖，且设置工厂方法不唯一，否则要覆盖，如果不是，就看是不是 component
     * scan扫描进来的bean定义，是的话要覆盖，如果不是就看是不是内部的，如果是也覆盖
     *
     * @param beanMethod
     * @param beanName
     * @return
    ...
    protected boolean isOverriddenByExistingDefinition(BeanMethod beanMethod, String beanName) {
        ...
        if (existingBeanDef instanceof ConfigurationClassBeanDefinition) {
            ConfigurationClassBeanDefinition ccbd =
                    (ConfigurationClassBeanDefinition) existingBeanDef;
            if (ccbd.getMetadata()
                    .getClassName()
                    .equals(beanMethod.getConfigurationClass().getMetadata().getClassName())) {
                if (ccbd.getFactoryMethodMetadata()
                        .getMethodName()
                        .equals(ccbd.getFactoryMethodName())) {
                    // 设置工厂方法不唯一，说明有重载
                    ccbd.setNonUniqueFactoryMethodName(
                            ccbd.getFactoryMethodMetadata().getMethodName());
                }
                // 不覆盖
                return true;
            } else {
        ...
        if (this.registry instanceof DefaultListableBeanFactory
                && !((DefaultListableBeanFactory) this.registry)
    ...
    private static class ConfigurationClassBeanDefinition extends RootBeanDefinition
        ...
        public boolean isFactoryMethod(Method candidate) {
            return (super.isFactoryMethod(candidate)
                    ...
                    && BeanAnnotationHelper.determineBeanNameFor(candidate)
                            .equals(this.derivedBeanName));
        }
        ...
        public boolean shouldSkip(ConfigurationClass configClass) {
            ...
            if (skip == null) {
                if (configClass.isImported()) {
                    ...
                    if (allSkipped) {
                        // The config classes that imported this one were all skipped, therefore we
                        // are skipped...
                        skip = true;
                    }
                }
