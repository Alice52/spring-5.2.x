...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
    ...
    /** o BeanFactoryPostProcessors to apply on refresh. */
    ...
    /** 用来存放applicationListeners的集合对象 Statically specified listeners. */
    private final Set<ApplicationListener<?>> applicationListeners = new LinkedHashSet<>();

    /**
     * 创建上下文的唯一标识
     *
     * <p>Unique id for this context, if any.
     */
     ...
     *
     * <p>Create a new AbstractApplicationContext with no parent.
     */
    ...
    /**
     * Set a friendly name for this context. Typically done during initialization of concrete
    ...
    /**
     * Return the parent context, or {@code null} if there is no parent (that is, this context is
    ...
    /**
     * Return the {@code Environment} for this application context in configurable form, allowing
     ...
     *
     * <p>Default value is determined by {@link #createEnvironment()}. Replacing the default with
     * this method is one option but configuration through {@link #getEnvironment()} should also be
     ...
     * 将给定事件发布到所有监听器
     *
     * <p>Publish the given event to all listeners.
     *
     ...
     * listener implementations. Thus, MessageSource implementations cannot publish events.
     *
     * @param event the event to publish (may be an {@link ApplicationEvent} or a payload object to
     *     be turned into a {@link PayloadApplicationEvent})
     */
    @Override
     ...
     * org.springframework.core.io.support.PathMatchingResourcePatternResolver}, supporting
     * Ant-style location patterns.
     *
    ...
    public void refresh() throws BeansException, IllegalStateException {
        synchronized (this.startupShutdownMonitor) {
            ...
            /**
             * 前戏，做容器刷新前的准备工作 1、设置容器的启动时间 2、设置活跃状态为true 3、设置关闭状态为false
             * 4、获取Environment对象，并加载当前系统的属性值到Environment对象中 5、准备监听器和事件的集合对象，默认为空的集合
             */
            prepareRefresh();
     ...
     *
     * @see org.springframework.core.env.PropertySource.StubPropertySource see
    ...
    protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        // aspectj提供了两种织入方式，第一种是通过特殊编译器，在编译器，将aspectj语言编写的切面类织入到java类中，第二种是类加载期织入，就是下面的load time
        // weaving，此处后续讲
     ...
     *
     * <p>Instantiate and invoke all registered BeanFactoryPostProcessor beans, respecting explicit
     * order if given.
     *
     ...
     *
     * <p>Instantiate and register all BeanPostProcessor beans, respecting explicit order if given.
     *
     ...
     *
     * <p>Initialize the LifecycleProcessor. Uses DefaultLifecycleProcessor if none defined in the
     * context.
     *
     * @see org.springframework.context.support.DefaultLifecycleProcessor
     */
     ...
     *
     * <p>Template method which can be overridden to add context-specific refresh work. Called on
     ...
     * 注册监听器
     *
     * <p>Add beans that implement ApplicationListener as listeners. Doesn't affect other listeners,
    ...
    /**
     * Finish the initialization of this context's bean factory, initializing all remaining
     * singleton beans.
     */
    ...
    /**
     * 完成刷新 Finish the refresh of this context, invoking the LifecycleProcessor's onRefresh() method
     * and publishing the {@link org.springframework.context.event.ContextRefreshedEvent}.
     */
    ...
    /**
     * Cancel this context's refresh attempt, resetting the {@code active} flag after an exception
     * got thrown.
     *
     ...
     * {@link AnnotationUtils}, {@link ResolvableType} and {@link CachedIntrospectionResults}
     * caches.
     *
     * @since 4.2
    ...
    /**
     * Register a shutdown hook {@linkplain Thread#getName() named} {@code
     * SpringContextShutdownHook} with the JVM runtime, closing this context on JVM shutdown unless
     * it has already been closed at that time.
     *
     ...
     *
     * <p>The {@link #close()} method is the native way to shut down an ApplicationContext, which
     ...
     *
     * <p>Delegates to {@code doClose()} for the actual closing procedure. Also removes a JVM
     ...
     * implementation destroy all cached singletons in this context, invoking {@code
     * DisposableBean.destroy()} and/or the specified "destroy-method".
     *
     * <p>Can be overridden to add context-specific bean destruction steps right before or right
     * after standard singleton destruction, while the context's BeanFactory is still active.
     *
     * @see #getBeanFactory()
     ...
     *
     * <p>Called at the end of {@link #doClose}'s shutdown procedure, after this context's
     * BeanFactory has been closed. If custom shutdown logic needs to execute while the BeanFactory
     * is still active, override the {@link #destroyBeans()} method instead.
     */
    ...
    /**
     * Subclasses must implement this method to release their internal bean factory. This method
     * gets invoked by {@link #close()} after all other shutdown work.
     *
     ...
     * internal bean factory. The internal factory should generally be considered unavailable once
