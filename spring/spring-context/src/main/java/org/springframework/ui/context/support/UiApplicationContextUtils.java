...
/**
 * 用于UI应用程序上下文实现的实用程序类。提供对名为'themeSource' * 的特殊Bean的支持，该Bean类型为 themeSource
 *
 * <p>Utility class for UI application context implementations. Provides support for a special bean
...
public abstract class UiApplicationContextUtils {
    ...
    /**
     * Name of the ThemeSource bean in the factory. If none is supplied, theme resolution is
     ...
     *
     * <p>Initialize the ThemeSource for the given application context, autodetecting a bean with
     * the name "themeSource". If no such bean is found, a default (empty) ThemeSource will be used.
     *
