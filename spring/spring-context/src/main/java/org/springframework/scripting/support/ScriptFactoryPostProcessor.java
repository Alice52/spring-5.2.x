...
public class ScriptFactoryPostProcessor extends InstantiationAwareBeanPostProcessorAdapter
        ...
        try {
            ...
            if (ex instanceof BeanCreationException
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Could not determine scripted object type for bean '" + beanName + "'",
                            ex);
                }
            }
        }
    ...
    protected void prepareScriptBeans(
        ...
        synchronized (this.scriptBeanFactory) {
            if (!this.scriptBeanFactory.containsBeanDefinition(scriptedObjectBeanName)) {
                ...
                ScriptSource scriptSource =
                        getScriptSource(
                                scriptFactoryBeanName, scriptFactory.getScriptSourceLocator());
                Class<?>[] interfaces = scriptFactory.getScriptInterfaces();
    ...
    /**
     * Get the refresh check delay for the given {@link ScriptFactory} {@link BeanDefinition}. If
     * the {@link BeanDefinition} has a {@link org.springframework.core.AttributeAccessor metadata
     * attribute} under the key {@link #REFRESH_CHECK_DELAY_ATTRIBUTE} which is a valid {@link
     * Number} type, then this value is used. Otherwise, the {@link #defaultRefreshCheckDelay} value
     * is used.
     *
     ...
     *
     * <p>By default, supported locators are Spring resource locations (such as
    ...
    protected Class<?> createConfigInterface(BeanDefinition bd, @Nullable Class<?>[] interfaces) {
        ...
        for (PropertyValue pv : pvs) {
            ...
            Signature signature =
                    new Signature(
                            setterName, Type.VOID_TYPE, new Type[] {Type.getType(propertyType)});
            maker.add(signature, new Type[0]);
        }
        if (bd.getInitMethodName() != null) {
            Signature signature =
                    new Signature(bd.getInitMethodName(), Type.VOID_TYPE, new Type[0]);
            maker.add(signature, new Type[0]);
        }
        if (StringUtils.hasText(bd.getDestroyMethodName())) {
            Signature signature =
                    new Signature(bd.getDestroyMethodName(), Type.VOID_TYPE, new Type[0]);
            maker.add(signature, new Type[0]);
        }
        return maker.create();
    }
