...
public class MBeanClientInterceptor
     ...
     *
     * <p>Can be turned on to allow for hot restart of the JMX server, automatically reconnecting
     * and retrying in case of an IOException.
     */
    ...
    /**
     * Prepares the {@code MBeanServerConnection} if the "connectOnStartup" is turned on (which it
     * is by default).
     */
    @Override
    ...
    public void prepare() {
        synchronized (this.preparationMonitor) {
            if (this.server != null) {
                ...
                this.serverToUse =
                        this.connector.connect(this.serviceUrl, this.environment, this.agentId);
            }
            this.invocationHandler = null;
            if (this.useStrictCasing) {
                Assert.state(this.objectName != null, "No ObjectName set");
                // Use the JDK's own MBeanServerInvocationHandler, in particular for native MXBean
                // support.
     ...
     * information is used by the proxy when determining whether an invocation matches a valid
     * operation or attribute on the management interface of the managed resource.
     */
        ...
        try {
            ...
            for (MBeanOperationInfo infoEle : operationInfo) {
                ...
                this.allowedOperations.put(
                        new MethodCacheKey(infoEle.getName(), paramTypes), infoEle);
            }
            ...
            if (this.invocationHandler != null) {
                result =
                        this.invocationHandler.invoke(
                                invocation.getThis(), method, invocation.getArguments());
            } else {
    ...
    private Object invokeAttribute(PropertyDescriptor pd, MethodInvocation invocation)
        ...
        if (invocation.getMethod().equals(pd.getReadMethod())) {
            if (inf.isReadable()) {
                ...
                throw new InvalidInvocationException(
                        "Attribute '" + attributeName + "' is not readable");
            }
            ...
            if (inf.isWritable()) {
                ...
                throw new InvalidInvocationException(
                        "Attribute '" + attributeName + "' is not writable");
            }
        } else {
            throw new IllegalStateException(
                    "Method ["
                            + invocation.getMethod()
                            + "] is neither a bean property getter nor a setter");
        }
    }
    ...
    private Collection<?> convertDataArrayToTargetCollection(
            Object[] array, Class<?> collectionType, Class<?> elementType)
            throws NoSuchMethodException {
    ...
    private static final class MethodCacheKey implements Comparable<MethodCacheKey> {
        ...
        /**
         * Create a new instance of {@code MethodCacheKey} with the supplied method name and
         * parameter list.
         *
        ...
        public String toString() {
            return this.name
                    + "("
                    + StringUtils.arrayToCommaDelimitedString(this.parameterTypes)
                    + ")";
        }
            ...
            for (int i = 0; i < this.parameterTypes.length; i++) {
                result =
                        this.parameterTypes[i]
                                .getName()
                                .compareTo(other.parameterTypes[i].getName());
                if (result != 0) {
                    return result;
                }
            }
