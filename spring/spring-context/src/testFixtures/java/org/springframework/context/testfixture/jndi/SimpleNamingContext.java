...
/**
 * Copy of the standard {@link org.springframework.mock.jndi.SimpleNamingContext} for testing
 * purposes.
 *
 * <p>Simple implementation of a JNDI naming context. Only supports binding plain Objects to String
...
public class SimpleNamingContext implements Context {
    ...
    /**
     * Create a new naming context with the given naming root, the given name/object map, and the
     ...
     * setting up a JVM-level JNDI environment. Use SimpleNamingContextBuilder to set up JNDI
     * bindings then.
     *
                    ...
                    if (!contents.containsKey(strippedName)) {
                        try {
                            contents.put(
                                    strippedName,
                                    createObject(
                                            strippedName, context.lookup(proot + strippedName)));
