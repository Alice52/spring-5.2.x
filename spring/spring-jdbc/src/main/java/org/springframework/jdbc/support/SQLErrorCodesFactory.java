...
public class SQLErrorCodesFactory {
    ...
    /**
     * The name of custom SQL error codes file, loading from the root of the class path (e.g. from
     ...
     *
     * <p>Not public to enforce Singleton design pattern. Would be private except to allow testing
     * via overriding the {@link #loadResource(String)} method.
     *
     ...
     *
     * <p><b>Not to be overridden by application developers, who should obtain instances of this
     * class from the static {@link #getInstance()} method.</b>
     *
     ...
     * @return the {@code SQLErrorCodes} instance for the given database (never {@code null};
     *     potentially empty)
        ...
        if (sec == null) {
            for (SQLErrorCodes candidate : this.errorCodesMap.values()) {
                if (PatternMatchUtils.simpleMatch(
                        candidate.getDatabaseProductNames(), databaseName)) {
                    sec = candidate;
                    break;
                }
            }
     ...
     * "databaseProductName" from the {@link java.sql.DatabaseMetaData}, or an empty error codes
     * instance if no {@code SQLErrorCodes} were found.
     *
     * @param dataSource the {@code DataSource} identifying the database
     * @return the corresponding {@code SQLErrorCodes} object (never {@code null}; potentially
     *     empty)
     * @see java.sql.DatabaseMetaData#getDatabaseProductName()
     */
    ...
    /**
     * Return {@link SQLErrorCodes} for the given {@link DataSource}, evaluating
     * "databaseProductName" from the {@link java.sql.DatabaseMetaData}, or {@code null} if case of
     * a JDBC meta-data access problem.
     *
     * @param dataSource the {@code DataSource} identifying the database
     * @return the corresponding {@code SQLErrorCodes} object, or {@code null} in case of a JDBC
     *     meta-data access problem
     * @since 5.2.9
     * @see java.sql.DatabaseMetaData#getDatabaseProductName()
     */
    @Nullable
     ...
     *
     * @param dataSource the {@code DataSource} identifying the database
     * @return the corresponding {@code SQLErrorCodes} object that got removed, or {@code null} if
     *     not registered
     * @since 4.3.5
