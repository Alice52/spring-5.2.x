...
public abstract class StatementCreatorUtils {
     ...
     * java.sql.ParameterMetaData#getParameterType} completely, i.e. to never even attempt to
     * retrieve {@link PreparedStatement#getParameterMetaData()} for {@link
     * StatementCreatorUtils#setNull} calls.
     *
     ...
     * connection pool issues in case of an exception thrown from {@code getParameterType} (as
     * reported on JBoss AS 7) or in case of performance problems (as reported on PostgreSQL).
     */
        ...
        if (sqlType == SqlTypeValue.TYPE_UNKNOWN || (sqlType == Types.OTHER && typeName == null)) {
            ...
            if (sqlTypeToUse == null) {
                ...
                if (databaseProductName.startsWith("Informix")
                        || (jdbcDriverName.startsWith("Microsoft")
                                && jdbcDriverName.contains("SQL Server"))) {
                    // "Microsoft SQL Server JDBC Driver 3.0" versus "Microsoft JDBC Driver 4.0 for
                    // SQL Server"
                    useSetObject = true;
    ...
    private static void setValue(
        ...
        if (inValue instanceof SqlTypeValue) {
            ...
            if (strVal.length() > 4000) {
                // Necessary for older Oracle drivers, in particular when running against an Oracle
                // 10 database.
                // Should also work fine against other drivers/databases since it uses standard JDBC
                // 4.0 API.
            ...
            if (isStringValue(inValue.getClass())) {
                ...
                ps.setTimestamp(
                        paramIndex, new java.sql.Timestamp(((java.util.Date) inValue).getTime()));
            } else if (inValue instanceof Calendar) {
                Calendar cal = (Calendar) inValue;
                ps.setTimestamp(paramIndex, new java.sql.Timestamp(cal.getTime().getTime()), cal);
            } else {
    ...
    /**
     * Clean up all resources held by parameter values which were passed to an execute method. This
     * is for example important for closing LOB values.
     *
