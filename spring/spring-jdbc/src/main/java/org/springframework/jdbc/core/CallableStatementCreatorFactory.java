...
public class CallableStatementCreatorFactory {
    ...
    /**
     * Create a new factory. Will need to add parameters via the {@link #addParameter} method or
     * have no parameters.
     *
     ...
     */
    public CallableStatementCreatorFactory(
            String callString, List<SqlParameter> declaredParameters) {
        this.callString = callString;
    ...
    /**
     * Set whether to use prepared statements that return a specific type of ResultSet. specific
     * type of ResultSet.
     *
    ...
    private class CallableStatementCreatorImpl
        ...
        public CallableStatement createCallableStatement(Connection con) throws SQLException {
            // If we were given a ParameterMapper, we must let the mapper do its thing to create the
            // Map.
                        ...
                        if (declaredParam instanceof SqlOutParameter) {
                            ...
                            if (declaredParam.isInputValueProvided()) {
                                StatementCreatorUtils.setParameterValue(
                                        cs, sqlColIndx, declaredParam, inValue);
                            }
                        }
                    } else {
        ...
        public String toString() {
            return "CallableStatementCreator: sql=["
                    + callString
                    + "]; parameters="
                    + this.inParameters;
        }
