...
public class CallMetaDataContext {
    ...
    private CallMetaDataProvider obtainMetaDataProvider() {
        Assert.state(
                ...
                "No CallMetaDataProvider - call initializeMetaData first");
        return this.metaDataProvider;
    }

    /**
     * Create a ReturnResultSetParameter/SqlOutParameter depending on the support provided by the
     * JDBC driver used for the database in use.
     *
     ...
     */
    public SqlParameter createReturnResultSetParameter(
            String parameterName, RowMapper<?> rowMapper) {
        ...
        if (provider.isReturnResultSetSupported()) {
            ...
            if (provider.isRefCursorSupported()) {
                return new SqlOutParameter(
                        parameterName, provider.getRefCursorSqlType(), rowMapper);
            } else {
    ...
    protected List<SqlParameter> reconcileParameters(List<SqlParameter> parameters) {
        ...
        for (SqlParameter param : parameters) {
            if (param.isResultsParameter()) {
                ...
                if (param instanceof SqlOutParameter) {
                    ...
                    if (isFunction()
                            && !metaDataParamNames.contains(paramNameToMatch)
                            && !returnDeclared) {
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "Using declared out parameter '"
                                            + paramName
        ...
        Map<String, String> limitedInParamNamesMap =
                new HashMap<>(this.limitedInParameterNames.size());
            ...
            if (declaredParams.containsKey(paramNameToCheck)
                ...
                if (meta.isReturnParameter()) {
                    ...
                    if (!isFunction()
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "Added meta-data return parameter for '"
                                            + returnNameToUse
                                            + "'");
                        }
                    }
                    ...
                    if (meta.getParameterType() == DatabaseMetaData.procedureColumnOut) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "Added meta-data in-out parameter for '"
                                            + paramNameToUse
                                            + "'");
                        }
                    } else {
                        ...
                        if (this.limitedInParameterNames.isEmpty()
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "Added meta-data in parameter for '"
                                                + paramNameToUse
                                                + "'");
                            }
                        } else {
        ...
        for (SqlParameter parameter : this.callParameters) {
            if (parameter.isInputValueProvided()) {
                ...
                String parameterNameToMatch =
                        obtainMetaDataProvider().parameterNameToUse(parameterName);
                        ...
                        if (parameterSource.hasValue(lowerCaseName)) {
                            ...
                            if (parameterSource.hasValue(englishLowerCaseName)) {
                                matchedParameters.put(
                                        parameterName,
                                        SqlParameterSourceUtils.getTypedValue(
                                                parameterSource, englishLowerCaseName));
                            } else {
                                String propertyName =
