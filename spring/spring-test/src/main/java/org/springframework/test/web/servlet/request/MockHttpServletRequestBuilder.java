...
public class MockHttpServletRequestBuilder
     ...
     * is because most applications don't actually depend on the name under which they're deployed.
     * If specified here, the context path must start with a "/" and must not end with a "/".
     *
    ...
    /**
     * Specify the portion of the requestURI that represents the path to which the Servlet is
     * mapped. This is typically a portion of the requestURI after the context path.
     *
     * <p>In most cases, tests can be written by omitting the servlet path from the requestURI. This
     * is because most applications don't actually depend on the prefix to which a servlet is
     * mapped. For example if a Servlet is mapped to {@code "/main/*"}, tests can be written with
     * the requestURI {@code "/accounts/1"} as opposed to {@code "/main/accounts/1"}. If specified
     * here, the servletPath must start with a "/" and must not end with a "/".
     *
    ...
    /**
     * Set the secure property of the {@link ServletRequest} indicating use of a secure channel,
     * such as HTTPS.
     *
     ...
     *
     * <p>If content is provided and {@link #contentType(MediaType)} is set to {@code
     * application/x-www-form-urlencoded}, the content will be parsed and used to populate the
     ...
     *
     * <p>If content is provided and {@code contentType} is set to {@code
     * application/x-www-form-urlencoded}, the content will be parsed and used to populate the
     * {@link #param(String, String...) request parameters} map.
     *
    ...
    /**
     * Set the 'Content-Type' header of the request as a raw String value, possibly not even well
    ...
    /**
     * Set the 'Accept' header using raw String values, possibly not even well formed (for testing
     * purposes).
     *
     * @param mediaTypes one or more media types; internally joined as comma-separated String
     */
     ...
     *
     * <p>In the Servlet API, a request parameter may be parsed from the query string and/or from
     * the body of an {@code application/x-www-form-urlencoded} request. This method simply adds to
     * the request parameter map. You may also use add Servlet request parameters by specifying the
     * query or form data through one of the following:
     *
     * <ul>
     *   <li>Supply a URL with a query to {@link MockMvcRequestBuilders}.
     *   <li>Add query params via {@link #queryParam} or {@link #queryParams}.
     *   <li>Provide {@link #content} with {@link #contentType} {@code
    ...
    /**
     * Append to the query string and also add to the {@link #param(String, String...) request
     * parameters} map. The parameter name and value are encoded when they are added to the query
     * string.
     *
     * @param name the parameter name
     * @param values one or more values
    ...
    /**
     * Append to the query string and also add to the {@link #params(MultiValueMap)} request
     * parameters} map. The parameter name and value are encoded when they are added to the query
     * string.
     *
     * @param params the parameters to add
     ...
     *
     * <p>Individual attributes provided via {@link #sessionAttr(String, Object)} override the
     * content of the session provided here.
     *
     ...
     * built directly into the {@code MockHttpServletRequestBuilder}. Implementation of this
     * interface can have builder-style methods themselves and be made accessible through static
     * factory methods.
     *
    ...
    public Object merge(@Nullable Object parent) {
        ...
        if (!(parent instanceof MockHttpServletRequestBuilder)) {
            throw new IllegalArgumentException(
                    "Cannot merge with [" + parent.getClass().getName() + "]");
        }
        ...
        if ("".equals(this.pathInfo)) {
            ...
            this.pathInfo =
                    (StringUtils.hasText(extraPath)
                            ? UrlPathHelper.defaultInstance.decodeRequestString(request, extraPath)
                            : null);
        }
    ...
    private void addRequestParams(
            MockHttpServletRequest request, MultiValueMap<String, String> map) {
        map.forEach(
                (key, values) ->
                        values.forEach(
                                value -> {
                                    ...
                                    request.addParameter(
                                            UriUtils.decode(key, StandardCharsets.UTF_8), value);
                                }));
    }
    ...
    private FlashMapManager getFlashMapManager(MockHttpServletRequest request) {
        ...
        try {
            ...
            flashMapManager =
                    wac.getBean(
                            DispatcherServlet.FLASH_MAP_MANAGER_BEAN_NAME, FlashMapManager.class);
