...
public class JsonPathExpectationsHelper {
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert the
     * resulting value with the given {@code Matcher}.
     *
    ...
    /**
     * An overloaded variant of {@link #assertValue(String, Matcher)} that also accepts a target
     * type for the resulting value. This can be useful for matching numbers reliably for example
            ...
            try {
                ...
                String message =
                        String.format(
                                ...
                                this.expression,
                                actualValue,
                                ClassUtils.getDescriptiveType(actualValue),
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that an
     * empty value exists at the given path.
     *
    ...
    public void assertValueIsEmpty(String content) {
        ...
        AssertionErrors.assertTrue(
                failureReason("an empty value", value), ObjectUtils.isEmpty(value));
    }
    ...
    /**
     * Evaluate the JSON path expression against the supplied {@code content} and assert that a
     * value, possibly {@code null}, exists.
     *
    ...
    public void doesNotHaveJsonPath(String content) {
        ...
        if (pathIsIndefinite() && value instanceof List) {
            AssertionErrors.assertTrue(
                    failureReason("no values", value), ((List<?>) value).isEmpty());
        } else {
     ...
     *
     * <p>This can be useful for matching numbers reliably for example coercing an integer into a
     * double.
     *
     * @param content the content to evaluate against
     * @return the result of the evaluation
