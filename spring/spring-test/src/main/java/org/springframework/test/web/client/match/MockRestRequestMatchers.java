...
public abstract class MockRestRequestMatchers {
    ...
    public static RequestMatcher requestToUriTemplate(String expectedUri, Object... uriVars) {
        Assert.notNull(expectedUri, "'uri' must not be null");
        URI uri =
                UriComponentsBuilder.fromUriString(expectedUri)
        ...
        return request -> {
            ...
            if (headerValues != null) {
                fail(
                        "Expected header <"
                                + name
                                + "> not to exist, but it exists with values: "
                                + headerValues);
            }
        };
    }
     ...
     * subset of the body. The JSON path expression can be a parameterized string using formatting
     ...
     * Access to request body matchers using a <a
     * href="https://github.com/jayway/JsonPath">JsonPath</a> expression to inspect a specific
     * subset of the body and a Hamcrest match for asserting the value found at the JSON path.
     *
     ...
     * XPath expression can be a parameterized string using formatting specifiers as defined in
