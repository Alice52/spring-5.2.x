...
public class DefaultJCacheOperationSource extends AnnotationJCacheOperationSource
    ...
    /**
     * Set the default {@link KeyGenerator}. If none is set, a {@link SimpleKeyGenerator} honoring
     * the JSR-107 {@link javax.cache.annotation.CacheKey} and {@link
     * javax.cache.annotation.CacheValue} will be used.
     */
    ...
    protected <T> T getBean(Class<T> type) {
        Assert.state(
                ...
                () -> "BeanFactory required for resolution of [" + type + "]");
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "No bean of type [" + type.getName() + "] found in application context",
                        ex);
            }
    ...
    protected CacheManager getDefaultCacheManager() {
        if (getCacheManager() == null) {
            Assert.state(
                    this.beanFactory != null,
                    "BeanFactory required for default CacheManager resolution");
            try {
                this.cacheManager =
                        SingletonSupplier.of(this.beanFactory.getBean(CacheManager.class));
            } catch (NoUniqueBeanDefinitionException ex) {
                throw new IllegalStateException(
    ...
    protected CacheResolver getDefaultCacheResolver() {
        if (getCacheResolver() == null) {
            this.cacheResolver =
                    SingletonSupplier.of(new SimpleCacheResolver(getDefaultCacheManager()));
        }
    ...
    class LazyCacheResolver implements CacheResolver {
        ...
        private final SingletonSupplier<CacheResolver> cacheResolver =
                SingletonSupplier.of(
                        () -> new SimpleExceptionCacheResolver(getDefaultCacheManager()));
