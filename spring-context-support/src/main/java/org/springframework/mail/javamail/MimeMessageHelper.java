...
public class MimeMessageHelper {
     ...
     * other mail clients tend to misinterpret inline elements as attachments and/or show
     * attachments inline as well.
     */
    public static final int MULTIPART_MODE_MIXED = 1;

    /**
     * Constant indicating a multipart message with a single root multipart element of type
     * "related". Texts, inline elements and attachements will all get added to that root element.
     *
     * <p>This was the default behavior from Spring 1.1 up to 1.2 final. This is the "Microsoft
     * multipart mode", as natively sent by Outlook. It is known to work properly on Outlook,
     * Outlook Express, Yahoo Mail, and to a large degree also on Mac Mail (with an additional
     * attachment listed for an inline element, despite the inline element also shown inline). Does
     * not work properly on Lotus Notes (attachments won't be shown there).
     */
     ...
     * Mail, and Lotus Notes. Does not work properly on Mac Mail. If you target Mac Mail or
     * experience issues with specific mails on Outlook, consider using MULTIPART_MODE_RELATED
     * instead.
     */
     ...
     *
     * <p>The character encoding for the message will be taken from the passed-in MimeMessage
     ...
     *
     * @param mimeMessage the mime message to work on
     * @param multipart whether to create a multipart message that supports alternative texts,
     *     inline elements and attachments (corresponds to MULTIPART_MODE_MIXED_RELATED)
    ...
    /**
     * Determine the MimeMultipart objects to use, which will be used to store attachments on the
     * one hand and text(s) and inline elements on the other hand.
     *
     ...
     *
     * <p>By default, the root MimeMultipart element will be of type "mixed" (MULTIPART_MODE_MIXED)
     * or "related" (MULTIPART_MODE_RELATED). The main multipart element will either be added as
     * nested element of type "related" (MULTIPART_MODE_MIXED_RELATED) or be identical to the root
     ...
     *
     * @param root the root MimeMultipart object, which attachments will be added to; or {@code
    ...
    /**
     * Return the underlying MIME "multipart/related" object, if any. Can be used to manually add
     * body parts, inline elements, etc.
     *
    ...
    private void setHtmlTextToMimePart(MimePart mimePart, String text) throws MessagingException {
        if (getEncoding() != null) {
            mimePart.setContent(
                    text, CONTENT_TYPE_HTML + CONTENT_TYPE_CHARSET_SUFFIX + getEncoding());
        } else {
     ...
     * for temporary files with arbitrary filenames (possibly ending in ".tmp" or the like)!
     *
     * <p>Note that the InputStream returned by the Resource implementation needs to be a <i>fresh
     * one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
     *
     * <p><b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@link #setText}; else, mail readers
     * might not be able to resolve inline references correctly.
     ...
     *
     * <p>You can determine the content type for any given filename via a Java Activation
     ...
     * <i>fresh one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple
     * times.
     *
     * <p><b>NOTE:</b> Invoke {@code addInline} <i>after</i> {@code setText}; else, mail readers
     * might not be able to resolve inline references correctly.
     *
     * @param contentId the content ID to use. Will end up as "Content-ID" header in the body part,
     *     surrounded by angle brackets: e.g. "myId" -> "&lt;myId&gt;". Can be referenced in HTML
     ...
     * one on each call</i>, as JavaMail will invoke {@code getInputStream()} multiple times.
     *
     * @param attachmentFilename the name of the attachment as it will appear in the mail (the
    ...
    public void addAttachment(
        ...
        DataSource dataSource =
                createDataSource(inputStreamSource, contentType, attachmentFilename);
        addAttachment(attachmentFilename, dataSource);
    }
