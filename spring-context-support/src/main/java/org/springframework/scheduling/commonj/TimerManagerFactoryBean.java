...
public class TimerManagerFactoryBean extends TimerManagerAccessor
     ...
     * creates. Depending on each ScheduledTimerListener's settings, it will be registered via one
     * of TimerManager's schedule methods.
     *
                ...
                if (scheduledTask.isOneTimeTask()) {
                    timer =
                            timerManager.schedule(
                                    scheduledTask.getTimerListener(), scheduledTask.getDelay());
                } else {
    ...
    /**
     * Cancels all statically registered Timers on shutdown, and stops the underlying TimerManager
     * (if not shared).
     *
