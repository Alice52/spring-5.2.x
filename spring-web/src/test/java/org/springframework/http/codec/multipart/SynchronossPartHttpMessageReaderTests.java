...
public class SynchronossPartHttpMessageReaderTests extends AbstractLeakCheckingTests {
    ...
    void resolveParts() {
        ...
        MultiValueMap<String, Part> parts =
                this.reader.readMono(PARTS_ELEMENT_TYPE, request, emptyMap()).block();
    ...
    void bodyError() {
        ...
        StepVerifier.create(this.reader.readMono(PARTS_ELEMENT_TYPE, request, emptyMap()))
                .verifyError();
    }
    ...
    void readPartsWithoutDemand() {
        ServerHttpRequest request = generateMultipartRequest();
        Mono<MultiValueMap<String, Part>> parts =
                this.reader.readMono(PARTS_ELEMENT_TYPE, request, emptyMap());
    ...
    void gh23768() throws IOException {
        ReadableByteChannel channel =
                new ClassPathResource("invalid.multipart", getClass()).readableChannel();
        Flux<DataBuffer> body =
                DataBufferUtils.readByteChannel(() -> channel, this.bufferFactory, 1024);
        ...
        ServerHttpRequest request =
                MockServerHttpRequest.post("/").contentType(contentType).body(body);
    ...
    void readFilePartTooBig() {
        testMultipartExceptions(
                reader -> reader.setMaxDiskUsagePerPart(5),
                ex -> {
                    assertThat(ex)
                            .isInstanceOf(DecodingException.class)
    ...
    void readPartHeadersTooBig() {
        testMultipartExceptions(
                reader -> reader.setMaxInMemorySize(1),
                ex -> {
                    assertThat(ex)
                            .isInstanceOf(DecodingException.class)
                            .hasMessageStartingWith("Failure while parsing part[1]");
    ...
    private void testMultipartExceptions(
        ...
        StepVerifier.create(
                        multipartReader.readMono(
                                PARTS_ELEMENT_TYPE, generateMultipartRequest(), emptyMap()))
