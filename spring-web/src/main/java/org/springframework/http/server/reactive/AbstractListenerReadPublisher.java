...
public abstract class AbstractListenerReadPublisher<T> implements Publisher<T> {
    ...
    private static final AtomicLongFieldUpdater<AbstractListenerReadPublisher>
            DEMAND_FIELD_UPDATER =
                    AtomicLongFieldUpdater.newUpdater(
                            AbstractListenerReadPublisher.class, "demand");
    ...
    /**
     * Check if data is available and either call {@link #onDataAvailable()} immediately or schedule
     * a notification.
     */
    ...
    /**
     * Invoked after an I/O read error from the underlying server or after a cancellation signal
     * from the downstream consumer to allow sub-classes to discard any current cached data they
     * might have.
     *
     * @since 5.0.11
     */
    ...
    /**
     * Read and publish data one at a time until there is no more data, no more demand, or perhaps
     * we completed in the mean time.
     *
    ...
    private void handleCompletionOrErrorBeforeDemand() {
        ...
        if (!state.equals(State.UNSUBSCRIBED) && !state.equals(State.SUBSCRIBING)) {
            ...
            if (ex != null) {
                if (rsReadLogger.isTraceEnabled()) {
                    rsReadLogger.trace(
                            getLogPrefix() + "Completed with error before demand: " + ex);
                }
                this.state.get().onError(this, ex);
            }
        }
    ...
    private enum State {
        ...
        /**
         * Very brief state where we know we have a Subscriber but must not send onComplete and
         * onError until we after onSubscribe.
         */
        SUBSCRIBING {
            @Override
        ...
        DEMAND {
            ...
            <T> void onDataAvailable(AbstractListenerReadPublisher<T> publisher) {
                ...
                // Else, either competing onDataAvailable (request vs container), or concurrent
                // completion
            }
        },
