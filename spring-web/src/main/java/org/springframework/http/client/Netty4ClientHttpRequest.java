...
class Netty4ClientHttpRequest extends AbstractAsyncClientHttpRequest implements ClientHttpRequest {
    ...
    protected ListenableFuture<ClientHttpResponse> executeInternal(final HttpHeaders headers)
        ...
        this.bootstrap
                .connect(this.uri.getHost(), getPort(this.uri))
    ...
    private FullHttpRequest createFullHttpRequest(HttpHeaders headers) {
        ...
        String path =
                this.uri
                        .toString()
                        .substring(this.uri.toString().indexOf(authority) + authority.length());
        ...
        if (!nettyRequest.headers().contains(HttpHeaders.CONTENT_LENGTH)
            ...
            nettyRequest
                    .headers()
                    .set(HttpHeaders.CONTENT_LENGTH, this.body.buffer().readableBytes());
        }
