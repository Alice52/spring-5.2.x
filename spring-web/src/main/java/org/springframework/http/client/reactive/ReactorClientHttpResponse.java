...
class ReactorClientHttpResponse implements ClientHttpResponse {
    ...
    /**
     * Constructor that matches the inputs from {@link
     * reactor.netty.http.client.HttpClient.ResponseReceiver#responseConnection(BiFunction)}.
     *
     * @since 5.2.8
     */
     ...
     *
     * @deprecated as of 5.2.8, in favor of {@link #ReactorClientHttpResponse(HttpClientResponse,
     *     Connection)}
     */
    @Deprecated
        ...
        this.response.cookies().values().stream()
                ...
                .forEach(
                        c ->
                                result.add(
                                        c.name(),
                                        ResponseCookie.fromClientResponse(c.name(), c.value())
                                                .domain(c.domain())
                                                .path(c.path())
                                                .maxAge(c.maxAge())
                                                .secure(c.isSecure())
    ...
    /**
     * Called by {@link ReactorClientHttpConnector} when a cancellation is detected but the content
     * has not been subscribed to. If the subscription never materializes then the content will
     * remain not drained. Or it could still materialize if the cancellation happened very early, or
     * the response reading was delayed for some reason.
     */
