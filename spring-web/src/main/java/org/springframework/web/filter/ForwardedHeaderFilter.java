...
public class ForwardedHeaderFilter extends OncePerRequestFilter {
    ...
    /**
     * Use this property to enable relative redirects as explained in {@link
     * RelativeRedirectFilter}, and also using the same response wrapper as that filter does, or if
     * both are configured, only one will wrap.
     *
    ...
    protected void doFilterInternal(
        ...
        if (this.removeOnly) {
            ForwardedHeaderRemovingRequest wrappedRequest =
                    new ForwardedHeaderRemovingRequest(request);
            ...
            HttpServletResponse wrappedResponse =
                    this.relativeRedirects
                            ? RelativeRedirectResponseWrapper.wrapIfNecessary(
    ...
    /**
     * Responsible for the contextPath, requestURI, and requestURL with forwarded headers in mind,
     * and also taking into account changes to the path of the underlying delegate request (e.g. on
     * a Servlet FORWARD).
     */
    private static class ForwardedPrefixExtractor {
         ...
         *
         * @param delegateRequest supplier for the current {@link
         *     HttpServletRequestWrapper#getRequest() delegate request} which may change during a
         *     forward (e.g. Tomcat.
        ...
        private String initRequestUrl() {
            return this.baseUrl
                    + (this.requestUri != null
                            ? this.requestUri
                            : this.delegate.get().getRequestURI());
        }
                ...
                path =
                        (path.startsWith(FOLDER_SEPARATOR)
                                ...
                                : StringUtils.applyRelativePath(
                                        this.request.getRequestURI(), path));
            }
