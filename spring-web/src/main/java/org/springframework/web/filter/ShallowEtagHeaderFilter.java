...
public class ShallowEtagHeaderFilter extends OncePerRequestFilter {
     ...
     * ShallowEtagHeaderFilter. The main reason for this is streaming scenarios which are not to be
     * cached and do not need an eTag.
     *
     * <p><strong>Note:</strong> This method must be called before the response is written to in
     * order for the entire response content to be written without caching.
     *
     * @since 4.2
     */
     ...
     *
     * <p>Should be configured using an {@code <init-param>} for parameter name "writeWeakETag" in
    ...
    /**
     * The default value is {@code false} so that the filter may delay the generation of an ETag
     * until the last asynchronously dispatched thread.
     */
    @Override
    ...
    protected void doFilterInternal(
        ...
        if (!isAsyncDispatch(request)
                && !(response instanceof ConditionalContentCachingResponseWrapper)) {
    ...
    private void updateResponse(HttpServletRequest request, HttpServletResponse response)
        ...
        ConditionalContentCachingResponseWrapper wrapper =
                WebUtils.getNativeResponse(
                        response, ConditionalContentCachingResponseWrapper.class);
        ...
        if (isEligibleForEtag(
                request, wrapper, wrapper.getStatus(), wrapper.getContentInputStream())) {
    ...
    /**
     * Whether an ETag should be calculated for the given request and response exchange. By default
     * this is {@code true} if all of the following match:
     *
     * <ul>
     *   <li>Response is not committed.
     *   <li>Response status codes is in the {@code 2xx} series.
     *   <li>Request method is a GET.
     *   <li>Response Cache-Control header does not contain "no-store" (or is not present at all).
     * </ul>
     *
    ...
    /**
     * Returns the raw OutputStream, instead of the one that does caching, if {@link
