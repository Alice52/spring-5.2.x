...
public class HandlerMethod {
    ...
    private void evaluateResponseStatus() {
        ...
        if (annotation == null) {
            annotation =
                    AnnotatedElementUtils.findMergedAnnotation(getBeanType(), ResponseStatus.class);
        }
        if (annotation != null) {
    ...
    private List<Annotation[][]> getInterfaceParameterAnnotations() {
        ...
        if (parameterAnnotations == null) {
            ...
            for (Class<?> ifc :
                    ClassUtils.getAllInterfacesForClassAsSet(this.method.getDeclaringClass())) {
        ...
        for (int i = 0; i < paramTypes.length; i++) {
            if (paramTypes[i]
                    != ResolvableType.forMethodParameter(
                                    candidate, i, this.method.getDeclaringClass())
                            .resolve()) {
                return false;
            }
        }
     ...
     * declared. In some cases the actual controller instance at request- processing time may be a
     * JDK dynamic proxy (lazy initialization, prototype beans, and others). {@code @Controller}'s
     * that require proxying should prefer class-based proxy mechanisms.
     */
                        ...
                        if (index < ifcAnns.length) {
                            ...
                            if (paramAnns.length > 0) {
                                List<Annotation> merged =
                                        new ArrayList<>(anns.length + paramAnns.length);
