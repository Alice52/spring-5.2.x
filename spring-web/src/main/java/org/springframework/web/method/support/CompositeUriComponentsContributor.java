...
/**
 * A {@link UriComponentsContributor} containing a list of other contributors to delegate to and
 * also encapsulating a specific {@link ConversionService} to use for formatting method argument
 * values as Strings.
 *
...
public class CompositeUriComponentsContributor implements UriComponentsContributor {
     ...
     * HandlerMethodArgumentResolvers}. Since both of these tend to be implemented by the same
     * class, the most convenient option is to obtain the configured {@code
     * HandlerMethodArgumentResolvers} in {@code RequestMappingHandlerAdapter} and provide that to
     * this constructor.
     *
     ...
     * HandlerMethodArgumentResolvers}. Since both of these tend to be implemented by the same
     * class, the most convenient option is to obtain the configured {@code
     * HandlerMethodArgumentResolvers} in the {@code RequestMappingHandlerAdapter} and provide that
     * to this constructor.
     *
     * <p>If the {@link ConversionService} argument is {@code null}, {@link
     * org.springframework.format.support.DefaultFormattingConversionService} will be used by
     * default.
     *
     * @param contributors a collection of {@link UriComponentsContributor} or {@link
     *     HandlerMethodArgumentResolver HandlerMethodArgumentResolvers}
     ...
     *
     * @return {@code true} if this {@code CompositeUriComponentsContributor} was created with
     *     contributors to delegate to
     */
        ...
        for (Object contributor : this.contributors) {
            if (contributor instanceof UriComponentsContributor) {
                ...
                if (ucc.supportsParameter(parameter)) {
                    ucc.contributeMethodArgument(
                            parameter, value, builder, uriVariables, conversionService);
                    break;
                }
            } else if (contributor instanceof HandlerMethodArgumentResolver) {
                if (((HandlerMethodArgumentResolver) contributor).supportsParameter(parameter)) {
