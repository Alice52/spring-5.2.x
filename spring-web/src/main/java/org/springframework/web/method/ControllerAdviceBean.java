...
public class ControllerAdviceBean implements Ordered {
     ...
     * @param beanFactory a {@code BeanFactory} to retrieve the bean type initially and later to
     *     resolve the actual bean
     * @param controllerAdvice the {@code @ControllerAdvice} annotation for the bean, or {@code
     ...
     *
     * <p>As of Spring Framework 5.2, the {@code ControllerAdviceBean} instances in the returned
     * list are sorted using {@link OrderComparator#sort(List)}.
     *
     * @see #getOrder()
     * @see OrderComparator
     * @see Ordered
     ...
     * algorithm and cached. Note, however, that a {@link ControllerAdvice @ControllerAdvice} bean
     * that is configured as a scoped bean &mdash; for example, as a request-scoped or
     * session-scoped bean &mdash; will not be eagerly resolved. Consequently, {@link Ordered} is
     * not honored for scoped {@code @ControllerAdvice} beans.
     *
     * <ul>
     *   <li>If the {@linkplain #resolveBean resolved bean} implements {@link Ordered}, use the
     *       value returned by {@link Ordered#getOrder()}.
     *   <li>If the {@linkplain #getBeanType() bean type} is known, use the value returned by {@link
     *       OrderUtils#getOrder(Class, int)} with {@link Ordered#LOWEST_PRECEDENCE} used as the
     *       default order value.
     *   <li>Otherwise use {@link Ordered#LOWEST_PRECEDENCE} as the default, fallback order value.
     * </ul>
     *
     * @see #resolveBean()
     */
     ...
     *
     * <p>As of Spring Framework 5.2, once the bean instance has been resolved it will be cached if
     * it is a singleton, thereby avoiding repeated lookups in the {@code BeanFactory}.
     */
