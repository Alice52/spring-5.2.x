...
public class CorsConfiguration {
     ...
     * client-originated address. Consider using the {@code ForwardedHeaderFilter} in order to
     * choose from a central place whether to extract and use, or to discard such headers. See the
    ...
    /**
     * Set the list of headers that a pre-flight request can list as allowed for use during an
     * actual request.
     *
     ...
     *
     * <p>Use this method to flip the initialization model to start with open defaults that permit
     * all cross-origin requests for GET, HEAD, and POST requests. Note however that this method
     * will not override any existing values already set.
     *
     ...
     *
     * <p>Combining lists like {@code allowedOrigins}, {@code allowedMethods}, {@code
     * allowedHeaders} or {@code exposedHeaders} is done in an additive way. For example, combining
     * {@code ["GET", "POST"]} with {@code ["PATCH"]} results in {@code ["GET", "POST", "PATCH"]},
     ...
     * @return the list of allowed headers to list in the response of a pre-flight request, or
     *     {@code null} if none of the supplied request headers is allowed
     */
    @Nullable
