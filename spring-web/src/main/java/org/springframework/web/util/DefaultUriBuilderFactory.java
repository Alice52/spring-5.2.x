...
public class DefaultUriBuilderFactory implements UriBuilderFactory {
     ...
     * applied as a base URI to every UriBuilder via {@link UriComponentsBuilder#uriComponents}
     * unless the UriBuilder itself was created with a URI template that already has a target
     * address.
     *
     ...
     *
     * <p><strong>Note:</strong> Prior to 5.1 the default was {@link EncodingMode#URI_COMPONENT
     * EncodingMode.URI_COMPONENT} therefore the {@code WebClient} {@code RestTemplate} have
     * switched their default behavior.
     *
     ...
     * EncodingMode#URI_COMPONENT EncodingMode.URI_COMPONENT}, which ensures that URI variables in
     * the path are encoded according to path segment rules and for example a '/' is encoded.
     *
    ...
    public enum EncodingMode {
        ...
        /**
         * Pre-encode the URI template first, then strictly encode URI variables when expanded, with
         * the following rules:
         *
         * <ul>
         *   <li>For the URI template replace <em>only</em> non-ASCII and illegal (within a given
         *       URI component type) characters with escaped octets.
         *   <li>For URI variables do the same and also replace characters with reserved meaning.
         * </ul>
         *
         * <p>For most cases, this mode is most likely to give the expected result because in treats
         * URI variables as opaque data to be fully encoded, while {@link #URI_COMPONENT} by
         * comparison is useful only if intentionally expanding URI variables with reserved
         * characters.
         *
         * @since 5.0.8
         ...
         * <em>only</em> non-ASCII and illegal (within a given URI component type) characters, but
         * not characters with reserved meaning.
         *
    ...
    private class DefaultUriBuilder implements UriBuilder {
        ...
        private UriComponentsBuilder initUriComponentsBuilder(String uriTemplate) {
            ...
            if (!StringUtils.hasLength(uriTemplate)) {
                ...
                result =
                        (uri.getHost() == null
                                ? baseUri.cloneBuilder().uriComponents(uri)
                                : builder);
            } else {
