...
class InternalPathPatternParser {
    ...
    public PathPattern parse(String pathPattern) throws PatternParseException {
        ...
        while (this.pos < this.pathPatternLength) {
            ...
            if (ch == separator) {
                ...
                // Check that the characters used for captured variable names are like java
                // identifiers
                if (this.insideVariableCapture) {
                    if ((this.variableCaptureStart + 1 + (this.isCaptureTheRestVariable ? 1 : 0))
                                    == this.pos
                        ...
                        throw new PatternParseException(
                                ...
                                Character.toString(ch));
                                    ...
                                    > (this.variableCaptureStart
                                            + 1
                                            + (this.isCaptureTheRestVariable ? 1 : 0))
    ...
    private boolean peekDoubleWildcard() {
        ...
        if (this.pathPatternData[this.pos + 1] != '*'
                || this.pathPatternData[this.pos + 2] != '*') {
            return false;
        }
        ...
        if (newPathElement instanceof CaptureTheRestPathElement) {
            ...
            if (this.currentPE == null) {
                ...
                throw new IllegalStateException(
                        "Expected SeparatorPathElement but was " + this.currentPE);
            }
        } else {
            if (this.headPE == null) {
                this.headPE = newPathElement;
            ...
            if (this.variableCaptureCount == 1
                ...
                if (this.isCaptureTheRestVariable) {
                    ...
                    try {
                        ...
                        throw new PatternParseException(
                                pse,
                                findRegexStart(this.pathPatternData, this.pathElementStart)
                                        + pse.getIndex(),
                                this.pathPatternData,
     ...
     * @return the index of the character after the ':' within the pattern expression relative to
