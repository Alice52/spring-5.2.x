...
public abstract class AbstractEncoderMethodReturnValueHandler
    ...
    private Flux<DataBuffer> encodeContent(
        ...
        ReactiveAdapter adapter =
                getAdapterRegistry().getAdapter(returnValueType.resolve(), content);
        ...
        if (adapter != null) {
            ...
            boolean isUnwrapped =
                    KotlinDetector.isKotlinReflectPresent()
                            ...
                            && !COROUTINES_FLOW_CLASS_NAME.equals(
                                    returnValueType.toClass().getName());
            ResolvableType genericType =
                    isUnwrapped ? returnValueType : returnValueType.getGeneric();
    ...
    /**
     * Invoked for a {@code null} return value, which could mean a void method or method returning
     * an async type parameterized by void.
     *
     * @param returnType return type of the handler method that produced the data
     * @param message the input message handled by the handler method
     * @return completion {@code Mono<Void>} for the handling
