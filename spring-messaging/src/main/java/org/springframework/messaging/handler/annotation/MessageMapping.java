 ...
 *
 * <ul>
 *   <li>STOMP over WebSocket -- the value is turned into a message and sent to a default response
 *       destination or to a custom destination specified with an {@link SendTo @SendTo} or {@link
 *       org.springframework.messaging.simp.annotation.SendToUser @SendToUser} annotation.
 *   <li>RSocket -- the response is used to reply to the stream request.
 * </ul>
 *
 * <p>Specializations of this annotation include {@link
 * org.springframework.messaging.simp.annotation.SubscribeMapping @SubscribeMapping} (e.g. STOMP
 * subscriptions) and {@link
 * org.springframework.messaging.rsocket.annotation.ConnectMapping @ConnectMapping} (e.g. RSocket
 * connections). Both narrow the primary mapping further and also match against the message type.
 * Both can be combined with a type-level {@code @MessageMapping} that declares a common pattern
 * prefix (or prefixes).
 *
 * <p>For further details on the use of this annotation in different contexts, see the following
 * sections of the Spring Framework reference:
 *
 * <ul>
 *   <li>STOMP over WebSocket <a
 *       href="https://docs.spring.io/spring/docs/current/spring-framework-reference/web.html#websocket-stomp-handle-annotations">
...
public @interface MessageMapping {
     ...
     *
     * <p>for RSocket this is either {@link org.springframework.util.AntPathMatcher AntPathMatcher}
     * or {@link org.springframework.web.util.pattern.PathPattern PathPattern} based pattern,
     * depending on which is configured, matched to the route of the stream request.
     *
