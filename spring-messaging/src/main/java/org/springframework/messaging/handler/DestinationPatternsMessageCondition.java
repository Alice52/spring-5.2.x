...
public class DestinationPatternsMessageCondition
    ...
    public DestinationPatternsMessageCondition(String[] patterns, RouteMatcher routeMatcher) {
        this(
                Collections.unmodifiableSet(prependLeadingSlash(patterns, routeMatcher)),
                routeMatcher);
    }
     ...
     *       patterns in "other" using {@link org.springframework.util.PathMatcher#combine(String,
     *       String)}.
    ...
    /**
     * Check if any of the patterns match the given Message destination and return an instance that
     * is guaranteed to contain matching patterns, sorted via {@link
     ...
     *     sorted matching patterns; or {@code null} either if a destination can not be extracted or
     *     there is no match
     */
    @Override
    @Nullable
    public DestinationPatternsMessageCondition getMatchingCondition(Message<?> message) {
        ...
        return new DestinationPatternsMessageCondition(
                new LinkedHashSet<>(matches), this.routeMatcher);
    }
     ...
     * #getMatchingCondition(Message)} to ensure they contain only patterns that match the request
