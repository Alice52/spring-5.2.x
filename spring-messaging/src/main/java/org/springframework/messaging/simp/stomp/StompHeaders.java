...
public class StompHeaders implements MultiValueMap<String, String>, Serializable {
    ...
    public void setContentType(@Nullable MimeType mimeType) {
        if (mimeType != null) {
            Assert.isTrue(
                    !mimeType.isWildcardType(), "'Content-Type' cannot contain wildcard type '*'");
            Assert.isTrue(
                    !mimeType.isWildcardSubtype(),
        ...
        Arrays.stream(acceptVersions)
                .forEach(
                        version ->
                                Assert.isTrue(
                                        version != null
                                                && (version.equals("1.1") || version.equals("1.2")),
    ...
    /**
     * Whether heartbeats are enabled. Returns {@code false} if {@link #setHeartbeat} is set to
    ...
    /**
     * Set the ack header to one of "auto", "client", or "client-individual". Applies to the
     * SUBSCRIBE and MESSAGE frames.
     */
