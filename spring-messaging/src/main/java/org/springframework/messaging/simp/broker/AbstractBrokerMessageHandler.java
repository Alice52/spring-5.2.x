...
public abstract class AbstractBrokerMessageHandler
    ...
    public void setPreservePublishOrder(boolean preservePublishOrder) {
        OrderedMessageSender.configureOutboundChannel(
                this.clientOutboundChannel, preservePublishOrder);
     ...
     * may still independently alternate between being on and off depending on the concrete
     ...
     *
     * <p>Note that this is in addition to the {@link #isRunning()} flag, which indicates whether
     * this message handler is running. In other words the message handler must first be running and
     * then the {@code #isBrokerAvailable()} flag may still independently alternate between being on
     * and off depending on the concrete sub-class implementation.
     *
