...
public class StompBrokerRelayMessageHandler extends AbstractBrokerMessageHandler {
    ...
    /**
     * Heartbeat starts once CONNECTED frame with heartbeat settings is received. If CONNECTED
     * doesn't arrive within a minute, we'll close the connection.
     */
     ...
     *
     * @param inboundChannel the channel for receiving messages from clients (e.g. WebSocket
     *     clients)
     * @param outboundChannel the channel for sending messages to clients (e.g. WebSocket clients)
     * @param brokerChannel the channel for the application to send messages to the broker
     * @param destinationPrefixes the broker supported destination prefixes; destinations that do
     *     not match the given prefix are ignored.
     */
    ...
    /**
     * Set the client passcode to use to create connections to the STOMP broker on behalf of
     * connected clients.
     *
     * <p>By default this is set to "guest".
     *
    ...
    /**
     * Set the login for the shared "system" connection used to send messages to the STOMP broker
     * from within the application, i.e. messages not associated with a specific client session
     * (e.g. REST/HTTP request handling method).
     *
     * <p>By default this is set to "guest".
     */
    ...
    /**
     * Return the interval, in milliseconds, at which the "system" connection will send heartbeats
     * to the STOMP broker.
     */
    ...
    /**
     * Set the interval, in milliseconds, at which the "system" connection will, in the absence of
    ...
    /**
     * Get the configured TCP client (never {@code null} unless not configured invoked and this
     * method is invoked before the handler is started and hence a default implementation
     * initialized).
     */
    @Nullable
     ...
     *
     * <p><strong>Note:</strong> when this property is used, any {@link #setRelayHost(String) host}
     * or {@link #setRelayPort(int) port} specified are effectively ignored.
     */
     ...
     * through the {@code StompBrokerRelayMessageHandler} that are sent to the client outbound
     * message channel.
     *
     * <p>By default this property is not set.
     */
    ...
    protected void startInternal() {
        ...
        accessor.setHeartbeat(
                this.systemHeartbeatSendInterval, this.systemHeartbeatReceiveInterval);
    ...
    protected void handleMessageInternal(Message<?> message) {
        ...
        if (sessionId == null) {
            if (!SimpMessageType.MESSAGE.equals(stompAccessor.getMessageType())) {
                if (logger.isErrorEnabled()) {
                    logger.error(
                            "Only STOMP SEND supported from within the server side. Ignoring "
                                    + message);
                }
                return;
            }
        ...
        if (command != null
                && command.requiresDestination()
                && !checkDestinationPrefix(destination)) {
            return;
        }
        ...
        if (StompCommand.CONNECT.equals(command) || StompCommand.STOMP.equals(command)) {
            ...
            if (handler == null) {
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "Ignoring DISCONNECT in session "
                                    + sessionId
    ...
    private class StompConnectionHandler implements TcpConnectionHandler<byte[]> {
        ...
        public void afterConnected(TcpConnection<byte[]> connection) {
                    ...
                    () -> {
                        if (this.tcpConnection != null && !this.isStompConnected) {
                            handleTcpConnectionFailure(
                                    "No CONNECTED frame received in "
            ...
            connection.send(
                    MessageBuilder.createMessage(
                            EMPTY_PAYLOAD, this.connectHeaders.getMessageHeaders()));
        }
        ...
        protected void handleTcpConnectionFailure(String error, @Nullable Throwable ex) {
            ...
            try {
                ...
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Failure while clearing TCP connection state in session "
                                        + this.sessionId,
                                ex2);
                    }
                }
        ...
        /**
         * Invoked after the STOMP CONNECTED frame is received. At this point the connection is
         * ready for sending STOMP messages to the broker.
         */
        ...
        private void initHeartbeats(StompHeaderAccessor connectedHeaders) {
            ...
            if (clientReceiveInterval > 0 && serverSendInterval > 0) {
                ...
                con.onReadInactivity(
                        () ->
                                handleTcpConnectionFailure(
                                        "No messages received in " + interval + " ms.", null),
                        interval);
            }
        }
         ...
         * CONNECTED frame. For client messages this should be false only if we lose the TCP
         * connection around the same time when a client message is being forwarded, so we simply
         * log the ignored message at debug level. For messages from within the application being
         * sent on the "system" connection an exception is raised so that components sending the
         * message have a chance to handle it -- by default the broker message channel is
         * synchronous.
         *
         * <p>Note that if messages arrive concurrently around the same time a TCP connection is
         * lost, there is a brief period of time before the connection is reset when one or more
         * messages may sneak through and an attempt made to forward them. Rather than synchronizing
         * to guard against that, this method simply lets them try and fail. For client sessions
         * that may result in an additional STOMP ERROR frame(s) being sent downstream but code
         * handling that downstream should be idempotent in such cases.
         *
         ...
         * pro-actively. However, if the DISCONNECT has a receipt header we leave the connection
         * open and expect the server will respond with a RECEIPT and then close the connection.
         *
    ...
    static {
        ...
        HEARTBEAT_MESSAGE =
                MessageBuilder.createMessage(
                        StompDecoder.HEARTBEAT_PAYLOAD, accessor.getMessageHeaders());
    }
}
