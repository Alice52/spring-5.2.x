...
public class GenericMessagingTemplate
     ...
     * isn't going to receive the reply either because it timed out, or because it already received
     * a reply, or because it got an exception while sending the request message.
     *
    ...
    protected final Message<?> doReceive(MessageChannel channel, long timeout) {
        Assert.notNull(channel, "MessageChannel is required");
        Assert.state(
                channel instanceof PollableChannel,
                "A PollableChannel is required to receive messages");
    ...
    private long receiveTimeout(Message<?> requestMessage) {
        Long receiveTimeout =
                headerToLong(requestMessage.getHeaders().get(this.receiveTimeoutHeader));
