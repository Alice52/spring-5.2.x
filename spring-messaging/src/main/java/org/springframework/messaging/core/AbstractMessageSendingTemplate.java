...
public abstract class AbstractMessageSendingTemplate<D> implements MessageSendingOperations<D> {
    ...
    /**
     * Convert the given Object to serialized form, possibly using a {@link MessageConverter}, wrap
     * it as a message with the given headers and apply the given post processor.
     *
        ...
        Message<?> message =
                (converter instanceof SmartMessageConverter
                        ? ((SmartMessageConverter) converter)
                                .toMessage(payload, messageHeaders, conversionHint)
                        : converter.toMessage(payload, messageHeaders));
        if (message == null) {
            String payloadType = payload.getClass().getName();
            Object contentType =
                    (messageHeaders != null
                            ? messageHeaders.get(MessageHeaders.CONTENT_TYPE)
                            : null);
    ...
    /**
     * Provides access to the map of input headers before a send operation. Subclasses can modify
     * the headers and then return the same or a different map.
     *
