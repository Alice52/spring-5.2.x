...
/**
 * Stores registrations of extractors for metadata entries. Each metadata entry is decoded based on
 * its {@code MimeType} and a name is assigned to the decoded value.
 *
...
public interface MetadataExtractorRegistry {
     ...
     * @param name assign a name for the decoded value; if not provided, then the mime type is used
     *     as the key
     */
     ...
     * ParameterizedTypeReference} instead of {@link Class} for specifying a target type with
     * generic parameters.
     *
     * @param mimeType the mime type of metadata entries to extract
     * @param targetType the target value type to decode to
