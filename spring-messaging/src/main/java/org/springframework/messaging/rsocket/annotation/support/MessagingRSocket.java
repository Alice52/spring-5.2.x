...
class MessagingRSocket implements RSocket {
                ...
                .switchOnFirst(
                        (signal, innerFlux) -> {
                            ...
                            return firstPayload == null
                                    ? innerFlux
                                    : handleAndReply(
                                            firstPayload, FrameType.REQUEST_CHANNEL, innerFlux);
                        });
    }
    ...
    private DataBuffer retainDataAndReleasePayload(Payload payload) {
        return PayloadUtils.retainDataAndReleasePayload(
                payload, this.strategies.dataBufferFactory());
    }
    ...
    private MessageHeaders createHeaders(
        ...
        for (Map.Entry<String, Object> entry : metadataValues.entrySet()) {
            if (entry.getKey().equals(MetadataExtractor.ROUTE_KEY)) {
                ...
                headers.setHeader(
                        DestinationPatternsMessageCondition.LOOKUP_DESTINATION_HEADER, route);
            } else {
