...
class RSocketBufferLeakTests {
                        ...
                        .interceptors(
                                registry ->
                                        registry.forResponder(
                                                payloadInterceptor)) // intercept responding
                ...
                RSocketRequester.builder()
                        .rsocketConnector(
                                conn ->
                                        conn.interceptors(
                                                registry ->
                                                        registry.forRequester(payloadInterceptor)))
    ...
    void assemblyTimeErrorForHandleAndReply() {
        ...
        StepVerifier.create(result)
                .expectErrorMatches(
                        ex -> {
                            String prefix =
                                    "Ambiguous handler methods mapped for destination 'A.B':";
    ...
    void subscriptionTimeErrorForHandleAndReply() {
        Mono<String> result =
                requester.route("not-decodable").data("foo").retrieveMono(String.class);
        StepVerifier.create(result)
                .expectErrorMatches(
                        ex -> {
                            String prefix =
                                    "Cannot decode to [org.springframework.core.io.Resource]";
    ...
    void errorSignalWithExceptionHandler() {
        Flux<String> result =
                requester.route("error-signal").data("foo").retrieveFlux(String.class);
        StepVerifier.create(result)
    ...
    void noSuchRouteOnChannelInteraction() {
        ...
        Flux<String> result =
                requester.route("no-such-route").data(input).retrieveFlux(String.class);
        StepVerifier.create(result)
        ...
        private void checkForLeak(PayloadLeakInfo info) {
            ...
            while (true) {
                try {
                    ...
                    assertThat(count == 0)
                            .as("Leaked payload (refCnt=" + count + "): " + info)
                            .isTrue();
                    break;
        ...
        private static class PayloadSavingDecorator extends AbstractRSocket {
            ...
            public Mono<io.rsocket.Payload> requestResponse(io.rsocket.Payload payload) {
                return this.delegate
                        .requestResponse(addPayload(payload))
                        .doOnSuccess(this::addPayload);
            }
