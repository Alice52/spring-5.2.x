...
public class FormTag extends AbstractHtmlElementTag {
    ...
    /**
     * Writes the opening part of the block '{@code form}' tag and exposes the form object name in
    ...
    protected int writeTagContent(TagWriter tagWriter) throws JspException {
        ...
        if (!isMethodBrowserSupported(getMethod())) {
            ...
            writeOptionalAttribute(
                    tagWriter,
                    VALUE_ATTRIBUTE,
                    processFieldValue(inputName, getMethod(), inputType));
            tagWriter.endTag();
        }
        ...
        this.previousNestedPath =
                (String)
                        this.pageContext.getAttribute(
                                NESTED_PATH_VARIABLE_NAME, PageContext.REQUEST_SCOPE);
        this.pageContext.setAttribute(
     ...
     *
     * <p>If the user configured an '{@code action}' value then the result of evaluating this value
     * is used. If the user configured an '{@code servletRelativeAction}' value then the value is
    ...
    /**
     * Process the action through a {@link RequestDataValueProcessor} instance if one is configured
     * or otherwise returns the action unmodified.
     */
