...
public class RequestContext {
    ...
    /**
     * Create a new RequestContext for the given request, using the given model attributes for
     * Errors retrieval.
     *
     * <p>This works with all View implementations. It will typically be used by View
     * implementations.
     *
     * <p><b>Will only work within a DispatcherServlet request.</b> Pass in a ServletContext to be
     * able to fallback to the root WebApplicationContext.
     *
     * @param request current HTTP request
     * @param model the model attributes for the current view (can be {@code null}, using the
     *     request attributes for Errors retrieval)
     * @see org.springframework.web.servlet.DispatcherServlet
     * @see #RequestContext(javax.servlet.http.HttpServletRequest,
     *     javax.servlet.http.HttpServletResponse, javax.servlet.ServletContext, Map)
     */
     ...
     *
     * <p>Typically coming from a DispatcherServlet's {@link LocaleResolver}. Also includes a
    ...
    protected TimeZone getFallbackTimeZone() {
        if (jstlPresent) {
            TimeZone timeZone =
                    JstlLocaleResolver.getJstlTimeZone(getRequest(), getServletContext());
            if (timeZone != null) {
                return timeZone;
            }
        }
    ...
    /**
     * Change the current locale to the specified locale and time zone context, storing the new
     * locale context through the configured {@link LocaleResolver}.
     *
     * @param locale the new locale
    ...
    /**
     * Change the current theme to the specified theme by name, storing the new theme name through
     ...
     * This is particularly useful for building an HTML form action target in combination with the
    ...
    public String getThemeMessage(String code, String defaultMessage) {
        String msg =
                getTheme().getMessageSource().getMessage(code, null, defaultMessage, getLocale());
        return (msg != null ? msg : "");
    }
    ...
    public String getThemeMessage(String code, @Nullable Object[] args, String defaultMessage) {
        String msg =
                getTheme().getMessageSource().getMessage(code, args, defaultMessage, getLocale());
        return (msg != null ? msg : "");
    }
     ...
     * @throws org.springframework.context.NoSuchMessageException if not found
     */
    public String getThemeMessage(String code, @Nullable List<?> args)
            throws NoSuchMessageException {
        return getTheme()
                .getMessageSource()
    ...
    /**
     * Retrieve the Errors instance for the given bind object, using the "defaultHtmlEscape"
     * setting.
     *
