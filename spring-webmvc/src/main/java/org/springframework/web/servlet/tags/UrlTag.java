...
public class UrlTag extends HtmlEscapingAwareTag implements ParamAware {
    ...
    /**
     * Build the query string from available parameters that have not already been applied as
     * template params.
     *
     ...
     * @param includeQueryStringDelimiter true if the query string should start with a '?' instead
     *     of '&'
     ...
     */
    protected String replaceUriTemplateParams(
            String uri, List<Param> params, Set<String> usedParams) throws JspException {
                    ...
                    try {
                        uri =
                                StringUtils.replace(
                                        ...
                                        template,
                                        (value != null
                                                ? UriUtils.encodePathSegment(value, encoding)
                                                : ""));
                    } catch (UnsupportedCharsetException ex) {
                        throw new JspException(ex);
                    }
