...
public abstract class HtmlEscapingAwareTag extends RequestContextAwareTag {
    ...
    /**
     * Return the applicable default for the use of response encoding with HTML escaping for this
     * tag.
     *
     * <p>The default implementation checks the RequestContext's setting, falling back to {@code
     * false} in case of no explicit default given.
     ...
     *
     * <p>The response encoding will be taken into account if the "responseEncodedHtmlEscape"
            ...
            if (isResponseEncodedHtmlEscape()) {
                out =
                        HtmlUtils.htmlEscape(
                                content, this.pageContext.getResponse().getCharacterEncoding());
            } else {
