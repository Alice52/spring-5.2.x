...
public abstract class AbstractHandshakeHandler implements HandshakeHandler, Lifecycle {
     ...
     * sub-protocol that matches a client-requested sub-protocol is accepted. If there are no
     * matches the response will not contain a {@literal Sec-WebSocket-Protocol} header.
     *
    ...
    public final boolean doHandshake(
        ...
        try {
            if (HttpMethod.GET != request.getMethod()) {
                ...
                if (logger.isErrorEnabled()) {
                    logger.error(
                            "Handshake failed due to unexpected HTTP method: "
                                    + request.getMethod());
                }
                return false;
            }
        ...
        List<WebSocketExtension> extensions =
                filterRequestedExtensions(request, requested, supported);
        Principal user = determineUser(request, wsHandler, attributes);

        if (logger.isTraceEnabled()) {
            logger.trace(
                    "Upgrading to WebSocket, subProtocol="
                            + subProtocol
                            + ", extensions="
                            + extensions);
        }
    ...
    protected void handleInvalidUpgradeHeader(
            ServerHttpRequest request, ServerHttpResponse response) throws IOException {
        if (logger.isErrorEnabled()) {
            logger.error(
    ...
    /**
     * Return whether the request {@code Origin} header value is valid or not. By default, all
     * origins as considered as valid. Consider using an {@link OriginHandshakeInterceptor} for
     * filtering origins if needed.
     */
