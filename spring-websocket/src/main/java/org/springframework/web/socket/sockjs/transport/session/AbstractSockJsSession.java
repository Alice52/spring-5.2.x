...
public abstract class AbstractSockJsSession implements SockJsSession {
     ...
     *
     * <p>Servlet containers don't expose a client disconnected callback; see <a
     ...
     *
     * <p>We make a best effort to identify such network failures, on a per-server basis, and log
     * them under a separate log category. A simple one-line message is logged at DEBUG level, while
     * a full stack trace is shown at TRACE level.
     *
        ...
        switch (messages.length - i) {
            ...
            case 1:
                return (messages[i].trim().isEmpty()
                        ...
                        : Collections.singletonList(messages[i]));
            default:
        ...
        if (isOpen()) {
            ...
            try {
                if (isActive() && !CloseStatus.SESSION_NOT_RELIABLE.equals(status)) {
                    try {
                        writeFrameInternal(
                                SockJsFrame.closeFrame(status.getCode(), status.getReason()));
                    } catch (Throwable ex) {
    ...
    protected void scheduleHeartbeat() {
        ...
        synchronized (this.responseLock) {
            ...
            this.heartbeatFuture =
                    this.config.getTaskScheduler().schedule(this.heartbeatTask, time);
     ...
     * next request to come through. During this "downtime" the session is still open but inactive
     * and unable to send messages and therefore has to buffer them temporarily. A WebSocket session
     * by contrast is stateful and remain active until closed.
     */
    public abstract boolean isActive();

    /**
     * Actually close the underlying WebSocket session or in the case of HTTP transports complete
