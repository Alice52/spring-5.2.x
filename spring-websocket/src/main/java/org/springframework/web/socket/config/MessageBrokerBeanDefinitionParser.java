...
class MessageBrokerBeanDefinitionParser implements BeanDefinitionParser {
    ...
    public BeanDefinition parse(Element element, ParserContext context) {
        ...
        Object userDestHandler =
                registerUserDestHandler(
                        element, userRegistry, inChannel, brokerChannel, context, source);
        ...
        for (Element endpointElem : DomUtils.getChildElementsByTagName(element, "stomp-endpoint")) {
            ...
            for (String path : StringUtils.tokenizeToStringArray(pathAttribute, ",")) {
                path = path.trim();
                Assert.hasText(
                        path, () -> "Invalid <stomp-endpoint> path attribute: " + pathAttribute);
        ...
        Map<String, Object> scopeMap =
                Collections.singletonMap("websocket", new SimpSessionScope());
    ...
    private ManagedMap<String, Object> registerHandlerMapping(
        ...
        RootBeanDefinition handlerMappingDef =
                new RootBeanDefinition(WebSocketHandlerMapping.class);

        String orderAttribute = element.getAttribute("order");
        int order =
                orderAttribute.isEmpty() ? DEFAULT_MAPPING_ORDER : Integer.parseInt(orderAttribute);
    ...
    private RuntimeBeanReference getMessageChannel(
        ...
        if (element != null) {
            Element interceptorsElement =
                    DomUtils.getChildElementByTagName(element, "interceptors");
            interceptors.addAll(
    ...
    private RuntimeBeanReference registerStompHandler(
        ...
        Element errorHandlerElem =
                DomUtils.getChildElementByTagName(element, "stomp-error-handler");
        ...
        if (transportElem != null) {
            ...
            if (factoriesElement != null) {
                ...
                RootBeanDefinition factoryBean =
                        new RootBeanDefinition(DecoratingFactoryBean.class);
    ...
    private RuntimeBeanReference registerRequestHandler(
        ...
        if (sockJsService != null) {
            ...
            Element interceptElem =
                    DomUtils.getChildElementByTagName(element, "handshake-interceptors");
            ...
            List<String> origins =
                    Arrays.asList(StringUtils.tokenizeToStringArray(allowedOrigins, ","));
    ...
    private RootBeanDefinition registerMessageBroker(
        ...
        Element simpleBrokerElem =
                DomUtils.getChildElementByTagName(brokerElement, "simple-broker");
        ...
        if (simpleBrokerElem != null) {
            ...
            if (brokerElement.hasAttribute("path-matcher")) {
                ...
                brokerDef
                        .getPropertyValues()
                        .add("pathMatcher", new RuntimeBeanReference(pathMatcherRef));
            }
            if (simpleBrokerElem.hasAttribute("scheduler")) {
                String scheduler = simpleBrokerElem.getAttribute("scheduler");
                brokerDef
                        .getPropertyValues()
                        .add("taskScheduler", new RuntimeBeanReference(scheduler));
            }
            ...
            if (brokerRelayElem.hasAttribute("heartbeat-send-interval")) {
                values.add(
                        ...
                        brokerRelayElem.getAttribute("heartbeat-send-interval"));
            }
    ...
    private RuntimeBeanReference registerMessageConverter(
        ...
        Element convertersElement =
                DomUtils.getChildElementByTagName(element, "message-converters");
        ...
        if (convertersElement == null
            ...
            if (jackson2Present) {
                ...
                RootBeanDefinition resolverDef =
                        new RootBeanDefinition(DefaultContentTypeResolver.class);
                resolverDef
                        .getPropertyValues()
                        .add("defaultMimeType", MimeTypeUtils.APPLICATION_JSON);
    ...
    private RuntimeBeanReference registerMessagingTemplate(
        ...
        RootBeanDefinition beanDef =
                new RootBeanDefinition(SimpMessagingTemplate.class, cargs, null);
    ...
    private void registerAnnotationMethodMessageHandler(
        ...
        String prefixAttribute =
                messageBrokerElement.getAttribute("application-destination-prefix");
        values.add(
                "destinationPrefixes",
        ...
        RootBeanDefinition beanDef =
                new RootBeanDefinition(
                        WebSocketAnnotationMethodMessageHandler.class, cargs, values);
        ...
        if (resolversElement != null) {
            values.add(
                    "customArgumentResolvers", extractBeanSubElements(resolversElement, context));
        }
        ...
        if (handlersElement != null) {
            values.add(
                    "customReturnValueHandlers", extractBeanSubElements(handlersElement, context));
        }
    ...
    private RuntimeBeanReference getValidator(
        ...
        if (messageBrokerElement.hasAttribute("validator")) {
            ...
            String validatorName =
                    context.getReaderContext().registerWithGeneratedName(validatorDef);
    ...
    private ManagedList<Object> extractBeanSubElements(
            Element parentElement, ParserContext context) {
        ManagedList<Object> list = new ManagedList<>();
        list.setSource(context.extractSource(parentElement));
        for (Element beanElement :
                DomUtils.getChildElementsByTagName(parentElement, "bean", "ref")) {
            Object object = context.getDelegate().parsePropertySubElement(beanElement, null);
            list.add(object);
        }
        return list;
    ...
    private RuntimeBeanReference registerUserDestHandler(
        ...
        Object userDestResolver =
                registerUserDestResolver(brokerElem, userRegistry, context, source);
    ...
    private void registerWebSocketMessageBrokerStats(
        ...
        RuntimeBeanReference webSocketHandler =
