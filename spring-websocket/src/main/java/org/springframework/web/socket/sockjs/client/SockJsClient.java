...
public class SockJsClient implements WebSocketClient, Lifecycle {
     ...
     * requests. Set it if you want only a subset of handshake headers (e.g. auth headers) to be
     * used for other HTTP requests.
     *
     ...
     * an implementation of {@link InfoReceiver} that instance would have been used to initialize
     * this property, or otherwise it defaults to {@link RestTemplateXhrTransport}.
     *
    ...
    /**
     * Configure a {@code TaskScheduler} for scheduling a connect timeout task where the timeout
     * value is calculated based on the duration of the initial SockJS "Info" request. The connect
     * timeout task ensures a more timely fallback but is otherwise entirely optional.
     *
     ...
     * disabled and how long the request took (used for calculating transport timeout time) is
     * cached. This method can be used to clear that cache hence causing it to re-populate.
     */
