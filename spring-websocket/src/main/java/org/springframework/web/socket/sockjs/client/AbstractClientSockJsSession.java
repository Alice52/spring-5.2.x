...
public abstract class AbstractClientSockJsSession implements WebSocketSession {
    ...
    /**
     * Return a timeout cleanup task to invoke if the SockJS sessions is not fully established
     * within the retransmission timeout period calculated in {@code SockJsRequest} based on the
     * duration of the initial SockJS "Info" request.
     */
        ...
        if (this.state == State.NEW) {
            ...
            try {
                ...
                if (logger.isErrorEnabled()) {
                    logger.error(
                            "WebSocketHandler.afterConnectionEstablished threw exception in "
                                    + this,
                            ex);
                }
            }
    ...
    private void handleMessageFrame(SockJsFrame frame) {
        ...
        if (frameData != null) {
            try {
                ...
                if (logger.isErrorEnabled()) {
                    logger.error(
                            "Failed to decode data for SockJS \"message\" frame: "
                                    + frame
                                    + " in "
                                    + this,
                            ex);
        ...
        for (String message : messages) {
            if (isOpen()) {
                try {
                    ...
                    logger.error(
                            "WebSocketHandler.handleMessage threw an exception on "
                                    + frame
                                    + " in "
                                    + this,
                            ex);
